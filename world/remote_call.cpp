...
void RemoteCallResponse(const std::string &connection_id,
	...
	uint32 sz = (uint32)(connection_id.size() + request_id.size() +
	                     error.size() + 3 + 16);
	auto iter = res.begin();
	while (iter != res.end()) {
...
void register_remote_call_handlers() {
	...
	remote_call_methods["World.SaveFileContents"] =
	    handle_rc_save_file_contents;
}
...
void handle_rc_relay(const std::string &method,
	...
	uint32 sz = (uint32)(request_id.size() + connection_id.size() +
	                     method.size() + 3 + 16);
...
// TODO: We need to look at potential security concerns on direct file access
// like this.
