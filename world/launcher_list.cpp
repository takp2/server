...
LauncherList::LauncherList() : nextID(1) {}
...
void LauncherList::Process() {
		...
		if (!l->Process()) {
			...
			if (l != nullptr) {
				Log(Logs::Detail, Logs::WorldServer,
				    "Removing pending launcher %d", l->GetID());
			} else {
				Log(Logs::Detail, Logs::WorldServer,
				    "Removing pending launcher with no known ID");
			}
			...
			if (res != m_launchers.end()) {
				Log(Logs::Detail, Logs::WorldServer, "Ghosting launcher %s",
				    name.c_str());
				delete res->second;
			}
			Log(Logs::Detail, Logs::WorldServer,
			    "Removing pending launcher %d. Adding %s to active list.",
			    l->GetID(), name.c_str());
	...
	while (curl != m_launchers.end()) {
		...
		if (!l->Process()) {
			// launcher has died before it identified itself.
			Log(Logs::Detail, Logs::WorldServer, "Removing launcher %s (%d)",
			    l->GetName(), l->GetID());
...
LauncherLink *LauncherList::Get(const char *name) {
	std::map<std::string, LauncherLink *>::iterator res;
	res = m_launchers.find(name);
	if (res == m_launchers.end()) return (nullptr);
	return (res->second);
...
LauncherLink *LauncherList::FindByZone(const char *short_name) {
	...
	for (; cur != end; ++cur) {
		if (cur->second->ContainsZone(short_name)) return (cur->second);
	}
	return (nullptr);
}
...
void LauncherList::Add(EmuTCPConnection *conn) {
	...
	Log(Logs::Detail, Logs::WorldServer, "Adding pending launcher %d",
	    it->GetID());
	m_pendingLaunchers.push_back(it);
}

int LauncherList::GetLauncherCount() { return (m_launchers.size()); }
