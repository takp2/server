...
void ZSList::ShowUpTime(WorldTCPConnection* con, const char* adminname) {
	...
	con->SendEmoteMessage(
	    adminname, 0, AccountStatus::Player, CC_Default,
	    fmt::format("Worldserver Uptime | {}", time_string).c_str());
}
...
void ZSList::Process() {
	if (shutdowntimer && shutdowntimer->Check()) {
		Log(Logs::Detail, Logs::WorldServer,
		    "Shutdown timer has expired. Telling all zones to shut down and "
		    "exiting. (fake sigint)");
	...
	if (reminder && reminder->Check() && shutdowntimer) {
		SendEmoteMessage(
		    0, 0, AccountStatus::Player, CC_Yellow,
		    fmt::format("[SYSTEM] World coming down in {} minutes.",
	...
	while (iterator.MoreElements()) {
		if (!iterator.GetData()->Process()) {
			...
			in.s_addr = zs->GetIP();
			Log(Logs::Detail, Logs::WorldServer,
			    "Removing zoneserver #%d at %s:%d", zs->GetID(),
...
void ZSList::ListLockedZones(const char* to, WorldTCPConnection* connection) {
	...
	for (auto& zone : pLockedZones) {
		if (zone) {
			connection->SendEmoteMessageRaw(to, 0, AccountStatus::Player,
			                                CC_Default,
			                                database.GetZoneName(zone, true));
			x++;
		}
	}
	connection->SendEmoteMessage(to, 0, AccountStatus::Player, CC_Default,
	                             "%i zones locked.", x);
}

void ZSList::SendZoneStatus(const char* to, int16 admin,
	...
	while (iterator.MoreElements()) {
		...
		if (zone_server_data->GetZoneID()) {
			...
			stream << "" << zone_server_data->GetZoneName() << " ("
			       << zone_server_data->GetZoneID() << ")";
		...
		fmt::format_to(
		    out, "{} {} :: {}  :: {}:{} :: {} :: {}:{} :: {} :: ({})",
		    buffer.c_str(), zone_server_data->GetID(), is_static_string.c_str(),
		    inet_ntoa(in), zone_server_data->GetPort(),
		    zone_server_data->NumPlayers(), zone_server_data->GetCAddress(),
		    zone_server_data->GetCPort(), zone_data_string.c_str(),
		    zone_server_data->GetZoneOSProcessID());

		if (out.size() >= 3584) {
			auto output = fmt::to_string(out);
			connection->SendEmoteMessageRaw(to, 0, AccountStatus::Player,
			                                CC_NPCQuestSay, output.c_str());
			out.clear();
		} else {
			if (connection->IsConsole())
				fmt::format_to(out, "\r\n");
	...
	if (connection->IsConsole()) {
		...
		fmt::format_to(out, "{} {} servers online.^", buffer.c_str(),
		               servers_online);
	}

	fmt::format_to(out,
	               "{} {} zones are static zones, {} zones are booted zones, "
	               "{} zones available.",
	               buffer.c_str(), static_zones, booted_zones, avail_zones);
...
void ZSList::SendZoneCountConsole(const char* to, int16 admin,
                                  WorldTCPConnection* connection,
                                  bool zonepop) {
	LinkedListIterator<ZoneServer*> iterator(list, BACKWARD);
...
void ZSList::SendChannelMessage(const char* from, const char* to,
                                uint8 chan_num, uint8 language,
...
void ZSList::SendChannelMessageRaw(const char* from, const char* to,
                                   uint8 chan_num, uint8 language,
                                   uint8 lang_skill, const char* message) {
	if (!message) return;
...
void ZSList::SendEmoteMessage(const char* to, uint32 to_guilddbid,
...
void ZSList::SendEmoteMessageRaw(const char* to, uint32 to_guilddbid,
                                 int16 to_minstatus, uint32 type,
                                 const char* message) {
	if (!message) return;
	auto pack = new ServerPacket;
	...
	if (to) {
		if (to[0] == '*') {
			...
			if (con)
				con->SendEmoteMessageRaw(to, to_guilddbid, to_minstatus, type,
				                         message);
			delete pack;
			return;
		}
	...
	char tempto[64] = {0};
	if (to) strn0cpy(tempto, to, 64);
	pack->Deflate();
	if (tempto[0] == 0) {
		SendPacket(pack);
		if (to_guilddbid == 0) console_list.SendEmoteMessageRaw(type, message);
	} else {
...
void ZSList::SOPZoneBootup(const char* adminname, uint32 ZoneServerID,
                           const char* zonename, bool iMakeStatic) {
	ZoneServer* zs = 0;
	ZoneServer* zs2 = 0;
	uint32 zoneid;
	if (!(zoneid = database.GetZoneID(zonename)))
		...
		if (ZoneServerID != 0)
			...
			SendEmoteMessage(
			    adminname, 0, AccountStatus::Player, CC_Default,
			    "Error: SOP_ZoneBootup: ServerID must be specified");

		if (!zs)
			SendEmoteMessage(adminname, 0, AccountStatus::Player, CC_Default,
			...
			if (zs2 != 0)
				SendEmoteMessage(
				    adminname, 0, AccountStatus::Player, CC_Default,
				    fmt::format("Error: SOP_ZoneBootup: zone '{}' already "
				                "being hosted by ZoneServer ID {} ",
				                zonename, zs2->GetID())
				        .c_str());
			else {
				zs->TriggerBootup(zoneid, adminname, iMakeStatic);
				SendEmoteMessage(adminname, 0, AccountStatus::Player,
				                 CC_Default,
				                 fmt::format("Booting {} on zoneserverid {} ",
				                             zonename, ZoneServerID)
				                     .c_str());
			}
		}
	}
...
uint16 ZSList::GetAvailableZonePort() {
	...
	while (i != LastAllocatedPort && port == 0) {
		if (i > Config->ZonePortHigh) i = Config->ZonePortLow;
...
uint32 ZSList::TriggerBootup(uint32 iZoneID) {
	...
	iterator.Reset();
	while (iterator.MoreElements()) {
		if (iterator.GetData()->GetZoneID() == 0 &&
		    !iterator.GetData()->IsBootingUp()) {
	...
	/*Old Random boot zones use this if your server is distributed across
	computers. LinkedListIterator<ZoneServer*> iterator(list);
	...
	while(iterator.MoreElements()) {
	    if (iterator.GetData()->GetZoneID() == 0 &&
	!iterator.GetData()->IsBootingUp()) { tmp[y++] = iterator.GetData();
	    }
	    iterator.Advance();
	}
	if (y == 0) {
...
int ZSList::GetZoneCount() { return (numzones); }
...
void ZSList::WorldShutDown(uint32 time, uint32 interval) {
	if (time > 0) {
		SendEmoteMessage(
		    0, 0, AccountStatus::Player, CC_Yellow,
		    fmt::format("[SYSTEM] World will be shutting down in {} minutes.",
		                (time / 60))
		        .c_str());
		...
		SendEmoteMessage(0, 0, AccountStatus::Player, CC_Yellow,
