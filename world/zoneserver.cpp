...
ZoneServer::~ZoneServer() {
	if (RunLoops) client_list.CLERemoveZSRef(this);
	tcpc->Free();
}
...
bool ZoneServer::SetZone(uint32 iZoneID, bool iStaticZone) {
	...
	if (iZoneID)
		Log(Logs::Detail, Logs::WorldServer, "Setting to '%s' (%d)%s",
		    (zn) ? zn : "", iZoneID, iStaticZone ? " (Static)" : "");

	zone_server_zone_id = iZoneID;
	if (iZoneID != 0) zone_server_previous_zone_id = iZoneID;
	...
	if (zn) {
		...
		if (database.GetZoneLongName((char*)zone_name, &longname, nullptr,
...
void ZoneServer::LSBootUpdate(uint32 zoneid, bool startup) {
	if (WorldConfig::get()->UpdateStats) {
		...
		if (startup) strcpy(bootup->compile_time, GetCompileTime());
		bootup->zone = zoneid;
...
void ZoneServer::LSSleepUpdate(uint32 zoneid) {
	if (WorldConfig::get()->UpdateStats) {
		...
		pack->pBuffer = new uchar[pack->size];
		memset(pack->pBuffer, 0, pack->size);
		ServerLSZoneSleep_Struct* sleep =
		    (ServerLSZoneSleep_Struct*)pack->pBuffer;
		sleep->zone = zoneid;
...
bool ZoneServer::Process() {
	if (!tcpc->Connected()) return false;
	...
	while ((pack = tcpc->PopPacket())) {
		...
		switch (pack->opcode) {
			...
			case ServerOP_GroupInvite: {
				if (pack->size != sizeof(GroupInvite_Struct)) break;
			...
			case ServerOP_GroupFollow: {
				if (pack->size != sizeof(ServerGroupFollow_Struct)) break;

				ServerGroupFollow_Struct* sgfs =
				    (ServerGroupFollow_Struct*)pack->pBuffer;

				client_list.SendPacket(sgfs->gf.name1, pack);
				break;
			}
			case ServerOP_GroupFollowAck: {
				if (pack->size != sizeof(ServerGroupFollowAck_Struct)) break;

				ServerGroupFollowAck_Struct* sgfas =
				    (ServerGroupFollowAck_Struct*)pack->pBuffer;

				client_list.SendPacket(sgfas->Name, pack);
				break;
			}
			case ServerOP_GroupCancelInvite: {
				if (pack->size != sizeof(GroupCancel_Struct)) break;
			...
			case ServerOP_GroupSetID: {
				if (pack->size != sizeof(GroupSetID_Struct)) break;
				GroupSetID_Struct* gsi = (GroupSetID_Struct*)pack->pBuffer;
				// set groupid in CLE
				uint32 char_id = gsi->char_id;
				ClientListEntry* client_cle =
				    client_list.FindCLEByCharacterID(gsi->char_id);
				if (client_cle) client_cle->SetGroupID(gsi->group_id);
				break;
			}
			...
			case ServerOP_GroupLeave: {
				if (pack->size != sizeof(ServerGroupLeave_Struct)) break;
				zoneserver_list.SendPacket(pack);  // bounce it to all zones
				break;
			}

			case ServerOP_GroupJoin: {
				if (pack->size != sizeof(ServerGroupJoin_Struct)) break;
				zoneserver_list.SendPacket(pack);  // bounce it to all zones
				break;
			}

			case ServerOP_RaidGroupJoin: {
				if (pack->size != sizeof(ServerRaidGroupJoin_Struct)) break;
				zoneserver_list.SendPacket(pack);  // bounce it to all zones
				break;
			}

			case ServerOP_ForceGroupUpdate: {
				if (pack->size != sizeof(ServerForceGroupUpdate_Struct)) break;
				zoneserver_list.SendPacket(pack);  // bounce it to all zones
				break;
			}
			...
			case ServerOP_OOZGroupMessage: {
				...
				ServerGroupChannelMessage_Struct* gcm =
				    (ServerGroupChannelMessage_Struct*)pack->pBuffer;
				if (gcm->groupid != 0) {
					...
					for (auto it = vec.begin(); it != vec.end(); ++it) {
						...
						if (cle->Online() == CLE_Status_Zoning &&
						    groupid == gcm->groupid && !cle->TellQueueFull()) {
							// queue group chat message
							size_t struct_size =
							    sizeof(ServerChannelMessage_Struct) +
							    strlen(gcm->message) + 1;
							ServerChannelMessage_Struct* scm =
							    (ServerChannelMessage_Struct*)new uchar
							        [struct_size];
							...
							scm->queued = 1;
							cle->PushToTellQueue(
							    scm);  // deallocation is handled in processing
							           // or deconstructor
						}
					}
				}
			...
			case ServerOP_DisbandGroup: {
				if (pack->size != sizeof(ServerDisbandGroup_Struct)) break;
				ServerDisbandGroup_Struct* sdbg =
			...
			case ServerOP_ChangeGroupLeader: {
				if (pack->size != sizeof(ServerGroupLeader_Struct)) break;
				zoneserver_list.SendPacket(pack);  // bounce it to all zones
				break;
			}
			...
			case ServerOP_CheckGroupLeader: {
				...
				ServerGroupLeader_Struct* sgls =
				    (ServerGroupLeader_Struct*)pack->pBuffer;
				ClientListEntry* cle =
				    client_list.FindCharacter(sgls->leader_name);
			...
			case ServerOP_RaidAdd: {
				if (pack->size != sizeof(ServerRaidGeneralAction_Struct)) break;
			...
			case ServerOP_RaidGroupAdd: {
				if (pack->size != sizeof(ServerRaidGroupAction_Struct)) break;
			...
			case ServerOP_RaidSay: {
				...
				// check for characters zoning and queue messages
				ServerRaidMessage_Struct* srm =
				    (ServerRaidMessage_Struct*)pack->pBuffer;
				if (srm->rid != 0) {
					std::vector<ClientListEntry*> vec;
					client_list.GetClients("", vec);
					for (auto it = vec.begin(); it != vec.end(); ++it) {
						...
						if (cle->Online() == CLE_Status_Zoning &&
							...
							if (database.GetRaidGroupID(cle->name(), &raidid,
							                            &groupid)) {
								if (raidid == srm->rid &&
								    (pack->opcode == ServerOP_RaidSay ||
								     groupid == srm->gid)) {
									// queue chat message
									size_t struct_size =
									    sizeof(ServerChannelMessage_Struct) +
									    strlen(srm->message) + 1;
									ServerChannelMessage_Struct* scm =
									    (ServerChannelMessage_Struct*)new uchar
									        [struct_size];
									memset(scm, 0, struct_size);
									...
									scm->chan_num =
									    pack->opcode == ServerOP_RaidGroupSay
									        ? ChatChannel_Group
									        : ChatChannel_Raid;
									scm->queued = 1;
									cle->PushToTellQueue(
									    scm);  // deallocation is handled in
									           // processing or deconstructor
								}
							}
						}
					}
			...
			case ServerOP_SpawnCondition: {
				if (pack->size != sizeof(ServerSpawnCondition_Struct)) break;
				// bounce the packet to the correct zone server, if its up
				ServerSpawnCondition_Struct* ssc =
				    (ServerSpawnCondition_Struct*)pack->pBuffer;
				zoneserver_list.SendPacket(ssc->zoneID, pack);
				break;
			}
			case ServerOP_SpawnEvent: {
				if (pack->size != sizeof(ServerSpawnEvent_Struct)) break;
				// bounce the packet to the correct zone server, if its up
				ServerSpawnEvent_Struct* sse =
				    (ServerSpawnEvent_Struct*)pack->pBuffer;
				zoneserver_list.SendPacket(sse->zoneID, pack);
				break;
			}
			case ServerOP_ChannelMessage: {
				if (pack->size < sizeof(ServerChannelMessage_Struct)) break;
				ServerChannelMessage_Struct* scm =
				    (ServerChannelMessage_Struct*)pack->pBuffer;
				if (scm->chan_num == 20) {
				...
				if (scm->chan_num == ChatChannel_Tell ||
					...
					if (scm->deliverto[0] == '*') {
						Console* con = 0;
						con =
						    console_list.FindByAccountName(&scm->deliverto[1]);
						if (((!con) || (!con->SendChannelMessage(scm))) &&
						    (scm->chan_num == ChatChannel_Tell &&
						     scm->queued == 0))
							zoneserver_list.SendEmoteMessage(
							    scm->from, 0, AccountStatus::Player, CC_Default,
							    fmt::format(" {} is not online at this time ",
							                scm->to)
							        .c_str());
						break;
					}
					...
					// deliver to client
					ClientListEntry* cle =
					    client_list.FindCharacter(scm->deliverto);

					if (cle == 0 || cle->Online() < CLE_Status_Zoning ||
					    (cle->TellsOff() && (scm->fromadmin < cle->Admin() ||
					                         scm->fromadmin < 80))) {
						// client not found or has tells off
						if (scm->chan_num == ChatChannel_Tell) {
							ClientListEntry* sender =
							    client_list.FindCharacter(scm->from);
							if (!sender || !sender->Server()) break;
							scm->chan_num = ChatChannel_TellEcho;
							scm->queued = 3;  // offline
							strcpy(scm->deliverto, scm->from);
							// ideally this would be trimming off the message
							// too, oh well
						...
						if (scm->chan_num == ChatChannel_Tell) {
							...
							if (cle->TellQueueFull()) {
								...
								size_t struct_size =
								    sizeof(ServerChannelMessage_Struct) +
								    strlen(scm->message) + 1;
								ServerChannelMessage_Struct* temp =
								    (ServerChannelMessage_Struct*)new uchar
								        [struct_size];
								memset(
								    temp, 0,
								    struct_size);  // just in case, was seeing
								                   // some corrupt messages, but
								                   // it shouldn't happen
								memcpy(temp, scm, struct_size);
								temp->queued = 1;
								cle->PushToTellQueue(
								    temp);  // deallocation is handled in
								            // processing or deconstructor
						...
						if (scm->chan_num == ChatChannel_Tell)
							zoneserver_list.SendEmoteMessage(
							    scm->from, 0, AccountStatus::Player, CC_Default,
							    fmt::format(
							        " {} is not contactable at this time'",
							        scm->to)
							        .c_str());
					} else
						cle->Server()->SendPacket(pack);
				} else {
					if (scm->chan_num == ChatChannel_OOC ||
					...
					if (scm->chan_num == ChatChannel_Guild ||
					    scm->chan_num == ChatChannel_GMSAY ||
					    scm->chan_num == ChatChannel_Broadcast) {
						// check for characters zoning and see if they would be
						// eligible for this message, and if so, queue message
						std::vector<ClientListEntry*> vec;
						client_list.GetClients("", vec);
						for (auto it = vec.begin(); it != vec.end(); ++it) {
							ClientListEntry* cle = *it;
							if (cle->Online() == CLE_Status_Zoning &&
							    !cle->TellQueueFull()) {
								// is this client one of the targets of this
								// message?
								if ((scm->chan_num == ChatChannel_Guild &&
								     scm->guilddbid == cle->GuildID()) ||
								    (scm->chan_num == ChatChannel_GMSAY &&
								     cle->Admin() >= 80) ||
								    scm->chan_num == ChatChannel_Broadcast) {
									// queue chat message
									size_t struct_size =
									    sizeof(ServerChannelMessage_Struct) +
									    strlen(scm->message) + 1;
									ServerChannelMessage_Struct* scm2 =
									    (ServerChannelMessage_Struct*)new uchar
									        [struct_size];
									...
									scm2->queued = 1;
									cle->PushToTellQueue(
									    scm2);  // deallocation is handled in
									            // processing or deconstructor
								}
							}
						}
			...
			case ServerOP_EmoteMessage: {
				ServerEmoteMessage_Struct* sem =
				    (ServerEmoteMessage_Struct*)pack->pBuffer;
				zoneserver_list.SendEmoteMessageRaw(sem->to, sem->guilddbid,
				                                    sem->minstatus, sem->type,
				                                    sem->message);
				break;
			}
			...
			case ServerOP_RezzPlayer: {
				...
				if (zoneserver_list.SendPacket(pack)) {
					Log(Logs::Detail, Logs::WorldServer,
					    "Sent Rez packet for %s", sRezz->rez.your_name);
				} else {
					Log(Logs::Detail, Logs::WorldServer,
					    "Could not send Rez packet for %s",
					    sRezz->rez.your_name);
				}
				break;
			}
			...
			case ServerOP_MultiLineMsg: {
				ServerMultiLineMsg_Struct* mlm =
				    (ServerMultiLineMsg_Struct*)pack->pBuffer;
				client_list.SendPacket(mlm->to, pack);
				break;
			}
			case ServerOP_SetZone: {
				if (pack->size != sizeof(SetZone_Struct)) break;
			...
			case ServerOP_SetLaunchName: {
				if (pack->size != sizeof(LaunchName_Struct)) break;
				const LaunchName_Struct* ln =
				    (const LaunchName_Struct*)pack->pBuffer;
				launcher_name = ln->launcher_name;
				launched_name = ln->zone_name;
				database.ZoneConnected(database.GetZoneID(ln->zone_name),
				                       ln->zone_name);
				Log(Logs::Detail, Logs::WorldServer,
				    "Zone started with name %s by launcher %s",
				    launched_name.c_str(), launcher_name.c_str());
				break;
			}
			case ServerOP_ShutdownAll: {
				if (pack->size == 0) {
					...
					WorldShutDown_Struct* wsd =
					    (WorldShutDown_Struct*)pack->pBuffer;
					if (wsd->time == 0 && wsd->interval == 0 &&
					    zoneserver_list.shutdowntimer->Enabled()) {
			...
			case ServerOP_ZoneShutdown: {
				ServerZoneStateChange_struct* s =
				    (ServerZoneStateChange_struct*)pack->pBuffer;
				ZoneServer* zs = 0;
				if (s->ZoneServerID != 0)
					zs = zoneserver_list.FindByID(s->ZoneServerID);
				else if (s->zoneid != 0)
					zs = zoneserver_list.FindByName(
					    database.GetZoneName(s->zoneid));
				else
					zoneserver_list.SendEmoteMessage(
					    s->adminname, 0, AccountStatus::Player, CC_Default,
					    "Error: SOP_ZoneShutdown: neither ID nor name "
					    "specified");
			...
			case ServerOP_ZoneBootup: {
				...
				zoneserver_list.SOPZoneBootup(s->adminname, ZoneServerID,
				                              database.GetZoneName(s->zoneid),
				                              s->makestatic);
				break;
			}
			case ServerOP_ZoneStatus: {
				if (pack->size >= 1)
					zoneserver_list.SendZoneStatus((char*)&pack->pBuffer[1],
					                               (uint8)pack->pBuffer[0],
					                               this);
				break;
			}
			case ServerOP_AcceptWorldEntrance: {
				if (pack->size != sizeof(WorldToZone_Struct)) break;

				WorldToZone_Struct* wtz = (WorldToZone_Struct*)pack->pBuffer;
				Client* client = 0;
				client = client_list.FindByAccountID(wtz->account_id);
				if (client != 0) client->Clearance(wtz->response);
				break;
			}
			case ServerOP_ZoneToZoneRequest: {
				//
				// solar: ZoneChange is received by the zone the player is in,
				// then the zone sends a ZTZ which ends up here. This code then
				// find the target (ingress point) and boots it if needed, then
				// sends the ZTZ to it. The ingress server will decide wether
				// the player can enter, then will send back the ZTZ to here.
				// This packet is passed back to the egress server, which will
				// send a ZoneChange response back to the client which can be an
				// error, or a success, in which case the client will
				// disconnect, and their zone location will be saved when
				// ~Client is called, so it will be available when they ask to
				// zone.
				//
				...
				if (GetZoneID() == ztz->current_zone_id) {
					Log(Logs::Detail, Logs::WorldServer,
					    "Processing ZTZ for egress from zone for client %s",
					    ztz->name);
					...
					if (ingress_server) {
						Log(Logs::Detail, Logs::WorldServer,
						    "Found a zone already booted for %s", ztz->name);
						ztz->response = 1;
					}
					/* Boot the Zone*/
					else {
						...
						if (!RuleB(World, DontBootDynamics)) {
							if ((server_id = zoneserver_list.TriggerBootup(
							         ztz->requested_zone_id))) {
								Log(Logs::Detail, Logs::WorldServer,
								    "Successfully booted a zone for %s",
								    ztz->name);
								// bootup successful, ready to rock
								ztz->response = 1;
								ingress_server =
								    zoneserver_list.FindByID(server_id);
							} else {
								Log(Logs::Detail, Logs::WorldServer,
								    "FAILED to boot a zone for %s", ztz->name);
					...
					if (ztz->response != 0 && client) client->LSZoneChange(ztz);
					SendPacket(pack);  // send back to egress server
					if (ingress_server) {
						ingress_server->SendPacket(
						    pack);  // inform target server
					}
				}
				/* Response from Ingress server, route back to egress */
				else {
					Log(Logs::Detail, Logs::WorldServer,
					    "Processing ZTZ for ingress to zone for client %s",
					    ztz->name);
			...
			case ServerOP_ClientList: {
				if (pack->size != sizeof(ServerClientList_Struct)) {
					Log(Logs::Detail, Logs::WorldServer,
					    "Wrong size on ServerOP_ClientList. Got: %d, Expected: "
					    "%d",
					    pack->size, sizeof(ServerClientList_Struct));
					break;
				}
				client_list.ClientUpdate(
				    this, (ServerClientList_Struct*)pack->pBuffer);
				break;
			}
			case ServerOP_ClientListKA: {
				ServerClientListKeepAlive_Struct* sclka =
				    (ServerClientListKeepAlive_Struct*)pack->pBuffer;
				if (pack->size < 4 ||
				    pack->size != 4 + (4 * sclka->numupdates)) {
					Log(Logs::Detail, Logs::WorldServer,
					    "Wrong size on ServerOP_ClientListKA. Got: %d, "
					    "Expected: %d",
					    pack->size, (4 + (4 * sclka->numupdates)));
					break;
				}
			...
			case ServerOP_Who: {
				ServerWhoAll_Struct* whoall =
				    (ServerWhoAll_Struct*)pack->pBuffer;
				...
				client_list.SendWhoAll(whoall->fromid, whoall->from,
				                       whoall->admin, whom);
				safe_delete(whom);
				break;
			}
			case ServerOP_RequestOnlineGuildMembers: {
				ServerRequestOnlineGuildMembers_Struct* srogms =
				    (ServerRequestOnlineGuildMembers_Struct*)pack->pBuffer;
				Log(Logs::Detail, Logs::Guilds,
				    "ServerOP_RequestOnlineGuildMembers Recieved. FromID=%i "
				    "GuildID=%i",
			...
			case ServerOP_FriendsWho: {
				ServerFriendsWho_Struct* FriendsWho =
				    (ServerFriendsWho_Struct*)pack->pBuffer;
				client_list.SendFriendsWho(FriendsWho, this);
				break;
			}
			case ServerOP_ZonePlayer: {
				// ServerZonePlayer_Struct* szp = (ServerZonePlayer_Struct*)
				// pack->pBuffer;
				zoneserver_list.SendPacket(pack);
				break;
			}
			case ServerOP_KickPlayer: {
				ServerKickPlayer_Struct* skp =
				    (ServerKickPlayer_Struct*)pack->pBuffer;
				ClientListEntry* cle =
				    client_list.FindCLEByAccountID(skp->AccountID);
				if (cle) {
			...
			case ServerOP_IsOwnerOnline: {
				if (pack->size != sizeof(ServerIsOwnerOnline_Struct)) break;
				ServerIsOwnerOnline_Struct* online =
				    (ServerIsOwnerOnline_Struct*)pack->pBuffer;
				ClientListEntry* cle =
				    client_list.FindCLEByAccountID(online->accountid);
				if (cle)
					online->online = 1;
				else
					online->online = 0;
			...
			case ServerOP_FlagUpdate: {
				ClientListEntry* cle =
				    client_list.FindCLEByAccountID(*((uint32*)pack->pBuffer));
				if (cle) cle->SetAdmin(*((int16*)&pack->pBuffer[4]));
				zoneserver_list.SendPacket(pack);
				break;
			}
			case ServerOP_GMGoto: {
				if (pack->size != sizeof(ServerGMGoto_Struct)) {
					Log(Logs::Detail, Logs::WorldServer,
					    "Wrong size on ServerOP_GMGoto. Got: %d, Expected: %d",
					...
					if (cle->Server() == 0)
						this->SendEmoteMessage(
						    ...
						    fmt::format(
					         ...
					         cle->Admin() >
					             gmg->admin)  // no snooping for anon GMs
						this->SendEmoteMessage(
						    gmg->myname, 0, AccountStatus::Player, CC_Red,
						    fmt::format("Error: {} not found", gmg->gotoname)
						        .c_str());
					else
						cle->Server()->SendPacket(pack);
				} else {
			...
			case ServerOP_Lock: {
				if (pack->size != sizeof(ServerLock_Struct)) {
					Log(Logs::Detail, Logs::WorldServer,
					    "Wrong size on ServerOP_Lock. Got: %d, Expected: %d",
					    pack->size, sizeof(ServerLock_Struct));
					break;
				}
			...
			case ServerOP_Motd: {
				if (pack->size != sizeof(ServerMotd_Struct)) {
					Log(Logs::Detail, Logs::WorldServer,
					    "Wrong size on ServerOP_Motd. Got: %d, Expected: %d",
					    pack->size, sizeof(ServerMotd_Struct));
					break;
				}
			...
			case ServerOP_Uptime: {
				if (pack->size != sizeof(ServerUptime_Struct)) {
					Log(Logs::Detail, Logs::WorldServer,
					    "Wrong size on ServerOP_Uptime. Got: %d, Expected: %d",
				...
				if (sus->zoneserverid == 0) {
					...
					ZoneServer* zs =
					    zoneserver_list.FindByID(sus->zoneserverid);
					if (zs) zs->SendPacket(pack);
				}
				break;
			}
			...
			case ServerOP_GetWorldTime: {
				...
				tod->start_realtime =
				    zoneserver_list.worldclock.getStartRealTime();
				SendPacket(outpack);
				safe_delete(outpack);
				break;
			}
			case ServerOP_SetWorldTime: {
				...
				zoneserver_list.worldclock.setEQTimeOfDay(
				    newtime->start_eqtime, newtime->start_realtime);
				Log(Logs::Detail, Logs::WorldServer,
				    "New time = %d-%d-%d %d:%d (%d)\n",
				    newtime->start_eqtime.year, newtime->start_eqtime.month,
				    (int)newtime->start_eqtime.day,
				    (int)newtime->start_eqtime.hour,
				    (int)newtime->start_eqtime.minute,
				    (int)newtime->start_realtime);
				database.SaveTime((int)newtime->start_eqtime.minute,
				                  (int)newtime->start_eqtime.hour,
				                  (int)newtime->start_eqtime.day,
				                  newtime->start_eqtime.month,
			...
			case ServerOP_IPLookup: {
				if (pack->size < sizeof(ServerGenericWorldQuery_Struct)) {
					Log(Logs::Detail, Logs::WorldServer,
					    "Wrong size on ServerOP_IPLookup. Got: %d, Expected "
					    "(at least): %d",
					    pack->size, sizeof(ServerGenericWorldQuery_Struct));
					break;
				}
				ServerGenericWorldQuery_Struct* sgwq =
				    (ServerGenericWorldQuery_Struct*)pack->pBuffer;
				if (pack->size == sizeof(ServerGenericWorldQuery_Struct))
			...
			case ServerOP_LockZone: {
				if (pack->size < sizeof(ServerLockZone_Struct)) {
					Log(Logs::Detail, Logs::WorldServer,
					    "Wrong size on ServerOP_LockZone. Got: %d, Expected: "
					    "%d",
					    pack->size, sizeof(ServerLockZone_Struct));
					break;
				}
				ServerLockZone_Struct* lock_zone =
				    (ServerLockZone_Struct*)pack->pBuffer;
				switch (lock_zone->op) {
					case ServerLockType::List:
						zoneserver_list.ListLockedZones(lock_zone->adminname,
						                                this);
						break;
					case ServerLockType::Lock:
						if (zoneserver_list.SetLockedZone(lock_zone->zoneID,
						                                  true))
							zoneserver_list.SendEmoteMessage(
							    0, 0, AccountStatus::QuestTroupe, CC_Yellow,
							    fmt::format(
							        "Zone locked: {} ",
							        database.GetZoneName(lock_zone->zoneID))
							        .c_str());
						else
							this->SendEmoteMessageRaw(
							    lock_zone->adminname, 0, AccountStatus::Player,
							    CC_Default, "Failed to change lock");
						break;
			...
			case ServerOP_SpawnPlayerCorpse: {
				SpawnPlayerCorpse_Struct* s =
				    (SpawnPlayerCorpse_Struct*)pack->pBuffer;
			...
			case ServerOP_Consent: {
				bool success = false;
				ZoneServer* zs;
				ServerOP_Consent_Struct* s =
				    (ServerOP_Consent_Struct*)pack->pBuffer;
				ClientListEntry* cle = client_list.FindCharacter(s->grantname);
				if (cle) {
					zs = zoneserver_list.FindByZoneID(cle->zone());
					if (zs) {
						// Sends packet to player granted consent. This causes
						// zone to call Consent() and sends the granted player a
						// success message.
						if (zs->SendPacket(pack)) {
							success = true;
							ClientListEntry* cle_reply =
							    client_list.FindCharacter(s->ownername);
							if (cle_reply) {
								auto reply = new ServerPacket(
								    ServerOP_Consent_Response,
								    sizeof(ServerOP_Consent_Struct));
								ServerOP_Consent_Struct* scs =
								    (ServerOP_Consent_Struct*)reply->pBuffer;
								...
								zs = zoneserver_list.FindByZoneID(
								    cle_reply->zone());
								if (zs) {
									// Sends packet to owner so they get the
									// success message. If this fails, consent
									// will still occur the owner just won't get
									// a message.
				...
				if (!success) {
					// Sends packet back to owner so they can save the consent
					// to the DB. (Granted player is not online or doesn't
					// exist.)
					auto reply =
					    new ServerPacket(ServerOP_Consent_Response,
					                     sizeof(ServerOP_Consent_Struct));
					ServerOP_Consent_Struct* scs =
					    (ServerOP_Consent_Struct*)reply->pBuffer;
					strcpy(scs->grantname, s->grantname);
			...
			case ServerOP_ConsentDenyByID: {
				if (pack->size != sizeof(ServerOP_ConsentDenyByID_Struct)) {
					Log(Logs::General, Logs::Error,
					    "ServerOP_ConsentDenyByID wrong struct size. Got: %d",
					    pack->size);
					break;
				}

				ServerOP_ConsentDenyByID_Struct* s =
				    (ServerOP_ConsentDenyByID_Struct*)pack->pBuffer;

				LinkedList<ConsentDenied_Struct*> purged_consent;
				database.ClearAllConsented(s->ownername, s->corpse_id,
				                           &purged_consent);

				// Creates a packet telling each player their consent has
				// expired, and if they're online also updates their
				// consent_list.
				LinkedListIterator<ConsentDenied_Struct*> iterator(
				    purged_consent);
				iterator.Reset();
				while (iterator.MoreElements()) {
					...
					if (cle) {
						...
						ServerPacket* scs_pack =
						    new ServerPacket(ServerOP_Consent_Response,
						                     sizeof(ServerOP_Consent_Struct));
						ServerOP_Consent_Struct* scs =
						    (ServerOP_Consent_Struct*)scs_pack->pBuffer;
						strcpy(scs->grantname, gname);
						strcpy(scs->ownername, cd->oname);
						scs->permission = 0;
						scs->message_string_id =
						    2103;  // You have been denied permission to drag
						           // %1's corpse.
						scs->corpse_id = cd->corpse_id;

						Log(Logs::Detail, Logs::WorldServer,
						    "Created ServerOP_Consent_Response packet. Owner: "
						    "%s Granted: %s CorpseID: %d",
						    scs->ownername, scs->grantname, scs->corpse_id);
						ZoneServer* zs =
			...
			case ServerOP_LSAccountUpdate: {
				Log(Logs::Detail, Logs::WorldServer,
				    "Received ServerOP_LSAccountUpdate packet from zone");
			...
			case ServerOP_CZSignalNPC: {
				...
				uint32 zone_id =
				    s->npctype_id / 1000u;  // NPC IDs have the zone IDs in
				                            // them.  who cares about pets
				ZoneServer* zs = zoneserver_list.FindByZoneID(zone_id);
				if (zs) zs->SendPacket(pack);
				break;
			}
			case ServerOP_CZSetEntityVariableByNPCTypeID: {
				CZSetEntVarByNPCTypeID_Struct* s =
				    (CZSetEntVarByNPCTypeID_Struct*)pack->pBuffer;
			...
			case ServerOP_Soulmark: {
				ServerRequestSoulMark_Struct* sss =
				    (ServerRequestSoulMark_Struct*)pack->pBuffer;
				...
				database.LoadSoulMarksForClient(
				    database.GetCharacterID(sss->entry.interrogatename), vec);

				if (!vec.empty()) {
					std::vector<SoulMarkEntry_Struct>::iterator it =
					    vec.begin();
					int i = 0;
			...
			case ServerOP_RequestTellQueue: {
				ServerRequestTellQueue_Struct* rtq =
				    (ServerRequestTellQueue_Struct*)pack->pBuffer;
				ClientListEntry* cle = client_list.FindCharacter(rtq->name);
				if (!cle || cle->TellQueueEmpty()) break;
			...
			case ServerOP_BootDownZones: {
				ServerDownZoneBoot_struct* s =
				    (ServerDownZoneBoot_struct*)pack->pBuffer;
				std::string query = StringFormat(
				    "SELECT zone FROM launcher_zones WHERE enabled = 1");
				auto results = database.QueryDatabase(query);
				if (!results.Success()) {
					Log(Logs::General, Logs::Error, "BootDownZones: %s",
				...
				for (auto row = results.begin(); row != results.end(); ++row) {
					...
					ZoneServer* zs =
					    zoneserver_list.FindByName(zone_name.c_str());
					if (zs == nullptr) {
						uint32 zoneserverid =
						    zoneserver_list.GetAvailableZoneID();
						if (zoneserverid == 0) {
							break;
						}

						zoneserver_list.SOPZoneBootup(s->adminname,
						                              zoneserverid,
						                              zone_name.c_str(), true);
					}
				}
			...
			default: {
				Log(Logs::Detail, Logs::WorldServer,
				    "Unknown ServerOPcode from zone 0x%04x, size %d",
...
void ZoneServer::SendEmoteMessage(const char* to, uint32 to_guilddbid,
...
void ZoneServer::SendEmoteMessageRaw(const char* to, uint32 to_guilddbid,
                                     int16 to_minstatus, uint32 type,
                                     const char* message) {
	if (!message) return;
	auto pack = new ServerPacket;
...
void ZoneServer::SendGroupIDs() {
	auto pack = new ServerPacket(ServerOP_GroupIDReply,
	                             sizeof(ServerGroupIDReply_Struct));
...
void ZoneServer::ChangeWID(uint32 iCharID, uint32 iWID) {
	auto pack =
	    new ServerPacket(ServerOP_ChangeWID, sizeof(ServerChangeWID_Struct));
...
void ZoneServer::TriggerBootup(uint32 iZoneID, const char* adminname,
                               bool iMakeStatic) {
	is_booting_up = true;
	zone_server_zone_id = iZoneID;

	auto pack = new ServerPacket(ServerOP_ZoneBootup,
	                             sizeof(ServerZoneStateChange_struct));
	ServerZoneStateChange_struct* s =
	    (ServerZoneStateChange_struct*)pack->pBuffer;
	s->ZoneServerID = zone_server_id;
	if (adminname != 0) strcpy(s->adminname, adminname);
...
void ZoneServer::IncomingClient(Client* client) {
	is_booting_up = true;
	auto pack = new ServerPacket(ServerOP_ZoneIncClient,
	                             sizeof(ServerZoneIncomingClient_Struct));
	ServerZoneIncomingClient_Struct* s =
	    (ServerZoneIncomingClient_Struct*)pack->pBuffer;
	s->zoneid = GetZoneID();
	s->wid = client->GetWID();
	s->ip = client->GetIP();
	...
	if (client->GetCLE()) s->tellsoff = client->GetCLE()->TellsOff();
