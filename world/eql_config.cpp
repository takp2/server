...
EQLConfig::EQLConfig(const char *launcher_name) : m_name(launcher_name) {
	LoadSettings();
}
...
void EQLConfig::LoadSettings() {
	LauncherZone tmp;
	char namebuf[128];
	database.DoEscapeString(namebuf, m_name.c_str(),
	                        m_name.length() & 0x3F);  // limit len to 64
	namebuf[127] = '\0';

	std::string query = StringFormat(
	    "SELECT dynamics FROM launcher WHERE name = '%s'", namebuf);
	auto results = database.QueryDatabase(query);
	if (!results.Success()) {
		LogError("EQLConfig::LoadSettings: {} ",
	...
	query = StringFormat(
	    "SELECT zone, port FROM launcher_zones WHERE launcher = '%s' AND "
	    "enabled = 1",
	    namebuf);
...
EQLConfig *EQLConfig::CreateLauncher(const char *name, uint8 dynamic_count) {
	char namebuf[128];
	database.DoEscapeString(namebuf, name,
	                        strlen(name) & 0x3F);  // limit len to 64
	namebuf[127] = '\0';

	std::string query =
	    StringFormat("INSERT INTO launcher (name, dynamics) VALUES('%s', %d)",
	                 namebuf, dynamic_count);
	auto results = database.QueryDatabase(query);
	if (!results.Success()) {
		return nullptr;
	}
...
void EQLConfig::DeleteLauncher() {
	...
	std::string query =
	    StringFormat("DELETE FROM launcher WHERE name = '%s'", namebuf);
	auto results = database.QueryDatabase(query);
	if (!results.Success()) {
		return;
	}

	query = StringFormat("DELETE FROM launcher_zones WHERE launcher = '%s'",
	                     namebuf);
	results = database.QueryDatabase(query);
	if (!results.Success()) {
		return;
...
bool EQLConfig::BootStaticZone(Const_char *short_name, uint16 port) {
	...
	if (database.GetZoneID(short_name) == 0) return false;
	...
	namebuf[127] = '\0';
	char zonebuf[32];
	database.DoEscapeString(zonebuf, short_name,
	                        strlen(short_name) & 0xF);  // limit len to 16
	zonebuf[31] = '\0';
...
bool EQLConfig::SetDynamicCount(int count) {
	...
	std::string query = StringFormat(
	    "UPDATE launcher SET dynamics=%d WHERE name='%s'", count, namebuf);
	auto results = database.QueryDatabase(query);
	if (!results.Success()) {
		return false;
	}
...
int EQLConfig::GetDynamicCount() const { return (m_dynamics); }

std::map<std::string, std::string> EQLConfig::GetZoneDetails(
    Const_char *zone_ref) {
