...
class ClientList {
   public:
	...
	void SendWhoAll(uint32 fromid, const char* to, int16 admin,
	                Who_All_Struct* whom);
	...
	void SendCLEList(const int16& admin, const char* to,
	                 WorldTCPConnection* connection, const char* iName = 0);
	...
	void CLEAdd(uint32 iLSID, const char* iLoginName, const char* iForumName,
	            const char* iLoginKey, int16 iWorldAdmin = 0, uint32 ip = 0,
	            uint8 local = 0, uint8 version = 0);
	...
	bool WhoAllFilter(ClientListEntry* client, Who_All_Struct* whom,
	                  int16 admin, int whomlen);
   ...
   protected:
	...
	// this is the list of people in any zone, not nescesarily connected to
	// world
	Timer CLStale_timer;
	uint32 NextCLEID;
