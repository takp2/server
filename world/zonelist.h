...
class ZSList {
   public:
	...
	void SendChannelMessage(const char* from, const char* to, uint8 chan_num,
	                        uint8 language, uint8 lang_skill,
	                        const char* message, ...);
	void SendChannelMessageRaw(const char* from, const char* to, uint8 chan_num,
	                           uint8 language, uint8 lang_skill,
	                           const char* message);
	...
	void SendZoneStatus(const char* to, int16 admin,
	                    WorldTCPConnection* connection);
	void SendZoneCountConsole(const char* to, int16 admin,
	                          WorldTCPConnection* connection,
	                          bool zonepop = false);
