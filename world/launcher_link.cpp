...
LauncherLink::LauncherLink(int id, EmuTCPConnection *c)
    : ID(id), tcpc(c), authenticated(false), m_name(""), m_bootTimer(2000) {
	m_dynamicCount = 0;
	m_bootTimer.Disable();
}

LauncherLink::~LauncherLink() { tcpc->Free(); }
	...
	while ((pack = tcpc->PopPacket())) {
		...
		switch (pack->opcode) {
			...
			case ServerOP_ZAAuth: {
				Log(Logs::Detail, Logs::WorldServer,
				    "Got authentication from %s when they are already "
				    "authenticated.",
				    m_name.c_str());
				break;
			}
			case ServerOP_LauncherConnectInfo: {
				const LauncherConnectInfo *it =
				    (const LauncherConnectInfo *)pack->pBuffer;
				if (HasName()) {
					Log(Logs::Detail, Logs::WorldServer,
					    "Launcher '%s' received an additional connect packet "
					    "with name '%s'. Ignoring.",
					    m_name.c_str(), it->name);
					break;
				}
				m_name = it->name;
				...
				if (config == nullptr) {
					Log(Logs::Detail, Logs::WorldServer,
					    "Unknown launcher '%s' connected. Disconnecting.",
					    it->name);
					Disconnect();
					break;
				}
				...
				Log(Logs::Detail, Logs::WorldServer,
				    "Launcher Identified itself as '%s'. Loading zone list.",
				    it->name);
				...
				for (; cur != end; cur++) {
					...
					zs.starts = 0;
					Log(Logs::Detail, Logs::WorldServer,
					    "%s: Loaded zone '%s' on port %d", m_name.c_str(),
					    cur->name.c_str(), zs.port);
			...
			case ServerOP_LauncherZoneStatus: {
				const LauncherZoneStatus *it =
				    (const LauncherZoneStatus *)pack->pBuffer;
				std::map<std::string, ZoneState>::iterator res;
				res = m_states.find(it->short_name);
				if (res == m_states.end()) {
					Log(Logs::Detail, Logs::WorldServer,
					    "%s: reported state for zone %s which it does not "
					    "have.",
					    m_name.c_str(), it->short_name);
					break;
				}
				Log(Logs::Detail, Logs::WorldServer,
				    "%s: %s reported state %s (%d starts)", m_name.c_str(),
				    it->short_name, it->running ? "STARTED" : "STOPPED",
				    it->start_count);
			...
			default: {
				Log(Logs::Detail, Logs::WorldServer,
				    "Unknown ServerOPcode from launcher 0x%04x, size %d",
...
void LauncherLink::BootZone(const char *short_name, uint16 port) {
	...
	zs.up = false;
	zs.starts = 0;
	Log(Logs::Detail, Logs::WorldServer, "%s: Loaded zone '%s' on port %d",
	    m_name.c_str(), short_name, zs.port);
...
void LauncherLink::StartZone(const char *short_name, uint16 port) {
	auto pack = new ServerPacket(ServerOP_LauncherZoneRequest,
	                             sizeof(LauncherZoneRequest));
...
void LauncherLink::BootDynamics(uint8 new_count) {
	if (m_dynamicCount == new_count) return;
	...
	if (m_dynamicCount < new_count) {
		...
		for (r = m_dynamicCount; r < new_count; r++) {
			...
			for (index = m_dynamicCount + 1; index < 255; index++) {
				...
				if (m_states.find(nbuf) != m_states.end()) continue;
				m_states[nbuf] = zs;
				StartZone(nbuf);
				break;
			}
		...
		// quick and dirty way to do this.. should do better (like looking for
		// idle zones)
		int found = 0;
		std::map<std::string, ZoneState>::iterator cur, end;
		cur = m_states.begin();
...
void LauncherLink::GetZoneDetails(const char *short_name,
