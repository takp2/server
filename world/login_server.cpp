...
LoginServer::LoginServer(const char* iAddress, uint16 iPort,
                         const char* Account, const char* Password, uint8 Type)
...
LoginServer::~LoginServer() { delete tcpc; }
	...
	while ((pack = tcpc->PopPacket())) {
		...
		switch (pack->opcode) {
			...
			case ServerOP_UsertoWorldReq: {
				UsertoWorldRequest_Struct* utwr =
				    (UsertoWorldRequest_Struct*)pack->pBuffer;
				...
				UsertoWorldResponse_Struct* utwrs =
				    (UsertoWorldResponse_Struct*)outpack->pBuffer;
				utwrs->lsaccountid = utwr->lsaccountid;
				utwrs->ToID = utwr->FromID;

				if (Config->Locked == true) {
					if (status < 80 && status > -1) utwrs->response = 0;
					if (status >= 80) utwrs->response = 1;
				} else {
					utwrs->response = 1;
				}

				int32 x = Config->MaxClients;
				if ((int32)numplayers >= x && x != -1 && x != 255 &&
				    status < 80) {
					utwrs->response = -3;
				}

				if (status == -1) utwrs->response = -1;
				if (status == -2) utwrs->response = -2;
				...
				if (RuleI(World, AccountSessionLimit) >= 0 &&
				    status < (RuleI(World, ExemptAccountLimitStatus)) &&
				    (RuleI(World, ExemptAccountLimitStatus) != -1) &&
				    client_list.CheckAccountActive(id)) {
					utwrs->response = -4;
					SendPacket(outpack);
					delete outpack;
					break;
				...
				if (!mule && RuleI(World, MaxClientsPerIP) >= 0 &&
				    !client_list.CheckIPLimit(id, utwr->ip, status)) {
					utwrs->response = -5;
					SendPacket(outpack);
					delete outpack;
				}
			...
			case ServerOP_LSClientAuth: {
				...
				client_list.CLEAdd(slsca->lsaccount_id, "eqemu",
				                   slsca->loginserver_name, slsca->key,
				                   slsca->worldadmin, slsca->ip, slsca->local);
				break;
			}
			case ServerOP_LSFatalError: {
				...
				Log(Logs::Detail, Logs::WorldServer,
				    "Login server responded with FatalError. Disabling "
				    "reconnect.");
#else
				Log(Logs::Detail, Logs::WorldServer,
				    "Login server responded with FatalError.");
#endif
				if (pack->size > 1) {
					Log(Logs::Detail, Logs::WorldServer, "     %s",
					    pack->pBuffer);
				}
			...
			case ServerOP_SystemwideMessage: {
				ServerSystemwideMessage* swm =
				    (ServerSystemwideMessage*)pack->pBuffer;
				zoneserver_list.SendEmoteMessageRaw(0, 0, AccountStatus::Player,
				                                    swm->type, swm->message);
				break;
			}
			...
			case ServerOP_LSAccountUpdate: {
				Log(Logs::Detail, Logs::WorldServer,
				    "Received ServerOP_LSAccountUpdate packet from "
				    "loginserver");
			...
			default: {
				Log(Logs::Detail, Logs::WorldServer,
				    "Unknown LSOpCode: 0x%04x size=%d", (int)pack->opcode,
				    pack->size);
...
void LoginServer::SendAccountUpdate(ServerPacket* pack) {
	ServerLSAccountUpdate_Struct* s =
	    (ServerLSAccountUpdate_Struct*)pack->pBuffer;
	if (CanUpdate()) {
		Log(Logs::Detail, Logs::WorldServer,
