...
void UCSConnection::SetConnection(EmuTCPConnection *inStream) {
	if (Stream) {
		Log(Logs::Detail, Logs::UCSServer,
		    "Incoming UCS Connection while we were already connected to a "
		    "UCS.");
		Stream->Disconnect();
	}
	...
	while ((pack = Stream->PopPacket())) {
		...
		switch (pack->opcode) {
			...
			case ServerOP_ZAAuth: {
				Log(Logs::Detail, Logs::UCSServer,
				    "Got authentication from UCS when they are already "
				    "authenticated.");
				break;
			}
			default: {
				Log(Logs::Detail, Logs::UCSServer,
				    "Unknown ServerOPcode from UCS 0x%04x, size %d",
...
void UCSConnection::SendMessage(const char *From, const char *Message) {
	auto pack = new ServerPacket(ServerOP_UCSMessage,
	                             strlen(From) + strlen(Message) + 2);
