...
glm::vec3 PathfinderNavmesh::GetRandomLocation(const glm::vec3 &start, int flags) {
	...
	dtStatus result = m_impl->query->findRandomPointAroundCircle(
	    start_ref, &current_location[0], 100.f, &filter, []() { return (float)zone->random.Real(0.0, 1.0); }, &randomRef, point);
