...
void QGlobalCache::LoadByNPCID(uint32 npcID) {
	std::string query = StringFormat(
	    "SELECT name, charid, npcid, zoneid, value, expdate "
	    "FROM quest_globals WHERE npcid = %d",
	    npcID);
	LoadBy(query);
}
...
void QGlobalCache::LoadByCharID(uint32 charID) {
	std::string query = StringFormat(
	    "SELECT name, charid, npcid, zoneid, value, expdate "
	    "FROM quest_globals WHERE charid = %d && npcid = 0",
	    charID);
	LoadBy(query);
}
...
void QGlobalCache::LoadByZoneID(uint32 zoneID) {
	std::string query = StringFormat(
	    "SELECT name, charid, npcid, zoneid, value, expdate "
	    "FROM quest_globals WHERE zoneid = %d && npcid = 0 && charid = 0",
	    zoneID);
	LoadBy(query);
}
