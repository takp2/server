...
#include "client_packet.h"
	...
	inline void ClearAllProximities() {
		entity_list.ProcessMove(this, glm::vec3(FLT_MAX, FLT_MAX, FLT_MAX));
		m_Proximity = glm::vec3(FLT_MAX, FLT_MAX, FLT_MAX);
	}
	...
	*/
	...
	bool Hungry() const {
		if (GetGM()) return false;
		return m_pp.hunger_level <= 3000;
	}  // Stomach is below the auto-consume threeshold of 3000.
	bool Thirsty() const {
		if (GetGM()) return false;
		return m_pp.thirst_level <= 3000;
	}
	bool Famished() const {
		if (GetGM()) return false;
		return m_pp.thirst_level <= 0 || m_pp.hunger_level <= 0;
	}  // Stomach is empty, sickness is eminent.
	bool FoodFamished() const {
		if (GetGM()) return false;
		return m_pp.hunger_level <= 0;
	}
	bool WaterFamished() const {
		if (GetGM()) return false;
		return m_pp.thirst_level <= 0;
	}
	bool FamishedSickness() const {
		if (GetGM()) return false;
		return m_pp.famished >= RuleI(Character, FamishedLevel) && Famished();
	}  // HP and Mana regen has stopped, Endurance will drop to 0.
	...
	void SetPendingRezzData(int XP, uint32 DBID, uint16 SpellID, const char* CorpseName) {
		PendingRezzXP = XP;
		PendingRezzDBID = DBID;
	...
	void SetActiveDisc(uint8 value, int16 spellid) {
		active_disc = value;
		active_disc_spell = spellid;
	}
	void FadeDisc() {
		BuffFadeBySpellID(active_disc_spell);
		p_timers.Clear(&database, pTimerSpellStart + active_disc_spell);
		active_disc = 0;
		active_disc_spell = 0;
		Log(Logs::General, Logs::Discs, "Fading currently enabled disc.");
	}
