...
void Client::Handle_Connect_OP_ZoneEntry(const EQApplicationPacket *app) {
	...
	*/
	...
	if (ClientVersion() != EQ::versions::ClientVersion::RoF2) {
		LogError("Client account {} character {} IP {} version is not RoF2, disconnecting client", AccountName(), GetName(), GetIP());
		Kick();
		eqs->Close();
		return;
	}
...
#if EQDEBUG >= 11
	Log(Logs::General, Logs
	    : Trading, "%s, purchase item.", GetName());
	DumpPacket(app);
#endif
...
void Client::Handle_OP_TargetCommand(const EQApplicationPacket *app) {
	...
	if (new_tar) {
			...
			if (DistanceSquared(GetBindSightTarget()->GetPosition(), new_tar->GetPosition()) > (zone->newzone_data.maxclip * zone->newzone_data.maxclip)) {
				if (DistanceSquared(m_Position, new_tar->GetPosition()) > (zone->newzone_data.maxclip * zone->newzone_data.maxclip + 40000)) {
					auto hacker_str = fmt::format(
					    " {} attempting to target something beyond the clip plane of {:.2f} units,"
		...
		} else if (DistanceSquared(m_Position, new_tar->GetPosition()) > (zone->newzone_data.maxclip * zone->newzone_data.maxclip + 40000)) {  // client will allow targeting something just beyond max clip just out of sight, so add another 200 for that.
			auto hacker_str = fmt::format(
			    " {} attempting to target something beyond the clip plane of {:.2f} units,"
			    " from ( {:.2f} , {:.2f} , {:.2f} to {} ( {:.2f} , {:.2f} , {:.2f} )",
			    GetName(),
