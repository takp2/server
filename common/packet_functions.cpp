...
void eqemu_free_func(voidpf opaque, voidpf address) { delete[] (char*)address; }
#endif
...
uint32 EstimateDeflateBuffer(uint32 len) {
	...
	if (deflateInit(&zstream, Z_FINISH) != Z_OK) return 0;
...
int DeflatePacket(const unsigned char* in_data, int in_length,
	...
	if (in_data == nullptr && out_data == nullptr && in_length == 0 &&
	    max_out_length == 0) {
...
uint32 InflatePacket(const uchar* indata, uint32 indatalen, uchar* outdata,
                     uint32 outdatalen, bool iQuiet) {
#ifdef REUSE_ZLIB
	static bool inited = false;
	static z_stream zstream;
	int zerror;

	if (indata == nullptr && outdata == nullptr && indatalen == 0 &&
	    outdatalen == 0) {
		// special delete state
		inflateEnd(&zstream);
		return (0);
	...
	if (zerror == Z_STREAM_END) {
		...
		if (!iQuiet) {
			std::cout << "Error: InflatePacket: inflate() returned " << zerror
			          << " '";
			if (zstream.msg) std::cout << zstream.msg;
			std::cout << "'" << std::endl;
#ifdef EQDEBUG
...
uint32 CRCLookup(uchar idx) {
	...
	if (idx == 1) return 0x77073096;

	if (idx == 2) return roll(CRCLookup(1), 1);

	if (idx == 4) return 0x076DC419;
