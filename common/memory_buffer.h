...
class MemoryBuffer {
   public:
	...
	friend MemoryBuffer operator+(MemoryBuffer lhs, const MemoryBuffer &rhs) {
		return lhs += rhs;
	}
	~MemoryBuffer();
	...
	void Write(T val) {
		static_assert(std::is_pod<T>::value,
		              "MemoryBuffer::Write<T>(T val) only works on pod and "
		              "string types.");
	...
	T Read() {
		static_assert(
		    std::is_pod<T>::value,
		    "MemoryBuffer::Read<T>() only works on pod and string types.");
		T temp;
...
class OutBuffer : public std::stringstream {
   public:
	...
	void overwrite(OutBuffer::pos_type position, const char *_Str,
