...
EQStreamIdentifier::~EQStreamIdentifier() {
	...
	for (; cur != end; ++cur) {
		Record *r = *cur;
		if (r != nullptr) r->stream->ReleaseFromUse();
		delete r;
	}
...
void EQStreamIdentifier::RegisterPatch(const EQStream::Signature &sig,
                                       const char *name,
...
void EQStreamIdentifier::RegisterOldPatch(const EQStream::Signature &sig,
                                          const char *name,
                                          OpcodeManager **opcodes,
                                          const StructStrategy *structs) {
	...
	while (cur != m_streams.end()) {
		...
		if (r->expire.Check(false)) {
			...
			Log(Logs::General, Logs::Netcode,
			    "[IDENTIFY] Unable to identify stream from %s:%d before "
			    "timeout.",
			    long2ip(r->stream->GetRemoteIP()).c_str(),
			    ntohs(r->stream->GetRemotePort()));
		...
		if (r->stream->GetState() != ESTABLISHED) {
			...
			Log(Logs::General, Logs::Netcode,
			    "[IDENTIFY] Unable to identify stream from %s:%d before it "
			    "closed.",
			    long2ip(r->stream->GetRemoteIP()).c_str(),
			    ntohs(r->stream->GetRemotePort()));
			switch (r->stream->GetState()) {
				case ESTABLISHED:
					Log(Logs::General, Logs::Netcode,
					    "[IDENTIFY] Stream state was Established");
					break;
				case CLOSING:
					Log(Logs::General, Logs::Netcode,
					    "[IDENTIFY] Stream state was Closing");
					break;
				case DISCONNECTING:
					Log(Logs::General, Logs::Netcode,
					    "[IDENTIFY] Stream state was Disconnecting");
					break;
				case CLOSED:
					Log(Logs::General, Logs::Netcode,
					    "[IDENTIFY] Stream state was Closed");
					break;
				default:
					Log(Logs::General, Logs::Netcode,
					    "[IDENTIFY] Stream state was Unestablished or unknown");
					break;
			}
			r->stream->ReleaseFromUse();
			delete r;
		...
		bool all_ready =
		    true;  //"all signatures were ready to check the stream"
		...
		for (; !found_one && curp != endp; ++curp) {
			...
			switch (res) {
				case EQStream::MatchNotReady:
					// the stream has not received enough packets to compare
					// with this signature
					//				Log.LogDebugType(Logs::General, Logs::Netcode,
					//"[IDENT_TRACE] %s:%d: Tried patch %s, but stream is not
					//ready for it.", long2ip(r->stream->GetRemoteIP()).c_str(),
					//ntohs(r->stream->GetRemotePort()), p->name.c_str());
					all_ready = false;
					break;
				case EQStream::MatchSuccessful: {
					...
					Log(Logs::General, Logs::Netcode,
					    "[IDENTIFY] Identified stream %s:%d with signature %s",
					    long2ip(r->stream->GetRemoteIP()).c_str(),
					    ntohs(r->stream->GetRemotePort()), p->name.c_str());

					// before we assign the eqstream to an interface, let the
					// stream recognize it is in use and the session should not
					// be reset any further
					r->stream->SetActive(true);

					// might want to do something less-specific here... some
					// day..
					EQStreamInterface *s =
					    new EQStreamProxy(r->stream, p->structs, p->opcodes);
					m_identified.push(s);
				...
				case EQStream::MatchFailed:
					// do nothing...
					Log(Logs::General, Logs::Netcode,
					    "[IDENT_TRACE] %s:%d: Tried patch %s, and it did not "
					    "match.",
					    long2ip(r->stream->GetRemoteIP()).c_str(),
					    ntohs(r->stream->GetRemotePort()), p->name.c_str());
					break;
		...
		if (all_ready && !found_one) {
			...
			Log(Logs::General, Logs::Netcode,
			    "[IDENTIFY] Unable to identify stream from %s:%d, no match "
			    "found.",
			    long2ip(r->stream->GetRemoteIP()).c_str(),
			    ntohs(r->stream->GetRemotePort()));
			r->stream->ReleaseFromUse();
	...
	while (oldcur != m_oldstreams.end()) {
		...
		if (r != nullptr && r->expire.Check(false)) {
			...
			Log(Logs::Detail, Logs::Netcode,
			    "Unable to identify stream from %s:%d before timeout.",
			    long2ip(r->stream->GetRemoteIP()).c_str(),
			    ntohs(r->stream->GetRemotePort()));
			r->stream->ReleaseFromUse();
			delete r;
		...
		if (r != nullptr && r->stream->GetState() != ESTABLISHED) {
			// the stream closed before it was identified.
			Log(Logs::Detail, Logs::Netcode,
			    "Unable to identify stream from %s:%d before it closed.",
			    long2ip(r->stream->GetRemoteIP()).c_str(),
			    ntohs(r->stream->GetRemotePort()));
			switch (r->stream->GetState()) {
				case ESTABLISHED:
					Log(Logs::Detail, Logs::Netcode,
					    "Stream state was Established");
					break;
				case CLOSING:
					Log(Logs::Detail, Logs::Netcode,
					    "Stream state was Closing");
					break;
				case DISCONNECTING:
					Log(Logs::Detail, Logs::Netcode,
					    "Stream state was Disconnecting");
					break;
				case CLOSED:
				...
				default:
					Log(Logs::Detail, Logs::Netcode,
					    "Stream state was Unestablished or unknown");
					break;
			}
			r->stream->ReleaseFromUse();
			delete r;
		...
		for (; !found_one && oldcurp != oldendp; ++oldcurp) {
			...
			switch (res) {
				case EQStream::MatchNotReady:
					// the stream has not received enough packets to compare
					// with this signature
					//				_log(NET__IDENT_TRACE, "%s:%d: Tried patch %s, but
					//stream is not ready for it.",
					//long2ip(r->stream->GetRemoteIP()).c_str(),
					//ntohs(r->stream->GetRemotePort()), p->name.c_str());
					all_ready = false;
					break;
				case EQStream::MatchSuccessful: {
					...
					Log(Logs::Detail, Logs::Netcode,
					    "Identified stream %s:%d with signature %s",
					    long2ip(r->stream->GetRemoteIP()).c_str(),
					    ntohs(r->stream->GetRemotePort()), p->name.c_str());
				...
				case EQStream::MatchFailed:
					// do nothing...
					Log(Logs::Detail, Logs::Netcode,
					    "%s:%d: Tried patch %s, and it did not match.",
					    long2ip(r->stream->GetRemoteIP()).c_str(),
					    ntohs(r->stream->GetRemotePort()), p->name.c_str());
					break;
			}
		...
		if (all_ready && !found_one) {
			// the stream cannot be identified.
			Log(Logs::Detail, Logs::Netcode,
			    "Unable to identify stream from %s:%d, no match found.",
			    long2ip(r->stream->GetRemoteIP()).c_str(),
			    ntohs(r->stream->GetRemotePort()));
			r->stream->ReleaseFromUse();
		}
...
EQStreamInterface *EQStreamIdentifier::PopIdentified() {
	if (m_identified.empty()) return (nullptr);
...
EQStreamIdentifier::Record::Record(EQStream *s)
    : stream(std::move(s)), expire(STREAM_IDENT_WAIT_MS) {
	expire.Start();
}

EQStreamIdentifier::OldRecord::OldRecord(EQOldStream *s)
    : stream(s), expire(STREAM_IDENT_WAIT_MS) {
	expire.Start();
}
