...
/*
 * This is an additive lagged fibonacci generator as seen in The Art of Computer
 * Programming, Vol. 2 This should roughly match the implementation that EQ's
 * client uses and be compatible with our Random class
 *
 * EQ's rand looks like it was from an example implementation that as posted on
 * pscode.com
 *
 * You might also want to consider defining BIASED_INT_DIST as well to more
 * closely match EQ
 */
...
class additive_lagged_fibonacci_engine {
   ...
   public:
	...
	static constexpr result_type default_seed =
	    19780503u;  // default for subtract_with_carry_engine

	additive_lagged_fibonacci_engine()
	    : additive_lagged_fibonacci_engine(default_seed) {}
	...
	result_type operator()() {
		result_type rand =
		    (state[state1] + state[state2]) & ((1u << word_size) - 1);
		state[state1] = rand;
		if (++state1 == long_lag) state1 = 0;
		if (++state2 == long_lag) state2 = 0;
   ...
   public:
	...
	friend bool operator==(
	    const additive_lagged_fibonacci_engine<UInt, W, J, K> &x,
	    const additive_lagged_fibonacci_engine<UInt, W, J, K> &y) {
		return std::equal(x.state, x.state + long_lag, y.state) &&
		       x.state1 == y.state1 && x.state2 == y.state2;
	}
	...
	template <typename UInt, size_t W, size_t J, size_t K, typename CharT,
	          typename Traits>
	friend std::basic_ostream<CharT, Traits> &operator<<(
	    std::basic_istream<CharT, Traits> &os,
	    additive_lagged_fibonacci_engine<UInt, W, J, K> &x) {
		...
		for (size_t i = 0; i < long_lag; ++i) os << x.state[i] << space;
	...
	friend std::basic_istream<CharT, Traits> &operator>>(
		...
		for (size_t i = 0; i < long_lag; ++i) is >> x.state[i];
		is >> x.state1;
		is >> x.state2;
