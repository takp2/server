...
#define PM_FINISHING \
	1  // Comment: manager received closing bits and is going to send final
	   // packet
...
type HI_LOSWAPshort(type a) {
	return (LO_BYTE(a) << 8) | (HI_BYTE(a) >> 8);
}
template <typename type>  // HI_LOSWAPlong
type HI_LOSWAPlong(type a) {
	return (LO_WORD(a) << 16) | (HIWORD(a) >> 16);
}
...
class EQStream : public EQStreamInterface {
   ...
   protected:
	...
	std::map<unsigned short, EQProtocolPacket *>
	    PacketQueue;  // not mutex protected, only accessed by caller of
	                  // Process()
	Mutex MInboundQueue;
	...
	EQRawApplicationPacket *MakeApplicationPacket(const unsigned char *buf,
	                                              uint32 len);
   ...
   public:
	...
	void AddBytesSent(uint32 bytes) { bytes_sent += bytes; }

	void AddBytesRecv(uint32 bytes) { bytes_recv += bytes; }
	...
	virtual const uint32 GetBytesSentPerSecond() const {
		if ((Timer::GetTimeSeconds() - create_time) == 0) return 0;
	...
	typedef enum { MatchNotReady,
		           MatchSuccessful,
		           MatchFailed } MatchState;
...
class EQOldStream : public EQStreamInterface {
   ...
   public:
	...
	int16 PacketsPending() { return packetspending; }
	...
	void MakeEQPacket(EQProtocolPacket *app,
	                  bool ack_req = true);  // Make a fragment eq packet and
	                                         // put them on the SQUEUE/RSQUEUE
	...
	std::vector<EQRawApplicationPacket *>
	    OutQueue;  // parced packets ready to go out of this class
   ...
   public:
	...
	float GetDataRate() {
		return (float)datarate_sec / 1024;
	}  // conversion back to kb/second
