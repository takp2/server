...
bool EQ::versions::IsValidClientVersion(ClientVersion client_version) {
	if (client_version <= ClientVersion::Unknown ||
	    client_version > LastClientVersion)
		return false;

	return true;
}

EQ::versions::ClientVersion EQ::versions::ValidateClientVersion(
...
uint32 EQ::versions::ConvertClientVersionToClientVersionBit(
    ClientVersion client_version) {
	switch (client_version) {
		case ClientVersion::Unknown:
			return bit_Unknown;
...
bool EQ::versions::IsValidOfflinePCMobVersion(MobVersion Mob_version) {
	if (Mob_version <= LastNonPCMobVersion ||
	    Mob_version > LastOfflinePCMobVersion)
		return false;

	return true;
}

EQ::versions::MobVersion EQ::versions::ValidateMobVersion(
...
EQ::versions::MobVersion EQ::versions::ValidatePCMobVersion(
    MobVersion Mob_version) {
	if (Mob_version <= MobVersion::Unknown || Mob_version > LastPCMobVersion)
		return MobVersion::Unknown;
...
EQ::versions::MobVersion EQ::versions::ValidateNonPCMobVersion(
    MobVersion Mob_version) {
	if (Mob_version <= LastPCMobVersion || Mob_version > LastNonPCMobVersion)
		return MobVersion::Unknown;
...
EQ::versions::MobVersion EQ::versions::ValidateOfflinePCMobVersion(
    MobVersion Mob_version) {
	if (Mob_version <= LastNonPCMobVersion ||
	    Mob_version > LastOfflinePCMobVersion)
		return MobVersion::Unknown;
