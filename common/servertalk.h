...
class ServerPacket {
   public:
	...
	ServerPacket* Copy() {
		...
		if (this->size) memcpy(ret->pBuffer, this->pBuffer, this->size);
	...
	bool Deflate() {
		...
		if ((!this->pBuffer) || (!this->size)) return false;
		uchar* tmp = new uchar[this->size + 128];
		uint32 tmpsize =
		    DeflatePacket(this->pBuffer, this->size, tmp, this->size + 128);
		if (!tmpsize) {
	...
	bool Inflate() {
		...
		uint32 tmpsize =
		    InflatePacket(this->pBuffer, this->size, tmp, InflatedSize);
		if (!tmpsize) {
			safe_delete_array(tmp);
			return false;
		}
...
struct ServerLSClientAuth {
	...
	char loginserver_name[64];  // username in login server's db
...
struct UsertoWorldResponse_Struct {
	uint32 lsaccountid;
	uint32 worldid;
	int8 response;  // -3) World Full, -2) Banned, -1) Suspended, 0) Denied, 1)
	                // Allowed
	uint32 FromID;
	uint32 ToID;
};
