...
class EQProtocolPacket : public BasePacket {
   ...
   protected:
	static bool ValidateCRC(const unsigned char *buffer, int length,
	                        uint32 Key);
	static uint32 Decompress(const unsigned char *buffer, const uint32 length,
	                         unsigned char *newbuf, uint32 newbufsize);
	static uint32 Compress(const unsigned char *buffer, const uint32 length,
	                       unsigned char *newbuf, uint32 newbufsize);
...
class EQOldPacket {
   ...
   public:
	...
	uint16 resend_count;     // Quagmire: Moving resend count to a packet by packet
	                         // basis
	uint16
	    dwLoopedOnce;  // Checks counter of times packet has looped. Basically a
	                   // bool but kept multiples for debugging purposes
	uint32 LastSent;   // Last time this packet was sent.
	bool Resend;

	// Quagmire: Made the CRC stuff static and public. Makes things easier
	// elsewhere.
...
class EQApplicationPacket : public EQPacket {
   ...
   public:
	...
	EQApplicationPacket(const EmuOpcode op)
	    : EQPacket(op, nullptr, 0), opcode_bypass(0) {
		app_opcode_size = GetExecutablePlatform() == ExePlatformUCS ? 1 : 2;
	}
	EQApplicationPacket(const EmuOpcode op, const uint32 len)
	    : EQPacket(op, nullptr, len), opcode_bypass(0) {
		app_opcode_size = GetExecutablePlatform() == ExePlatformUCS ? 1 : 2;
	}
	EQApplicationPacket(const EmuOpcode op, const unsigned char *buf,
	                    const uint32 len)
	    : EQPacket(op, buf, len), opcode_bypass(0) {
		app_opcode_size = GetExecutablePlatform() == ExePlatformUCS ? 1 : 2;
	}
...
class EQRawApplicationPacket : public EQApplicationPacket {
   ...
   public:
	EQRawApplicationPacket(uint16 opcode, const unsigned char *buf,
	                       const uint32 len);
	uint16 GetRawOpcode() const { return (opcode); }
