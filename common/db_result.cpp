...
DBResult::DBResult() : m_CurrentRow(), m_OneBeyondRow() {
	ZeroOut();
}

DBResult::DBResult(MYSQL_RES* result, uint32 rowsAffected,
                   uint32 rowCount, uint32 columnCount,
                   uint32 lastInsertedID,
                   uint32 errorNumber, char* errorBuffer)
	...
	m_Success = true;
	if (errorBuffer != nullptr) m_Success = false;

	m_ErrorNumber = errorNumber;
	m_ErrorBuffer = errorBuffer;
}

void DBResult::FreeInternals() {
	safe_delete_array(m_ErrorBuffer);

	if (m_Result != nullptr) mysql_free_result(m_Result);

	ZeroOut();
}

void DBResult::ZeroOut() {
	m_Success = false;
	m_Result = nullptr;
...
DBResult::~DBResult() { FreeInternals(); }

uint32 DBResult::LengthOfColumn(int columnIndex) {
	...
	// then we always return 0. Also applies if db screws
	// up and can't get the column lengths for whatever reason.
	if (m_ColumnLengths == nullptr) return 0;
	...
	if (columnIndex >= m_ColumnCount) return 0;

	return m_ColumnLengths[columnIndex];
}

const std::string DBResult::FieldName(int columnIndex) {
	if (columnIndex >= m_ColumnCount || m_Result == nullptr)
		return std::string();

	if (m_Fields == nullptr) m_Fields = mysql_fetch_fields(m_Result);

	return std::string(m_Fields[columnIndex].name);
}

DBResult::DBResult(DBResult&& moveItem)
...
DBResult& DBResult::operator=(DBResult&& other) {
