...
active if not specified) reset -> reset all rule values to their defaults. list
[catname] set (cat) (rule) (value) values [catname] -> show the values of all
rules in the specified category/
    */

const char *RuleManager::s_categoryNames[_CatCount + 1] = {
#define RULE_CATEGORY(catname) #catname,
#include "ruletypes.h"
    "InvalidCategory"};

const RuleManager::RuleInfo RuleManager::s_RuleInfo[_IntRuleCount +
                                                    _RealRuleCount +
                                                    _BoolRuleCount + 1] = {
...
bool RuleManager::ListRules(const char *catname,
                            std::vector<const char *> &into) {
	CategoryType cat = InvalidCategory;
	if (catname != nullptr) {
		cat = FindCategory(catname);
		if (cat == InvalidCategory) {
			Log(Logs::Detail, Logs::Rules, "Unable to find category '%s'",
			    catname);
			return (false);
		}
	}
...
bool RuleManager::GetRule(const char *rule_name, std::string &ret_val) {
	RuleType type;
	uint16 index;
	if (!_FindRule(rule_name, type, index)) return false;
...
bool RuleManager::SetRule(const char *rule_name, const char *rule_value,
                          Database *db, bool db_save) {
	if (rule_name == nullptr || rule_value == nullptr) return (false);
	...
	switch (type) {
		case IntRule:
			...
			Log(Logs::Detail, Logs::Rules, "Set rule %s to value %d", rule_name,
			    m_RuleIntValues[index]);
			break;
		case RealRule:
			m_RuleRealValues[index] = atof(rule_value);
			Log(Logs::Detail, Logs::Rules, "Set rule %s to value %.13f",
			    rule_name, m_RuleRealValues[index]);
			break;
		case BoolRule:
			uint32 val = 0;
			if (!strcasecmp(rule_value, "on") ||
			    !strcasecmp(rule_value, "true") ||
			    !strcasecmp(rule_value, "yes") ||
			    !strcasecmp(rule_value, "enabled") || !strcmp(rule_value, "1"))
				val = 1;
			m_RuleBoolValues[index] = val;
			Log(Logs::Detail, Logs::Rules, "Set rule %s to value %s", rule_name,
			    m_RuleBoolValues[index] == 1 ? "true" : "false");
			break;
	}

	if (db_save) _SaveRule(db, type, index);
...
bool RuleManager::_FindRule(const char *rule_name, RuleType &type_into,
                            uint16 &index_into) {
	if (rule_name == nullptr) return (false);
	...
	if (ruleset != nullptr) {
		...
		if (m_activeName != ruleset) {
			...
			if (m_activeRuleset == -1) {
				Log(Logs::Detail, Logs::Rules,
				    "Unable to find or create rule set %s", ruleset);
				return;
			}
			m_activeName = ruleset;
		}
		Log(Logs::Detail, Logs::Rules,
		    "Saving running rules into rule set %s (%d)", ruleset,
		    m_activeRuleset);
	} else {
		Log(Logs::Detail, Logs::Rules,
		    "Saving running rules into running rule set %s",
		    m_activeName.c_str(), m_activeRuleset);
	}
...
bool RuleManager::LoadRules(Database *db, const char *ruleset) {
	...
	if (rsid < 0) {
		Log(Logs::Detail, Logs::Rules,
		    "Failed to find ruleset '%s' for load operation. Canceling.",
		    ruleset);
		return (false);
	}
	...
	/* Load default ruleset values first if we're loading something other than
	 * default */
	if (strcasecmp(ruleset, "default") != 0) {
		...
		if (default_ruleset_id < 0) {
			Log(Logs::Detail, Logs::Rules,
			    "Failed to find default ruleset '%s' for load operation. "
			    "Canceling.",
			    default_ruleset_name.c_str());
			return (false);
		}
		Log(Logs::Detail, Logs::Rules, "Loading rule set '%s' (%d)",
		    default_ruleset_name.c_str(), default_ruleset_id);

		std::string query = StringFormat(
		    "SELECT rule_name, rule_value FROM rule_values WHERE ruleset_id = "
		    "%d",
		    default_ruleset_id);
		...
		for (auto row = results.begin(); row != results.end(); ++row)
			if (!SetRule(row[0], row[1], nullptr, false))
				Log(Logs::Detail, Logs::Rules,
				    "Unable to interpret rule record for %s", row[0]);
	}
...
int RuleManager::GetRulesetID(Database *db, const char *rulesetname) {
	...
	std::string query =
	    StringFormat("SELECT ruleset_id FROM rule_sets WHERE name='%s'", rst);
...
int RuleManager::_FindOrCreateRuleset(Database *db, const char *ruleset) {
	...
	if (res >= 0) return res;  // found and existing one...
	...
	std::string query = StringFormat(
	    "INSERT INTO rule_sets (ruleset_id, name) VALUES(0, '%s')", rst);
	safe_delete_array(rst);
	auto results = db->QueryDatabase(query);
	if (!results.Success()) {
		return -1;
...
std::string RuleManager::GetRulesetName(Database *db, int id) {
	std::string query =
	    StringFormat("SELECT name FROM rule_sets WHERE ruleset_id=%d", id);
	auto results = db->QueryDatabase(query);
	if (!results.Success()) {
		return "";
	}
