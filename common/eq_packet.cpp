...
EQPacket::EQPacket(EmuOpcode op, const unsigned char *buf, uint32 len)
    : BasePacket(buf, len), emu_opcode(op) {}
...
void EQApplicationPacket::build_raw_header_dump(char *buffer,
                                                uint16 seq) const {
	BasePacket::build_raw_header_dump(buffer, seq);
	buffer += strlen(buffer);
...
#else
	sprintf(buffer, "[OpCode %s Size=%u]", OpcodeManager::EmuToName(emu_opcode),
	        size);
#endif
}
...
void EQRawApplicationPacket::build_raw_header_dump(char *buffer,
                                                   uint16 seq) const {
	BasePacket::build_raw_header_dump(buffer, seq);
	buffer += strlen(buffer);
...
void EQRawApplicationPacket::build_header_dump(char *buffer) const {
	...
	sprintf(buffer, "[OpCode 0x%04x (0x%04x) Size=%u]\n", emu_opcode, opcode,
	        size);
#else
	sprintf(buffer, "[OpCode %s (0x%04x) Size=%u]",
	        OpcodeManager::EmuToName(emu_opcode), opcode, size);
#endif
}
...
uint32 EQApplicationPacket::serialize(uint16 opcode,
	...
	if (app_opcode_size == 1)
		*(unsigned char *)dest = opcode;
		...
		// Application opcodes with a low order byte of 0x00 require an extra
		// 0x00 byte inserting prior to the opcode.
		if ((opcode & 0x00ff) == 0) {
			*(uint8 *)dest = 0;
...
/*EQProtocolPacket::EQProtocolPacket(uint16 op, const unsigned char *buf, uint32
len) :	BasePacket(buf, len), opcode(op)
{
...
bool EQProtocolPacket::ValidateCRC(const unsigned char *buffer, int length,
                                   uint32 Key) {
	bool valid = false;
	// OP_SessionRequest, OP_SessionResponse, OP_OutOfSession are not CRC'd
	if (buffer[0] == 0x00 &&
	    (buffer[1] == OP_SessionRequest || buffer[1] == OP_SessionResponse ||
		...
		if (packet_crc && comp_crc != packet_crc) {
			std::cout << "CRC mismatch: comp=" << std::hex << comp_crc
			          << ", packet=" << packet_crc << std::dec << std::endl;
		}
#endif
...
uint32 EQProtocolPacket::Decompress(const unsigned char *buffer,
                                    const uint32 length, unsigned char *newbuf,
                                    uint32 newbufsize) {
	uint32 newlen = 0;
	...
	if (length > 2 && buffer[flag_offset] == 0x5a) {
		newlen = InflatePacket(buffer + flag_offset + 1,
		                       length - (flag_offset + 1) - 2,
		                       newbuf + flag_offset, newbufsize - flag_offset) +
		         2;
		newbuf[newlen++] = buffer[length - 2];
		newbuf[newlen++] = buffer[length - 1];
	} else if (length > 2 && buffer[flag_offset] == 0xa5) {
		memcpy(newbuf + flag_offset, buffer + flag_offset + 1,
		       length - (flag_offset + 1));
		newlen = length - 1;
	} else {
...
uint32 EQProtocolPacket::Compress(const unsigned char *buffer,
                                  const uint32 length, unsigned char *newbuf,
                                  uint32 newbufsize) {
	...
	if (length > 30) {
		newlength = DeflatePacket(buffer + flag_offset, length - flag_offset,
		                          newbuf + flag_offset + 1, newbufsize);
		*(newbuf + flag_offset) = 0x5a;
		newlength += flag_offset + 1;
	} else {
		memmove(newbuf + flag_offset + 1, buffer + flag_offset,
		        length - flag_offset);
...
void EQProtocolPacket::ChatDecode(unsigned char *buffer, int size,
                                  int DecodeKey) {
	if ((size >= 2) && buffer[1] != 0x01 && buffer[0] != 0x02 &&
	    buffer[0] != 0x1d) {
		int Key = DecodeKey;
...
void EQProtocolPacket::ChatEncode(unsigned char *buffer, int size,
                                  int EncodeKey) {
...
EQRawApplicationPacket::EQRawApplicationPacket(uint16 opcode,
                                               const unsigned char *buf,
                                               const uint32 len)
    : EQApplicationPacket(OP_Unknown, buf, len), opcode(opcode) {}
EQRawApplicationPacket::EQRawApplicationPacket(const unsigned char *buf,
                                               const uint32 len)
    : EQApplicationPacket(OP_Unknown, buf + sizeof(uint16),
                          len - sizeof(uint16)) {
...
void DumpPacket(const EQApplicationPacket *app, bool iShowInfo) {
	...
	if (iShowInfo) {
		std::cout << "Dumping Applayer: 0x" << std::hex << std::setfill('0')
		          << std::setw(4) << app->GetOpcode() << std::dec;
...
// deletes pExtra
EQOldPacket::~EQOldPacket() {
	if (pExtra) delete[] pExtra;  // delete pExtra;
	if (ack_fields) delete[] ack_fields;
	Clear();
}
...
void EQOldPacket::DecodePacket(uint16 length, uchar *pPacket) {
	...
	if (length < 10)  // Adding checks for illegal packets, so we dont read
	                  // beyond the buffer
	{                 //      Minimum normal packet length is 10
		              //	cerr << "EQPacket.cpp: Illegal packet length" << endl;
		return;       // TODO: Do we want to check crc checksum on the incoming
		              // packets?
	}
	...
	//  HDR.b4_Unknown - b7 is sent when there are packets missed, and bitfield
	//  following b4-b7 list what is acked and not HDR.b3_Unknown is sent when
	//  we want a resend of all non-acked packets prior to value in b3ARSP
	bool bDumpPacket = false;
	if (HDR.b3_Unknown) {
		b3ARSP = ntohs(*intptr++);
		size += 2;
	}
	b4_size = (HDR.b7_Unknown << 3) | (HDR.b6_Unknown << 2) |
	          (HDR.b5_Unknown << 1) | HDR.b4_Unknown;
...
uint32 EQOldPacket::ReturnPacket(uchar **data, EQOldStream *netcon) {
	...
	*((uint16 *)&buf[2]) = ntohs(netcon->SACK.dwGSQ++);
	...
	//{
	//	netcon->SACK.dwGSQ = 120; //Magic number. Seems to cause the client to
	// recover. I don't think this is right now /shrug 	this->dwSEQ = 120;
	// }
...
EQRawApplicationPacket *EQOldPacket::MakeAppPacket() const {
	...
	EQRawApplicationPacket *res =
	    new EQRawApplicationPacket(dwOpCode, pExtra, dwExtraSize);
	return (res);
}
