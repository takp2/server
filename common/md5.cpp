...
MD5::MD5() { memset(pMD5, 0, 16); }

MD5::MD5(const uchar* buf, uint32 len) { Generate(buf, len, pMD5); }

MD5::MD5(const char* buf, uint32 len) {
	Generate((const uchar*)buf, len, pMD5);
}

MD5::MD5(const uint8 buf[16]) { Set(buf); }

MD5::MD5(const char* iMD5String) { Set(iMD5String); }

void MD5::Generate(const char* iString) {
	Generate((const uchar*)iString, strlen(iString));
}

void MD5::Generate(const uint8* buf, uint32 len) { Generate(buf, len, pMD5); }
...
bool MD5::Set(const char* iMD5String) {
	...
	for (int i = 0; i < 16; i++) {
		...
		if (!Seperator::IsHexNumber(tmp)) return false;
		pMD5[i] = hextoi(tmp);
	}
	return true;
}
...
MD5::operator const char*() {
	snprintf(pMD5String, sizeof(pMD5String),
	         "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
	         pMD5[0], pMD5[1], pMD5[2], pMD5[3], pMD5[4], pMD5[5], pMD5[6],
	         pMD5[7], pMD5[8], pMD5[9], pMD5[10], pMD5[11], pMD5[12], pMD5[13],
	         pMD5[14], pMD5[15]);
	return pMD5String;
}
...
bool MD5::operator==(const char* iMD5String) {
	...
	for (int i = 0; i < 16; i++) {
		tmp[2] = iMD5String[i * 2];
		tmp[3] = iMD5String[(i * 2) + 1];
		if (pMD5[i] != hextoi(tmp)) return false;
	}
	return true;
}
