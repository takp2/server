...
const char *const BaseGuildManager::GuildActionNames[_MaxGuildAction] = {
    "HearGuildChat", "SpeakGuildChat", "Invite", "Remove",
    "Promote", "Demote", "Set_MOTD", "War/Peace"};

BaseGuildManager::BaseGuildManager() : m_db(nullptr) {}
...
bool BaseGuildManager::LoadGuilds() {
	...
	for (auto row = results.begin(); row != results.end(); ++row)
		_CreateGuild(atoi(row[0]), row[1], atoi(row[2]), atoi(row[3]), row[4],
		             row[5], row[6], row[7]);
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (rankn > GUILD_MAX_RANK) {
			Log(Logs::Detail, Logs::Guilds,
			    "Found invalid (too high) rank %d for guild %d, skipping.",
			    rankn, guild_id);
			continue;
		}
		...
		if (res == m_guilds.end()) {
			Log(Logs::Detail, Logs::Guilds,
			    "Found rank %d for non-existent guild %d, skipping.", rankn,
			    guild_id);
			continue;
		}
...
bool BaseGuildManager::RefreshGuild(uint32 guild_id) {
	if (m_db == nullptr) {
		Log(Logs::Detail, Logs::Guilds,
		    "Requested to refresh guild %d when we have no database object.",
		    guild_id);
		return (false);
	}

	std::string query = StringFormat(
	    "SELECT name, leader, minstatus, motd, motd_setter, channel,url FROM "
	    "guilds WHERE id=%lu",
	...
	if (results.RowCount() == 0) {
		Log(Logs::Detail, Logs::Guilds,
		    "Unable to find guild %d in the database.", guild_id);
		return false;
	}

	auto row = results.begin();

	info = _CreateGuild(guild_id, row[0], atoi(row[1]), atoi(row[2]), row[3],
	                    row[4], row[5], row[6]);

	query = StringFormat(
	    "SELECT guild_id, `rank`, title, can_hear, can_speak, can_invite, "
	...
	Log(Logs::Detail, Logs::Guilds,
	    "Successfully refreshed guild %d from the database.", guild_id);

	return true;
}

BaseGuildManager::GuildInfo *BaseGuildManager::_CreateGuild(
    uint32 guild_id, const char *guild_name, uint32 leader_char_id,
    uint8 minstatus, const char *guild_motd, const char *motd_setter,
    const char *Channel, const char *URL) {
	std::map<uint32, GuildInfo *>::iterator res;
...
bool BaseGuildManager::_StoreGuildDB(uint32 guild_id) {
	if (m_db == nullptr) {
		Log(Logs::Detail, Logs::Guilds,
		    "Requested to store guild %d when we have no database object.",
		    guild_id);
		return (false);
	}
	...
	if (res == m_guilds.end()) {
		Log(Logs::Detail, Logs::Guilds,
		    "Requested to store non-existent guild %d", guild_id);
		return (false);
	}
	GuildInfo *info = res->second;

	std::string query = StringFormat("DELETE FROM guilds WHERE id=%lu",
	                                 (unsigned long)guild_id);
	...
	m_db->DoEscapeString(motd_set_esc, info->motd_setter.c_str(),
	                     info->motd_setter.length());

	// insert the new `guilds` entry
	query = StringFormat(
	    "INSERT INTO guilds (id,name,leader,minstatus,motd,motd_setter) "
	...
	for (rank = 0; rank <= GUILD_MAX_RANK; rank++) {
		...
		m_db->DoEscapeString(title_esc, rankInfo.name.c_str(),
		                     rankInfo.name.length());

		query = StringFormat(
		    "INSERT INTO guild_ranks "
		    "(guild_id,`rank`,title,can_hear,can_speak,can_invite,can_remove,"
		    "can_promote,can_demote,can_motd,can_warpeace)"
		    " VALUES(%d,%d,'%s',%d,%d,%d,%d,%d,%d,%d,%d)",
		    guild_id, rank, title_esc, rankInfo.permissions[GUILD_HEAR],
	...
	Log(Logs::Detail, Logs::Guilds, "Stored guild %d in the database",
	    guild_id);
...
uint32 BaseGuildManager::_GetFreeGuildID() {
	if (m_db == nullptr) {
		Log(Logs::Detail, Logs::Guilds,
		    "Requested find a free guild ID when we have no database object.");
		return (GUILD_NONE);
	}
	...
	//  LIMIT 1
	//
	//  Seems likely what we should be doing is auto incrementing the guild
	//  table inserting, then getting the id. NOT getting a free id then
	//  inserting. could be a race condition.
	...
	for (auto index = 1; index < RuleI(Guild, MaxGuilds); ++index) {
		...
		if (results.RowCount() == 0) {
			Log(Logs::Detail, Logs::Guilds,
			    "Located free guild ID %d in the database", index);
			return index;
		}
	}

	Log(Logs::Detail, Logs::Guilds,
	    "Unable to find a free guild ID when requested.");
	return GUILD_NONE;
}

uint32 BaseGuildManager::CreateGuild(const char *name, uint32 leader_char_id) {
	uint32 gid = DBCreateGuild(name, leader_char_id);
	if (gid == GUILD_NONE) return (GUILD_NONE);
...
bool BaseGuildManager::DeleteGuild(uint32 guild_id) {
	if (!DBDeleteGuild(guild_id)) return (false);
...
bool BaseGuildManager::RenameGuild(uint32 guild_id, const char *name) {
	if (!DBRenameGuild(guild_id, name)) return (false);
...
bool BaseGuildManager::SetGuildLeader(uint32 guild_id, uint32 leader_char_id) {
	...
	std::map<uint32, GuildInfo *>::const_iterator res;
	res = m_guilds.find(guild_id);
	if (res == m_guilds.end()) return (false);
	GuildInfo *info = res->second;
	uint32 old_leader = info->leader_char_id;

	if (!DBSetGuildLeader(guild_id, leader_char_id)) return (false);
...
bool BaseGuildManager::SetGuildMOTD(uint32 guild_id, const char *motd,
                                    const char *setter) {
	if (!DBSetGuildMOTD(guild_id, motd, setter)) return (false);
...
bool BaseGuildManager::SetGuild(uint32 charid, uint32 guild_id, uint8 rank) {
	if (rank > GUILD_MAX_RANK && guild_id != GUILD_NONE) return (false);
	...
	if (!DBSetGuild(charid, guild_id, rank)) return (false);
...
bool BaseGuildManager::SetGuildRank(uint32 charid, uint8 rank) {
	if (rank > GUILD_MAX_RANK) return (false);

	if (!DBSetGuildRank(charid, rank)) return (false);
...
bool BaseGuildManager::SetBankerFlag(uint32 charid, bool is_banker) {
	if (!DBSetBankerFlag(charid, is_banker)) return (false);
...
bool BaseGuildManager::SetAltFlag(uint32 charid, bool is_alt) {
	if (!DBSetAltFlag(charid, is_alt)) return (false);
...
bool BaseGuildManager::SetPublicNote(uint32 charid, const char *note) {
	if (!DBSetPublicNote(charid, note)) return (false);
...
uint32 BaseGuildManager::DBCreateGuild(const char *name, uint32 leader) {
	...
	if (new_id == GUILD_NONE) return (GUILD_NONE);
	...
	if (!_StoreGuildDB(new_id)) {
		Log(Logs::Detail, Logs::Guilds,
		    "Error storing new guild. It may have been partially created which "
		    "may need manual removal.");
		return (GUILD_NONE);
	}

	Log(Logs::Detail, Logs::Guilds, "Created guild %d in the database.",
	    new_id);
...
bool BaseGuildManager::DBDeleteGuild(uint32 guild_id) {
	...
	if (m_db == nullptr) {
		Log(Logs::Detail, Logs::Guilds,
		    "Requested to delete guild %d when we have no database object.",
		    guild_id);
		return (false);
	}
	...
	query = StringFormat("DELETE FROM guild_members WHERE guild_id=%lu",
	                     (unsigned long)guild_id);
	QueryWithLogging(query, "clearing chars in guild");

	Log(Logs::Detail, Logs::Guilds, "Deleted guild %d from the database.",
	    guild_id);
...
bool BaseGuildManager::DBRenameGuild(uint32 guild_id, const char *name) {
	if (m_db == nullptr) {
		Log(Logs::Detail, Logs::Guilds,
		    "Requested to rename guild %d when we have no database object.",
		    guild_id);
		return false;
	}
	...
	if (!results.Success()) {
		Log(Logs::Detail, Logs::Guilds, "Error renaming guild %d '%s': %s",
		    guild_id, query.c_str(), results.Success());
		safe_delete_array(esc);
		return false;
	}
	safe_delete_array(esc);

	Log(Logs::Detail, Logs::Guilds, "Renamed guild %s (%d) to %s in database.",
	    info->name.c_str(), guild_id, name);
...
bool BaseGuildManager::DBSetGuildLeader(uint32 guild_id, uint32 leader) {
	if (m_db == nullptr) {
		Log(Logs::Detail, Logs::Guilds,
		    "Requested to set the leader for guild %d when we have no database "
		    "object.",
		    guild_id);
		return false;
	}
	...
	if (!DBSetGuildRank(info->leader_char_id, GUILD_OFFICER)) return false;
	// set the new leader to leader
	if (!DBSetGuildRank(leader, GUILD_LEADER)) return false;

	Log(Logs::Detail, Logs::Guilds,
	    "Set guild leader for guild %d to %d in the database", guild_id,
	    leader);
...
bool BaseGuildManager::DBSetGuildMOTD(uint32 guild_id, const char *motd,
                                      const char *setter) {
	if (m_db == nullptr) {
		Log(Logs::Detail, Logs::Guilds,
		    "Requested to set the MOTD for guild %d when we have no database "
		    "object.",
		    guild_id);
		return (false);
	}
	...
	Log(Logs::Detail, Logs::Guilds, "Set MOTD for guild %d in the database",
	    guild_id);
...
bool BaseGuildManager::DBSetGuildURL(uint32 GuildID, const char *URL) {
	...
	Log(Logs::Detail, Logs::Guilds, "Set URL for guild %d in the database",
	    GuildID);
...
bool BaseGuildManager::DBSetGuildChannel(uint32 GuildID, const char *Channel) {
	...
	Log(Logs::Detail, Logs::Guilds, "Set Channel for guild %d in the database",
	    GuildID);
...
bool BaseGuildManager::DBSetGuild(uint32 charid, uint32 guild_id, uint8 rank) {
	if (m_db == nullptr) {
		Log(Logs::Detail, Logs::Guilds,
		    "Requested to set char to guild %d when we have no database "
		    "object.",
		    guild_id);
		return (false);
	}
	...
	Log(Logs::Detail, Logs::Guilds,
	    "Set char %d to guild %d and rank %d in the database.", charid,
	    guild_id, rank);
	return true;
}
...
bool BaseGuildManager::DBSetGuildRank(uint32 charid, uint8 rank) {
	std::string query = StringFormat(
	    "UPDATE guild_members SET `rank`=%d WHERE char_id=%d", rank, charid);
...
bool BaseGuildManager::DBSetBankerFlag(uint32 charid, bool is_banker) {
	std::string query =
	    StringFormat("UPDATE guild_members SET banker=%d WHERE char_id=%d",
...
bool BaseGuildManager::DBSetAltFlag(uint32 charid, bool is_alt) {
	std::string query =
	    StringFormat("UPDATE guild_members SET alt=%d WHERE char_id=%d",
	                 is_alt ? 1 : 0, charid);
...
static void ProcessGuildMember(MySQLRequestRow row, CharGuildInfo &into) {
	...
	if (into.guild_id == 0) into.guild_id = GUILD_NONE;
	if (into.rank > GUILD_MAX_RANK) into.rank = GUILD_RANK_NONE;
}
...
bool BaseGuildManager::GetEntireGuild(uint32 guild_id,
	...
	Log(Logs::Detail, Logs::Guilds,
	    "Retreived entire guild member list for guild %d from the database",
	    guild_id);
...
bool BaseGuildManager::GetCharInfo(const char *char_name, CharGuildInfo &into) {
	if (m_db == nullptr) {
		Log(Logs::Detail, Logs::Guilds,
		    "Requested char info on %s when we have no database object.",
		    char_name);
		return (false);
	}
	...
	Log(Logs::Detail, Logs::Guilds,
	    "Retreived guild member info for char %s from the database", char_name);
...
bool BaseGuildManager::GetCharInfo(uint32 char_id, CharGuildInfo &into) {
	if (m_db == nullptr) {
		Log(Logs::Detail, Logs::Guilds,
		    "Requested char info on %d when we have no database object.",
		    char_id);
		return false;
	}
	...
	auto row = results.begin();
	ProcessGuildMember(row, into);
	Log(Logs::Detail, Logs::Guilds, "Retreived guild member info for char %d",
	    char_id);
...
unsigned char *BaseGuildManager::MakeOldGuildList(uint32 &length) const {
	...
	for (int16 r = 0; r <= RuleI(Guild, MaxGuilds); r++) {
		...
		if (GetGuildNameByID(r, tmp)) {
			...
			Log(Logs::Detail, Logs::Guilds, "Added Guild: %i (%s)",
			    gl->Guilds[r].guildID, gl->Guilds[r].name);
		} else {
			gl->Guilds[r].guildID = r;
...
const char *BaseGuildManager::GetRankName(uint32 guild_id, uint8 rank) const {
	if (rank > GUILD_MAX_RANK) return ("Invalid Rank");
	std::map<uint32, GuildInfo *>::const_iterator res;
	res = m_guilds.find(guild_id);
	if (res == m_guilds.end()) return ("Invalid Guild Rank");
	return (res->second->ranks[rank].name.c_str());
}

const char *BaseGuildManager::GetGuildName(uint32 guild_id) const {
	if (guild_id == GUILD_NONE) return ("");
	std::map<uint32, GuildInfo *>::const_iterator res;
	res = m_guilds.find(guild_id);
	if (res == m_guilds.end()) return ("Invalid Guild");
	return (res->second->name.c_str());
}

bool BaseGuildManager::GetGuildNameByID(uint32 guild_id,
...
bool BaseGuildManager::GetGuildMOTD(uint32 guild_id, char *motd_buffer,
                                    char *setter_buffer) const {
	std::map<uint32, GuildInfo *>::const_iterator res;
	res = m_guilds.find(guild_id);
	if (res == m_guilds.end()) return (false);
...
bool BaseGuildManager::GetGuildChannel(uint32 GuildID,
                                       char *ChannelBuffer) const {
	std::map<uint32, GuildInfo *>::const_iterator res;
	res = m_guilds.find(GuildID);
	if (res == m_guilds.end()) return (false);
...
bool BaseGuildManager::GuildExists(uint32 guild_id) const {
	if (guild_id == GUILD_NONE) return (false);
...
bool BaseGuildManager::IsGuildLeader(uint32 guild_id, uint32 char_id) const {
	if (guild_id == GUILD_NONE) {
		Log(Logs::Detail, Logs::Guilds,
		    "Check leader for char %d: not a guild.", char_id);
		return (false);
	}
	std::map<uint32, GuildInfo *>::const_iterator res;
	res = m_guilds.find(guild_id);
	if (res == m_guilds.end()) {
		Log(Logs::Detail, Logs::Guilds,
		    "Check leader for char %d: invalid guild.", char_id);
		return (false);  // invalid guild
	}
	Log(Logs::Detail, Logs::Guilds,
	    "Check leader for guild %d, char %d: leader id=%d", guild_id, char_id,
	    res->second->leader_char_id);
...
uint32 BaseGuildManager::FindGuildByLeader(uint32 leader) const {
	...
	for (; cur != end; ++cur) {
		if (cur->second->leader_char_id == leader) return (cur->first);
	}
	return (GUILD_NONE);
}

// returns the rank to be sent to the client for display purposes, given their
// eqemu rank.
uint8 BaseGuildManager::GetDisplayedRank(uint32 guild_id, uint8 rank,
                                         uint32 char_id) const {
	std::map<uint32, GuildInfo *>::const_iterator res;
	res = m_guilds.find(guild_id);
	if (res == m_guilds.end()) return (3);  // invalid guild rank
	if (res->second->ranks[rank].permissions[GUILD_WARPEACE] ||
	    res->second->leader_char_id == char_id)
		return (2);  // leader rank
	else if (res->second->ranks[rank].permissions[GUILD_INVITE] ||
	         res->second->ranks[rank].permissions[GUILD_REMOVE] ||
	         res->second->ranks[rank].permissions[GUILD_MOTD])
...
bool BaseGuildManager::CheckGMStatus(uint32 guild_id, uint8 status) const {
	if (status >= 250) {
		Log(Logs::Detail, Logs::Guilds,
		    "Check permission on guild %d with user status %d > 250, granted.",
		    guild_id, status);
	...
	if (res == m_guilds.end()) {
		Log(Logs::Detail, Logs::Guilds,
		    "Check permission on guild %d with user status %d, no such guild, "
		    "denied.",
		    guild_id, status);
		return (false);  // invalid guild
	}
	...
	Log(Logs::Detail, Logs::Guilds,
	    "Check permission on guild %s (%d) with user status %d. Min status %d: "
	    "%s",
	    res->second->name.c_str(), guild_id, status, res->second->minstatus,
	    granted ? "granted" : "denied");

	return (granted);
}

bool BaseGuildManager::CheckPermission(uint32 guild_id, uint8 rank,
                                       GuildAction act) const {
	if (rank > GUILD_MAX_RANK) {
		Log(Logs::Detail, Logs::Guilds,
		    "Check permission on guild %d and rank %d for action %s (%d): "
		    "Invalid rank, denied.",
	...
	Log(Logs::Detail, Logs::Guilds,
	    "Check permission on guild %s (%d) and rank %s (%d) for action %s "
	    "(%d): %s",
	    res->second->name.c_str(), guild_id,
	    res->second->ranks[rank].name.c_str(), rank, GuildActionNames[act], act,
	    granted ? "granted" : "denied");
...
bool BaseGuildManager::LocalDeleteGuild(uint32 guild_id) {
	std::map<uint32, GuildInfo *>::iterator res;
	res = m_guilds.find(guild_id);
	if (res == m_guilds.end()) return (false);  // invalid guild
	m_guilds.erase(res);
	return (true);
}
...
BaseGuildManager::RankInfo::RankInfo() {
	uint8 r;
	for (r = 0; r < _MaxGuildAction; r++) permissions[r] = false;
}
