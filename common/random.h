...
/* This uses mt19937 (or other compatible engine) seeded with the
 * std::random_device. The idea is to have this be included as a member of
 * another class so mocking out for testing is easier If you need to reseed
 * random.Reseed() Eventually this should be derived from an abstract base class
 */
...
class Random {
   public:
	...
	bool Roll(const int required) { return Int(0, 99) < required; }

	// valid values 0.0 - 1.0
	bool Roll(const double required) { return Real(0.0, 1.0) <= required; }

	// same range as client's roll0
	// This is their main high level RNG function
	int Roll0(int max) {
		if (max - 1 > 0) return Int(0, max - 1);
		return 0;
	}

	// std::shuffle requires a RNG engine passed to it, so lets provide a
	// wrapper to use our engine
