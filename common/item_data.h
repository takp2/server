...
enum ItemTypes : uint8 {
	...
	/*----*/ ItemTypeSkull,
	/*5516*/ ItemTypeBook,  // skill-up tomes/books? (would probably need a pp
	                        // flag if true...)
	/*5517*/ ItemTypeNote,
	/*5518*/ ItemTypeKey,
	/*----*/ ItemTypeCoin,
...
enum BagTypes : uint8 {
	...
	/*----*/ BagTypeUnknown1,  // (a coin pouch/purse?) (no database entries as
	                           // of peq rev 69)
	...
	/*----*/ BagTypeTradersSatchel,  // <*Database Reference Only> (db: Yellow
	                                 // Trader's Satchel Token?)
	/*----*/ BagTypeCount
};
...
struct ItemData {
	...
	int16 Mana;       // Mana
	int16 AC;         // AC
	int8 MaxCharges;  // Maximum charges items can hold: -1 if not a chargeable
	                  // item
	int8 GMFlag;
	int8 Light;    // Light
	...
	bool Stackable;
	int8 Stackable_;  // EQMac uses 0 for non-stackable, 1 for stackable, and 3
	                  // indicates the item has a spell effect. 2 seems to be
	                  // unused.
	int16 StackSize;
	...
	uint8 BagType;   // 0:Small Bag, 1:Large Bag, 2:Quiver, 3:Belt Pouch ...
	                 // there are 50 types
	...
	int32 CastTime;  // Cast Time for clicky effects, in milliseconds. Also
	                 // determines how long food/drink lasts.
	int32 CastTime_;
	uint32 RecastType;
	uint32 RecastDelay;
	...
	uint8 RecSkill;  // Recommended skill to use item (refers to primary skill
	                 // of item)
	int32 ProcRate;
