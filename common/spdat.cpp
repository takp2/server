...
bool IsMezSpell(uint16 spell_id) { return IsEffectInSpell(spell_id, SE_Mez); }
...
bool IsStunSpell(uint16 spell_id) { return IsEffectInSpell(spell_id, SE_Stun); }
...
bool IsEvacSpell(uint16 spellid) { return IsEffectInSpell(spellid, SE_Succor); }
...
// Spells with 0 duration that apply instant damage.  This includes stuns.  Use
// IsPureNukeSpell() to exclude stuns
bool IsDirectDamageSpell(uint16 spellid) {
	if (spells[spellid].buffduration > 0) return false;
...
// Any spell with a direct/instant damage component/effect.  This includes many
// DoTs, many stuns, some debuffs with a damage component like Storm Comet
bool HasDirectDamageEffect(uint16 spellid) {
	for (int o = 0; o < EFFECT_COUNT; o++) {
		uint32 tid = spells[spellid].effectid[o];
		if (tid == SE_CurrentHPOnce && spells[spellid].base[o] < 0) return true;
...
// Spells with a duration > 0 and apply damage every tick.  These spells may
// also have a direct damage component
bool IsDOTSpell(uint16 spellid) {
	if (spells[spellid].buffduration < 1) return false;

	for (int o = 0; o < EFFECT_COUNT; o++) {
		uint32 tid = spells[spellid].effectid[o];
		if (tid == SE_CurrentHP && spells[spellid].base[o] < 0) return true;
	}

	return false;
}

bool IsFearSpell(uint16 spell_id) { return IsEffectInSpell(spell_id, SE_Fear); }
...
bool IsCureSpell(uint16 spell_id) {
	...
	for (int i = 0; i < EFFECT_COUNT; i++) {
		if (sp.effectid[i] == SE_DiseaseCounter ||
		    sp.effectid[i] == SE_PoisonCounter ||
		    sp.effectid[i] == SE_CurseCounter)
			CureEffect = true;
	}

	if (CureEffect && IsBeneficialSpell(spell_id)) return true;
...
bool IsSlowSpell(uint16 spell_id) {
	...
	for (int i = 0; i < EFFECT_COUNT; i++)
		if ((sp.effectid[i] == SE_AttackSpeed && sp.base[i] < 100)) return true;
...
bool IsHasteSpell(uint16 spell_id) {
	...
	for (int i = 0; i < EFFECT_COUNT; i++)
		if (sp.effectid[i] == SE_AttackSpeed) return (sp.base[i] < 100);
...
bool IsHarmonySpell(uint16 spell_id) {
	...
	return (IsEffectInSpell(spell_id, SE_Harmony) ||
...
bool IsLullSpell(uint16 spell_id) { return IsEffectInSpell(spell_id, SE_Lull); }
...
bool IsGroupOnlySpell(uint16 spell_id) {
	return IsValidSpell(spell_id) &&
	       (spells[spell_id].goodEffect == 2 || spell_id == SPELL_TORPOR ||
...
bool IsBeneficialSpell(uint16 spell_id) {
	...
	if (spells[spell_id].goodEffect == 1) {
		// If the target type is not ST_Self or ST_Pet or ST_GroupTeleport and
		// is a SE_CancelMagic spell it is not Beneficial
		SpellTargetType tt = spells[spell_id].targettype;
		if (tt != ST_Self && tt != ST_Pet && tt != ST_GroupTeleport &&
		    IsEffectInSpell(spell_id, SE_CancelMagic))
			return false;

		// When our targettype is ST_Target, ST_AETarget, ST_Aniaml, ST_Undead,
		...
		if (tt == ST_Target || tt == ST_AETarget || tt == ST_Animal ||
			...
			// If the resisttype is magic and SpellAffectIndex is Calm/dispell
			// sight it's not beneficial.
	...
	// And finally, if goodEffect is not 0 or if it's a group spell it's
	// beneficial
...
bool IsNeutralSpell(uint16 spell_id) {
	...
	return (IsBindSightSpell(spell_id) ||
	        (IsMemBlurSpell(spell_id) && !IsMezSpell(spell_id)) ||
	        IsLuclinPortSpell(spell_id) || IsHarmonySpell(spell_id) ||
	        spells[spell_id].effectid[0] ==
	            SE_Teleport2);  // some PoP NPC teleports are flagged beneficial
}
...
bool IsDireCharmSpell(uint16 spell_id) {
	if (IsEffectInSpell(spell_id, SE_Charm) &&
	    spells[spell_id].buffdurationformula == 50)
		return true;
	else
		return false;
}

bool IsRootSpell(uint16 spell_id) { return IsEffectInSpell(spell_id, SE_Root); }
...
bool IsHealingSpell(uint16 spellid) {
	if (IsEffectInSpell(spellid, SE_HealOverTime) ||
	    IsEffectInSpell(spellid, SE_PercentalHeal) ||
	    IsEffectInSpell(spellid, SE_CompleteHeal))
		return true;
...
bool IsAEDurationSpell(uint16 spell_id) {
	if (IsValidSpell(spell_id) &&
	    (spells[spell_id].targettype == ST_AETarget ||
	     spells[spell_id].targettype == ST_UndeadAE) &&
...
bool IsPureNukeSpell(uint16 spell_id) {
	...
	for (i = 0; i < EFFECT_COUNT; i++)
		if (!IsBlankSpellEffect(spell_id, i)) effect_count++;
...
bool IsPureDispelSpell(uint16 spell_id) {
	...
	for (i = 0; i < EFFECT_COUNT; i++)
		if (!IsBlankSpellEffect(spell_id, i) &&
		    spells[spell_id].effectid[i] != SE_CancelMagic)
			return false;
...
bool IsPartialCapableSpell(uint16 spell_id) {
	if (spells[spell_id].no_partial_resist) return false;

	// spell uses 600 (partial) scale if first effect is damage, else it uses
	// 200 scale. this includes DoTs.  no_partial_resist excludes spells like
	// necro snares
	for (int o = 0; o < EFFECT_COUNT; o++) {
		uint16 tid = spells[spell_id].effectid[o];

		if (IsBlankSpellEffect(spell_id, o)) continue;

		if ((tid == SE_CurrentHPOnce || tid == SE_CurrentHP) &&
		    spells[spell_id].base[o] < 0)
			return true;
...
bool IsResistableSpell(uint16 spell_id) {
	...
	// Torven: dispels do not have a MR check; they have a different check that
	// is entirely level based
	if (IsDetrimentalSpell(spell_id) && !IsPureDispelSpell(spell_id) &&
	    !IsAllianceSpellLine(spell_id) &&
	    spells[spell_id].resisttype != RESIST_NONE) {
		return true;
	}
...
bool IsBardSong(uint16 spell_id) {
	if (IsValidSpell(spell_id) && spells[spell_id].bardsong) return true;
...
bool IsEffectInSpell(uint16 spellid, int effect) {
	...
	if (!IsValidSpell(spellid)) return false;

	for (j = 0; j < EFFECT_COUNT; j++)
		if (spells[spellid].effectid[j] == effect) return true;
...
bool IsBlankSpellEffect(uint16 spellid, int effect_index) {
	...
	if (effect == SE_Blank ||
	    (effect == SE_CHA && base == 0 && formula == 100) ||
			...
			if (spells[sp2].effectid[blocked_slot] == blocked_effect) {
				if (value < blocked_below_value) {
					Log(Logs::General, Logs::Spells,
					    "Spell %d blocks one or more effects found in spell "
					    "%d.",
					    sp1, sp2);
					return true;
				}
...
bool IsValidSpell(uint16 spellid) {
	if (SPDAT_RECORDS > 0 && spellid != 0 && spellid != 1 &&
	    spellid != SPELL_UNKNOWN && spellid < SPDAT_RECORDS &&
...
int GetMinLevel(uint16 spell_id) { return spells[spell_id].min_castinglevel; }

int GetSpellLevel(uint16 spell_id, int classa) {
	if (classa >= PLAYER_CLASS_COUNT) return 255;
...
int GetSpellEffectIndex(uint16 spell_id, int effect) {
	...
	if (!IsValidSpell(spell_id)) return -1;

	for (i = 0; i < EFFECT_COUNT; i++)
		if (spells[spell_id].effectid[i] == effect) return i;
...
int CanUseSpell(uint16 spellid, int classa, int level) {
	...
	if (!IsValidSpell(spellid) || classa >= PLAYER_CLASS_COUNT) return 0;
...
bool IsRuneSpell(uint16 spell_id) {
	if (IsValidSpell(spell_id))
		for (int i = 0; i < EFFECT_COUNT; i++)
			if (spells[spell_id].effectid[i] == SE_Rune) return true;
...
bool IsMagicRuneSpell(uint16 spell_id) {
	if (IsValidSpell(spell_id))
		for (int i = 0; i < EFFECT_COUNT; i++)
			if (spells[spell_id].effectid[i] == SE_AbsorbMagicAtt) return true;

	return false;
}

bool IsManaTapSpell(uint16 spell_id) {
	if (IsValidSpell(spell_id) && spells[spell_id].manatapspell) return true;

	return false;
}

bool IsAllianceSpellLine(uint16 spell_id) {
	if (IsEffectInSpell(spell_id, SE_AddFaction)) return true;

	return false;
}

bool IsDeathSaveSpell(uint16 spell_id) {
	if (IsEffectInSpell(spell_id, SE_DeathSave)) return true;
...
bool IsShadowStepSpell(uint16 spell_id) {
	if (IsEffectInSpell(spell_id, SE_ShadowStep)) return true;

	return false;
}

bool IsSuccorSpell(uint16 spell_id) {
	if (IsEffectInSpell(spell_id, SE_Succor)) return true;

	return false;
}

bool IsTeleportSpell(uint16 spell_id) {
	if (IsEffectInSpell(spell_id, SE_Teleport)) return true;

	return false;
}

bool IsGateSpell(uint16 spell_id) {
	if (IsEffectInSpell(spell_id, SE_Gate)) return true;
...
int GetSpellEffectDescNum(uint16 spell_id) {
	if (IsValidSpell(spell_id)) return spells[spell_id].effectdescnum;

	return -1;
}

DmgShieldType GetDamageShieldType(uint16 spell_id, int32 DSType) {
	// If we have a DamageShieldType for this spell from the damageshieldtypes
	// table, return that, else, make a guess, based on the resist type. Default
	// return value is DS_THORNS
	if (IsValidSpell(spell_id)) {
		Log(Logs::Detail, Logs::Spells,
		    "DamageShieldType for spell %i (%s) is %X\n", spell_id,
...
bool IsRegularSingleTargetHealSpell(uint16 spell_id) {
	if (spells[spell_id].effectid[0] == 0 && spells[spell_id].base[0] > 0 &&
	    spells[spell_id].targettype == ST_Target &&
	    spells[spell_id].buffduration == 0 && !IsFastHealSpell(spell_id) &&
	    !IsCompleteHealSpell(spell_id) && !IsHealOverTimeSpell(spell_id) &&
	    !IsGroupSpell(spell_id))
		return true;
...
bool IsGroupCompleteHealSpell(uint16 spell_id) {
	if (IsGroupSpell(spell_id) && IsCompleteHealSpell(spell_id)) return true;

	return false;
}

bool IsGroupHealOverTimeSpell(uint16 spell_id) {
	if (IsGroupSpell(spell_id) && IsHealOverTimeSpell(spell_id) &&
	    spells[spell_id].buffduration < 10)
		return true;
...
bool IsDebuffSpell(uint16 spell_id) {
	if (IsBeneficialSpell(spell_id) || IsEffectHitpointsSpell(spell_id) ||
	    IsStunSpell(spell_id) || IsMezSpell(spell_id) ||
	    IsCharmSpell(spell_id) || IsSlowSpell(spell_id) ||
...
bool IsResistDebuffSpell(uint16 spell_id) {
	if ((IsEffectInSpell(spell_id, SE_ResistFire) ||
...
bool IsSelfConversionSpell(uint16 spell_id) {
	...
	if (spells[spell_id].targettype == ST_Self &&
	    ...
	    spells[spell_id].base[GetSpellEffectIndex(spell_id, SE_CurrentMana)] >
	        0 &&
...
bool IsBuffSpell(uint16 spell_id) {
	if (IsValidSpell(spell_id)) {
		...
		if (spells[spell_id].buffdurationformula == 0) return false;

		if (spells[spell_id].buffduration ||
...
bool IsSuspendableSpell(uint16 spell_id) {
	if (IsValidSpell(spell_id) && spells[spell_id].suspendable) return true;
...
bool RequiresComponents(uint16 spell_id) {
	if (IsValidSpell(spell_id)) {
		for (int t_count = 0; t_count < 4; t_count++) {
			int32 component = IsBardSong(spell_id)
			                      ? -1
			                      : spells[spell_id].components[t_count];
			int32 focuscomponent = spells[spell_id].NoexpendReagent[t_count];

			if (component == -1 && focuscomponent == -1) continue;

			if (component > 0 || focuscomponent > 0) return true;
		}
	}
...
bool DetrimentalSpellAllowsRest(uint16 spell_id) {
	if (IsValidSpell(spell_id)) return spells[spell_id].AllowRest;
...
bool IsSpellUsableThisZoneType(uint16 spell_id, uint8 zone_type) {
	// check if spell can be cast in any zone (-1 or 255), then if spell
	// zonetype matches zone's zonetype
	//  || spells[spell_id].zonetype == 255 comparing signed 8 bit int to 255 is
	//  always false
...
const char *GetSpellName(int16 spell_id) { return spells[spell_id].name; }

bool IsRacialIllusion(uint16 spell_id) {
	for (int i = 0; i < EFFECT_COUNT; i++) {
		if (spells[spell_id].effectid[i] == SE_Illusion &&
		    (spells[spell_id].base[i] == HUMAN ||
		     spells[spell_id].base[i] == BARBARIAN ||
		     spells[spell_id].base[i] == ERUDITE ||
		     spells[spell_id].base[i] == WOOD_ELF ||
		     spells[spell_id].base[i] == HIGH_ELF ||
		     spells[spell_id].base[i] == DARK_ELF ||
		     spells[spell_id].base[i] == HALF_ELF ||
		     spells[spell_id].base[i] == DWARF ||
		     spells[spell_id].base[i] == TROLL ||
		     spells[spell_id].base[i] == OGRE ||
		     spells[spell_id].base[i] == HALFLING ||
		     spells[spell_id].base[i] == GNOME ||
		     spells[spell_id].base[i] == IKSAR ||
	...
	if (IsBeneficialSpell(spell_id)) {
		for (int i = 0; i < EFFECT_COUNT; i++) {
			if (spells[spell_id].effectid[i] == SE_MovementSpeed &&
			    spells[spell_id].base[i] != 0)  // Stoicism, Torpor are negative
				return true;
		}
	}
	return false;
...
bool IsRainSpell(uint16 spell_id) {
	if (IsDetrimentalSpell(spell_id) &&
	    spells[spell_id].targettype == ST_AETarget &&
	    spells[spell_id].AEDuration > 2000 &&
	    spells[spell_id].AEDuration <
	        360000)  // Sentinel and Sanctuary have durations of 360,000
	{
		return true;
	}
...
bool IsShrinkSpell(uint16 spell_id) {
	...
	for (j = 0; j < EFFECT_COUNT; j++) {
		if (spells[spell_id].effectid[j] == SE_ModelSize &&
		    spells[spell_id].base[j] < 100) {
			return true;
		}
	}
...
bool IsLuclinPortSpell(uint16 spell_id) {
	if (spell_id == 2062 || spell_id == 2706 || spell_id == 2707 ||
	    spell_id == 2708 || spell_id == 2709 || spell_id == 2935)
		return true;
...
bool IsInvisSpell(uint16 spell_id) {
	return IsEffectInSpell(spell_id, SE_Invisibility) ||
	       IsEffectInSpell(spell_id, SE_InvisVsUndead) ||
	       IsEffectInSpell(spell_id, SE_InvisVsAnimals);
}

bool IsFixedDurationInvisSpell(uint16 spell_id) {
	if (spell_id == 2765 /* Camouflage (innate from AA) */ ||
...
bool IsInstrumentModdableSpellEffect(uint16 spell_id, int effect_index) {
	...
	switch (SPA) {
		case SE_CurrentHP:
		case SE_ArmorClass:
		case SE_ATK:            // Jonthan's Provocation, McVaxius` Rousing Rondo,
		                        // Jonthan's Inspiration, Warsong of Zek
		case SE_MovementSpeed:  // maybe only positive values should be modded?
		                        // Selo`s Consonant Chain uses this for snare
		case SE_STR:
		case SE_DEX:
		case SE_AGI:
		case SE_STA:
		...
		case SE_DamageShield:    // Psalm of Warmth, Psalm of Vitality, Psalm of
		                         // Cooling, Psalm of Purity, McVaxius` Rousing
		                         // Rondo, Warsong of Zek, Psalm of Veeshan
		case SE_AbsorbMagicAtt:  // Psalm of Mystic Shielding, Niv`s Melody of
		                         // Preservation, Shield of Songs, Niv`s
		                         // Harmonic
		case SE_ResistAll:       // Psalm of Veeshan
			return true;

		case SE_CurrentMana: {
			// Only these mana songs are moddable: Cassindra`s Chorus of
			// Clarity, Denon`s Dissension, Cassindra`s Chant of Clarity,
			// Ervaj's Lost Composition but we override the mod for the mana
			// regen songs in Mob::GetInstrumentMod()
			if (SPA == SE_CurrentMana &&
			    spells[spell_id].buffdurationformula == 0 &&
			    spells[spell_id].targettype != ST_Tap &&
			    spells[spell_id].targettype != ST_TargetAETap)
				return true;
			return false;
...
bool IsSplurtFormulaSpell(uint16 spell_id) {
	for (int i = 0; i < EFFECT_COUNT; i++) {
		...
		if (formula == 107 || formula == 108 || formula == 120 ||
		    formula == 122)
			return true;
	}
...
bool IsMGBCompatibleSpell(uint16 spell_id) {
	...
	if (s.can_mgb && s.goodEffect && s.buffdurationformula &&
	    (s.targettype == ST_Group || s.targettype == ST_GroupTeleport))
		return true;
...
bool IsDispellableSpell(uint16 spell_id) {
	...
	if (spells[spell_id].dispel_flag) return false;

	for (int i = 0; i < EFFECT_COUNT; i++) {
		int spa = spells[spell_id].effectid[i];

		// spells with counters must be cured with the same counter effect and
		// can't be dispelled with cancel magic effects
		if (spa == SE_PoisonCounter || spa == SE_DiseaseCounter ||
		    spa == SE_CurseCounter)
			return false;

		if (spa == SE_EyeOfZomm || spa == SE_SummonHorse ||
		    spa == SE_CompleteHeal)
			return false;
	}
