...
bool EQ::saylink::DegenerateLinkBody(SayLinkBody_Struct& say_link_body_struct,
	...
	say_link_body_struct.action_id =
	    (uint8)strtol(say_link_body.substr(0, 1).c_str(), nullptr, 16);
	say_link_body_struct.item_id =
	    (uint32)strtol(say_link_body.substr(1, 5).c_str(), nullptr, 16);

	return true;
}

bool EQ::saylink::GenerateLinkBody(
    std::string& say_link_body,
	...
	say_link_body = StringFormat(
	    "%1X"
	    "%s",
	    (0x0F & say_link_body_struct.action_id), itemid);
...
EQ::SayLinkEngine::SayLinkEngine() { Reset(); }
...
const std::string& EQ::SayLinkEngine::GenerateLink() {
	...
	if ((m_LinkBody.length() == EQ::constants::SAY_LINK_BODY_SIZE) &&
	    (m_LinkText.length() > 0)) {
	...
	if ((m_Link.length() == 0) ||
	    (m_Link.length() > EQ::constants::SAY_LINK_MAXIMUM_SIZE)) {
		m_Error = true;
		m_Link = "<LINKER ERROR>";
		LogError(
		    "SayLinkEngine::GenerateLink() failed to generate a useable say "
		    "link");
		LogError(
		    ">> LinkType: {}, Lengths: [link: {}({}), body: {}({}), text: "
		    "{}({})]",
		    m_LinkType, m_Link.length(), EQ::constants::SAY_LINK_MAXIMUM_SIZE,
		    m_LinkBody.length(), EQ::constants::SAY_LINK_BODY_SIZE,
		    m_LinkText.length(), EQ::constants::SAY_LINK_TEXT_SIZE);
...
void EQ::SayLinkEngine::generate_body() {
	...
	m_LinkBody = StringFormat(
	    "%1X"
	    "%s",
	    (0x0F & m_LinkBodyStruct.action_id), itemid);
}
...
// If link sayid is over 1000 it'll pop up a item info window instead of the
// questsay.
std::string EQ::SayLinkEngine::GenerateQuestSaylink(std::string saylink_text,
                                                    bool silent,
	...
	if (silent) saylink_id = saylink_id + 500;
...
std::string Saylink::Create(std::string saylink_text, bool silent,
                            std::string link_name) {
	return EQ::SayLinkEngine::GenerateQuestSaylink(saylink_text, silent,
	                                               link_name);
}
