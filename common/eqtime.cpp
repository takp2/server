...
// Input: Current Time (as a time_t), a pointer to the TimeOfDay_Struct that
// will be written to. Output: 0=Error, 1=Sucess

int EQTime::getEQTimeOfDay(time_t timeConvert,
                           struct TimeOfDay_Struct *eqTimeOfDay) {
	/* check to see if we have a reference time to go by. */
	if (eqTime.start_realtime == 0) return 0;
...
bool EQTime::IsTimeBefore(TimeOfDay_Struct *base, TimeOfDay_Struct *test) {
	if (base->year > test->year) return (true);
	if (base->year < test->year) return (false);
	// same years
	if (base->month > test->month) return (true);
	if (base->month < test->month) return (false);
	// same month
	if (base->day > test->day) return (true);
	if (base->day < test->day) return (false);
	// same day
	if (base->hour > test->hour) return (true);
	if (base->hour < test->hour) return (false);
	// same hour...
...
void EQTime::ToString(TimeOfDay_Struct *t, std::string &str) {
	char buf[128];
	snprintf(buf, 128, "%.2d/%.2d/%.4d %.2d:%.2d", t->month, t->day, t->year,
	         t->hour, t->minute);
	buf[127] = '\0';
	str = buf;
}
...
bool EQTime::IsNightTime() {
	...
	if (tod.hour >= 19) return true;
	if (tod.hour < 5) return true;
...
bool EQTime::IsInbetweenTime(uint8 mintime, uint8 maxtime) {
	...
	if (maxtime < mintime) {
		if ((tod.hour >= mintime && tod.hour > maxtime) ||
		    (tod.hour < mintime && tod.hour <= maxtime)) {
			return true;
		}
	} else {
		if (tod.hour >= mintime && tod.hour <= maxtime) {
