...
void Register(EQStreamIdentifier &into) {
	...
	if (opcodes == NULL) {
		...
		// TODO: figure out how to support shared memory with multiple
		// patches...
		opcodes = new RegularOpcodeManager();
		if (!opcodes->LoadOpcodes(opfile.c_str())) {
			logger.LogDebugType(EQEmuLogSys::General, EQEmuLogSys::Netcode,
			                    "[OPCODES] Error loading opcodes file %s. Not "
			                    "registering patch %s.",
			                    opfile.c_str(), name);
			return;
		}
...
void Reload() {
	...
	// opcode managers because we need to change the manager pointer, which
	// means we need to go to every stream and replace it's manager.
	...
	if (opcodes != NULL) {
		...
		if (!opcodes->ReloadOpcodes(opfile.c_str())) {
			logger.LogDebugType(
			    EQEmuLogSys::General, EQEmuLogSys::Netcode,
			    "[OPCODES] Error reloading opcodes file %s for patch %s.",
			    opfile.c_str(), name);
			return;
		}
		logger.LogDebugType(EQEmuLogSys::General, EQEmuLogSys::Netcode,
		                    "[OPCODES] Reloaded opcodes for patch %s", name);
	}
}
...
DECODE(OP_SetServerFilter) {
	SETUP_DIRECT_DECODE(SetServerFilter_Struct,
	                    structs::SetServerFilter_Struct);
	int r;
	for (r = 0; r < 25; r++) {
		IN(filters[r]);
	}
