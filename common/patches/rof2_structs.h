...
struct Item_Struct {
	...
	/*0186*/ uint8 unknown0186[2];  // Client dump has equipSlot/location as a
	...
	/*0208*/ float heading;
	/*0212*/ uint32
	    inv_refnum;  // Unique serial. This is required by apply poison.
	/*0216*/ int16 log;
	/*0218*/ int16 loot_log;
	/*0220*/ int16
	    avatar_level;  // Usually 01, sometimes seen as FFFF, once as 0.
	...
	union {
		struct {
			...
			/*0246*/ int8 MaxCharges;  // Maximum number of charges, for
			                           // rechargable? (Sept 25, 2002)
			...
			/*0276*/ int8
			    Stackable;  //  1= stackable, 3 = normal, 0 = ? (not stackable)
		} common;
		struct {
			...
			/*0261*/ int32 buffer1[4];   // Not used, fills out space in the
			                             // packet so ShowEQ doesn't complain.
		} book;
		struct {
			/*0228*/ int32 buffer2[10];  // Not used, fills out space in the
			                             // packet so ShowEQ doesn't complain.
...
// Added this struct for eqemu and started eimplimentation
// ProcessOP_SendLoginInfo
...
static const uint32 MAX_PP_SKILL =
    100;  // _SkillPacketArraySize;	// 100 - actual skills buffer size
...
#define pp_bank_inv_size 8
	...
	/*0926*/ int16
	    cursorbaginventory[pp_cursorbaginventory_size];  // If a bag is in slot
	                                                     // 0, this is where the
	                                                     // bag's items are
	/*0946*/ struct ItemProperties_Struct
	    bagItemProperties[pp_containerinv_size];  // Just like InvItemProperties
	...
	/*4768*/ int16
	    bank_inv[pp_bank_inv_size];  // Player Bank Inventory Item Numbers
	/*4784*/ int16
	    bank_cont_inv[pp_containerinv_size];  // Player Bank Inventory Item
	                                          // Numbers (Bags)
