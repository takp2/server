#define ENCODE(x)                                      \
	void Strategy::Encode_##x(EQApplicationPacket **p, \
...
#define CopyBlock(to, field, from, field1, field2)         \
	memcpy((void *)&to->field, (const void *)from->field1, \
	       StructDist(from, field1, field2));
...
#define OUT_array(x, n) \
	for (__i = 0; __i < n; __i++) eq->x[__i] = emu->x[__i];
...
#define ENCODE_LENGTH_EXACT(struct_)                                         \
	if ((*p)->size != sizeof(struct_)) {                                     \
		Log(Logs::Detail, Logs::Netcode,                                     \
		    "Wrong size on outbound %s (" #struct_ "): Got %d, expected %d", \
...
#define ENCODE_LENGTH_ATLEAST(struct_)                         \
	if ((*p)->size < sizeof(struct_)) {                        \
		Log(Logs::Detail, Logs::Netcode,                       \
		    "Wrong size on outbound %s (" #struct_             \
...
#define ENCODE_FORWARD(other_op) Encode_##other_op(p, dest, ack_req);
...
// Just checks to make sure the extra data is actually there. EQ clients could
// previously send 0-sized packets that require a decode and crash the server.
...
#define MEMSET_IN(emu_struct) memset(__packet->pBuffer, 0, sizeof(emu_struct));
...
#define FINISH_DIRECT_DECODE() delete[] __eq_buffer;
...
#define DECODE_LENGTH_EXACT(struct_)                                         \
	if (__packet->size != sizeof(struct_)) {                                 \
		...
		Log(Logs::Detail, Logs::Netcode,                                     \
		    "Wrong size on incoming %s (" #struct_ "): Got %d, expected %d", \
...
#define DECODE_LENGTH_ATLEAST(struct_)                                 \
	if (__packet->size < sizeof(struct_)) {                            \
		__packet->SetOpcode(OP_Unknown); /* invalidate the packet */   \
		Log(Logs::Detail, Logs::Netcode,                               \
		    "Wrong size on incoming %s (" #struct_                     \
		    "): Got %d, expected at least %d",                         \
		    opcodes->EmuToName(__packet->GetOpcode()), __packet->size, \
		    sizeof(struct_));                                          \
		return;                                                        \
...
#define DECODE_FORWARD(other_op) Decode_##other_op(__packet);
