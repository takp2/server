...
OpcodeManager::OpcodeManager() { loaded = false; }
...
bool OpcodeManager::LoadOpcodesFile(const char *filename, OpcodeSetStrategy *s,
	...
	while (!feof(opf)) {
		...
		if (fgets(line, sizeof(line), opf) == nullptr) break;

		// ignore any line that dosent start with OP_
		if (line[0] != 'O' || line[1] != 'P' || line[2] != '_') continue;
	...
	for (emu_op = OP_Unknown; emu_op < _maxEmuOpcode;
	     emu_op = (EmuOpcode)(emu_op + 1)) {
		// get the name of this emu opcode
		const char *op_name = OpcodeNames[emu_op];
		if (op_name[0] == '\0') {
			if (report_errors)
				printf(
				    "Over-ran the bounds of the opcode name array. You prolly "
				    "need to recompile the opcode stuff.\n");
			if (report_errors)
				printf(
				    "I will let you continue, since you may have gotten all "
				    "the opcodes you need.\n");
			break;
		}
...
RegularOpcodeManager::RegularOpcodeManager() : MutableOpcodeManager() {
	emu_to_eq = nullptr;
	eq_to_emu = nullptr;
}
...
bool RegularOpcodeManager::ReloadOpcodes(const char *filename,
                                         bool report_errors) {
	if (!loaded) return (LoadOpcodes(filename));
...
EmuOpcode RegularOpcodeManager::EQToEmu(const uint16 eq_op) {
	...
	// Disabled since current live EQ uses the entire uint16 bitspace for
	// opcodes 	if(eq_op > MAX_EQ_OPCODE) 		return(OP_Unknown);
	EmuOpcode res;
	MOpcodes.lock();
	res = eq_to_emu[eq_op];
	MOpcodes.unlock();
...
void RegularOpcodeManager::SetOpcode(EmuOpcode emu_op, uint16 eq_op) {
	...
	if (oldop != 0) eq_to_emu[oldop] = OP_Unknown;
...
void RegularOpcodeManager::NormalMemStrategy::Set(EmuOpcode emu_op,
                                                  uint16 eq_op) {
...
NullOpcodeManager::NullOpcodeManager() : MutableOpcodeManager() {}
...
uint16 NullOpcodeManager::EmuToEQ(const EmuOpcode emu_op) { return (0); }

EmuOpcode NullOpcodeManager::EQToEmu(const uint16 eq_op) {
	return (OP_Unknown);
}

EmptyOpcodeManager::EmptyOpcodeManager() : MutableOpcodeManager() {}
