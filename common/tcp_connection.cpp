...
bool TCPConnection::Connect(const char* irAddress, uint16 irPort, char* errbuf) {
	...
	if (!tmpIP) {
		if (errbuf) {
#ifdef _WINDOWS
			snprintf(errbuf, TCPConnection_ErrorBufferSize, "hostname failed to resolve code: %i", WSAGetLastError());
#else
			snprintf(errbuf, TCPConnection_ErrorBufferSize, "hostname failed to resolve code %i: %s", errno, strerror(errno));
#endif
		}
		return false;
	}
...
bool TCPConnection::ConnectIP(uint32 in_ip, uint16 in_port, char* errbuf) {
	...
	if (ConnectionType != Outgoing) {
		LogError("Failed connection type, not outgoing");
		// If this code runs, we got serious problems
		// Crash and burn.
		return false;
	}
	...
	if ((connection_socket = socket(AF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET || connection_socket == 0) {
		if (errbuf) {
#ifdef _WINDOWS
			snprintf(errbuf, TCPConnection_ErrorBufferSize, "connectip: allocating socket: %i", WSAGetLastError());
#else
			snprintf(errbuf, TCPConnection_ErrorBufferSize, "connectip: allocating socket: %s", strerror(errno));
#endif
		}
		SetState(TCPS_Ready);
		SetAsyncConnect(false);
	...
	if (connect(connection_socket, (PSOCKADDR)&server_sin, sizeof(server_sin)) == SOCKET_ERROR) {
		if (errbuf) {
			snprintf(errbuf, TCPConnection_ErrorBufferSize, "connectip: connect %i", WSAGetLastError());
		}
		closesocket(connection_socket);
		connection_socket = 0;
		SetState(TCPS_Ready);
	...
	if (connect(connection_socket, (struct sockaddr*)&server_sin, sizeof(server_sin)) == SOCKET_ERROR) {
		if (errbuf) {
			snprintf(errbuf, TCPConnection_ErrorBufferSize, "connectip: connect %s", strerror(errno));
		}
		close(connection_socket);
		connection_socket = 0;
		SetState(TCPS_Ready);
