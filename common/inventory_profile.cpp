...
void ItemInstQueue::push(EQ::ItemInstance* inst) { m_list.push_back(inst); }
...
bool EQ::InventoryProfile::SetInventoryVersion(
    versions::MobVersion inventory_version) {
	if (!m_mob_version_set) {
		...
		Log(Logs::General, Logs::Error,
		    "InventoryVersion set request after initial set (old: %u, new: %u)",
		    static_cast<uint32>(m_mob_version),
		    static_cast<uint32>(inventory_version));
		return false;
	}
}
...
EQ::ItemInstance* EQ::InventoryProfile::GetItem(int16 slot_id) const {
	...
	} else if ((slot_id >= invslot::GENERAL_BEGIN &&
	            slot_id <= invslot::GENERAL_END)) {
		// Personal inventory slots
		result = _GetItem(m_inv, slot_id);
	} else if ((slot_id >= invslot::EQUIPMENT_BEGIN &&
	            slot_id <= invslot::EQUIPMENT_END)) {
		// Equippable slots (on body)
		result = _GetItem(m_worn, slot_id);
	}

	// Inner bag slots
	else if (slot_id >= invbag::TRADE_BAGS_BEGIN &&
	         slot_id <= invbag::TRADE_BAGS_END) {
		// Trade bag slots
		ItemInstance* inst =
		    _GetItem(m_trade, InventoryProfile::CalcSlotId(slot_id));
		if (inst && inst->IsClassBag()) {
			result = inst->GetItem(InventoryProfile::CalcBagIdx(slot_id));
		}
	} else if (slot_id >= invbag::BANK_BAGS_BEGIN &&
	           slot_id <= invbag::BANK_BAGS_END) {
		// Bank bag slots
		ItemInstance* inst =
		    _GetItem(m_bank, InventoryProfile::CalcSlotId(slot_id));
		if (inst && inst->IsClassBag()) {
			result = inst->GetItem(InventoryProfile::CalcBagIdx(slot_id));
		}
	} else if (slot_id >= invbag::CURSOR_BAG_BEGIN &&
	           slot_id <= invbag::CURSOR_BAG_END) {
		// Cursor bag slots
		ItemInstance* inst = m_cursor.peek_front();
		if (inst && inst->IsClassBag()) {
			result = inst->GetItem(InventoryProfile::CalcBagIdx(slot_id));
		}
	} else if (slot_id >= invbag::GENERAL_BAGS_BEGIN &&
	           slot_id <= invbag::GENERAL_BAGS_END) {
		// Personal inventory bag slots
		ItemInstance* inst =
		    _GetItem(m_inv, InventoryProfile::CalcSlotId(slot_id));
		if (inst && inst->IsClassBag()) {
			result = inst->GetItem(InventoryProfile::CalcBagIdx(slot_id));
		}
	}
...
EQ::ItemInstance* EQ::InventoryProfile::GetItem(int16 slot_id,
                                                uint8 bagidx) const {
...
bool EQ::InventoryProfile::DeleteItem(int16 slot_id, uint8 quantity) {
	...
	if (item_to_delete && (quantity > 0)) {
		...
		if (item_to_delete->GetCharges() <= 0) {
			...
			// the item is not stackable, and is not a charged item, or is
			// expendable, delete it
		...
		// Charges still exist, or it is a charged item that is not expendable.
...
bool EQ::InventoryProfile::CheckNoDrop(int16 slot_id) {
	...
	if (inst->GetItem()->ItemClass == 1) {
		for (uint8 i = invbag::SLOT_BEGIN; i <= invbag::SLOT_END; i++) {
			ItemInstance* bagitem =
			    GetItem(InventoryProfile::CalcSlotId(slot_id, i));
...
EQ::ItemInstance* EQ::InventoryProfile::PopItem(int16 slot_id) {
	...
	} else if (slot_id >= invslot::TRADE_BEGIN &&
	           slot_id <= invslot::TRADE_END) {
		p = m_trade[slot_id];
...
bool EQ::InventoryProfile::HasSpaceForItem(const ItemData* ItemToTry,
                                           int16 Quantity) {
	if (ItemToTry->Stackable) {
		for (int16 i = invslot::GENERAL_BEGIN; i <= invslot::GENERAL_END; i++) {
			...
			if (InvItem && (InvItem->GetItem()->ID == ItemToTry->ID) &&
				...
				int ChargeSlotsLeft =
				    InvItem->GetItem()->StackSize - InvItem->GetCharges();
			...
			if (InvItem && InvItem->IsClassBag()) {
				int16 BaseSlotID =
				    InventoryProfile::CalcSlotId(i, invbag::SLOT_BEGIN);
				uint8 BagSize = InvItem->GetItem()->BagSlots;
				for (uint8 BagSlot = invbag::SLOT_BEGIN; BagSlot < BagSize;
				     BagSlot++) {
					InvItem = GetItem(BaseSlotID + BagSlot);

					if (InvItem && (InvItem->GetItem()->ID == ItemToTry->ID) &&
					    (InvItem->GetCharges() <
					     InvItem->GetItem()->StackSize)) {
						int ChargeSlotsLeft = InvItem->GetItem()->StackSize -
						                      InvItem->GetCharges();
	...
	for (int16 i = invslot::GENERAL_BEGIN; i <= invslot::GENERAL_END; i++) {
		...
		} else if (InvItem->IsClassBag() &&
		           CanItemFitInContainer(ItemToTry, InvItem->GetItem())) {
			int16 BaseSlotID =
			    InventoryProfile::CalcSlotId(i, invbag::SLOT_BEGIN);
...
// Checks that user has at least 'quantity' number of items in a given inventory
// slot Returns first slot it was found in, or INVALID_INDEX if not found

// This function has a flaw in that it only returns the last stack that it
// looked at when quantity is greater than 1 and not all of quantity can be
// found in 1 stack.
int16 EQ::InventoryProfile::HasItem(int16 item_id, uint8 quantity,
                                    uint8 where) {
	...
	if (where & invWhereWorn) {
		...
		if (slot_id != INVALID_INDEX) return slot_id;
	}
...
int16 EQ::InventoryProfile::HasArtifactItem() {
	...
	for (int16 i = invslot::slotCursor; i <= invslot::GENERAL_END; i++) {
		if (GetItem(i) && GetItem(i)->GetItem()->Lore[0] == '#') return i;
	}

	for (int16 i = invbag::GENERAL_BAGS_BEGIN; i <= invbag::GENERAL_BAGS_END;
	     i++) {
		if (GetItem(i) && GetItem(i)->GetItem()->Lore[0] == '#') return i;
	}
...
int16 EQ::InventoryProfile::HasItemByUse(uint8 use, uint8 quantity,
                                         uint8 where) {
	int16 slot_id = INVALID_INDEX;
...
int16 EQ::InventoryProfile::FindFreeSlot(bool for_bag, bool try_cursor,
                                         uint8 min_size, bool is_arrow) {
	...
	if (!for_bag) {
		for (int16 i = invslot::GENERAL_BEGIN; i <= invslot::GENERAL_END; i++) {
			...
			if (inst && inst->IsClassBag() &&
				...
				if (is_arrow &&
				    inst->GetItem()->BagType == item::BagTypeQuiver) {
					int16 base_slot_id =
					    InventoryProfile::CalcSlotId(i, invslot::SLOT_BEGIN);
...
// This is a mix of HasSpaceForItem and FindFreeSlot..due to existing coding
// behavior, it was better to add a new helper function...
int16 EQ::InventoryProfile::FindFreeSlotForTradeItem(const ItemInstance* inst) {
	// Do not arbitrarily use this function..it is designed for use with
	// Client::ResetTrade() and Client::FinishTrade(). If you have a need, use
	// it..but, understand it is not a compatible replacement for
	// Inventory::FindFreeSlot().
	//
	// I'll probably implement a bitmask in the new inventory system to avoid
	// having to adjust stack bias -U

	if (!inst || !inst->GetID()) return INVALID_INDEX;

	// step 1: find room for bags (caller should really ask for slots for bags
	// first to avoid sending them to cursor..and bag item loss)
	if (inst->IsClassBag()) {
		for (int16 free_slot = invslot::GENERAL_BEGIN;
		     free_slot <= invslot::GENERAL_END; ++free_slot)
			if (!m_inv[free_slot]) return free_slot;
	...
	if (inst->IsStackable()) {
		for (int16 free_slot = invslot::GENERAL_BEGIN;
			...
			if (!main_inst) continue;

			if ((main_inst->GetID() == inst->GetID()) &&
		...
		for (int16 free_slot = invslot::GENERAL_BEGIN;
			...
			if (main_inst->IsClassBag()) {  // if item-specific containers
				                            // already have bad items, we won't
				                            // fix it here...
				for (uint8 free_bag_slot = invbag::SLOT_BEGIN;
				     (free_bag_slot < main_inst->GetItem()->BagSlots) &&
				     (free_bag_slot <= invbag::SLOT_END);
				     ++free_bag_slot) {
					const ItemInstance* sub_inst =
					    main_inst->GetItem(free_bag_slot);

					if (!sub_inst) continue;

					if ((sub_inst->GetID() == inst->GetID()) &&
					    (sub_inst->GetCharges() <
					     sub_inst->GetItem()->StackSize))
						return InventoryProfile::CalcSlotId(free_slot,
						                                    free_bag_slot);
				}
			}
		}
	...
	for (int16 free_slot = invslot::GENERAL_BEGIN;
		...
		if (main_inst && main_inst->IsClassBag()) {
			if ((main_inst->GetItem()->BagSize < inst->GetItem()->Size) ||
			    (main_inst->GetItem()->BagType == item::BagTypeQuiver))
				continue;
...
int16 EQ::InventoryProfile::CalcSlotId(int16 slot_id) {
	...
	if (slot_id >= invbag::GENERAL_BAGS_BEGIN &&
		...
		parent_slot_id =
		    ...
		    (slot_id - invbag::GENERAL_BAGS_BEGIN) / invbag::SLOT_COUNT;
	...
	// this is not a bag range... using this risks over-writing existing items
	    ...
	    parent_slot_id = invslot::BANK_BEGIN + (slot_id - invslot::BANK_BEGIN) /
	invbag::SLOT_COUNT;
	*/
		...
		parent_slot_id =
		    invslot::BANK_BEGIN +
		    (slot_id - invbag::BANK_BAGS_BEGIN) / invbag::SLOT_COUNT;

	// else if (slot_id >= 3100 && slot_id <= 3179) should be
	// {3031..3110}..where did this range come from!!? (verified db save range)
	else if (slot_id >= invbag::TRADE_BAGS_BEGIN &&
	         slot_id <= invbag::TRADE_BAGS_END)
		parent_slot_id =
		    invslot::TRADE_BEGIN +
		    (slot_id - invbag::TRADE_BAGS_BEGIN) / invbag::SLOT_COUNT;
...
int16 EQ::InventoryProfile::CalcSlotId(int16 bagslot_id, uint8 bagidx) {
	if (!InventoryProfile::SupportsContainers(bagslot_id)) return INVALID_INDEX;

	int16 slot_id = INVALID_INDEX;

	if (bagslot_id == invslot::slotCursor ||
	    bagslot_id == invslot::CURSOR_QUEUE_BEGIN)
		slot_id = invbag::CURSOR_BAG_BEGIN + bagidx;

	else if (bagslot_id >= invslot::GENERAL_BEGIN &&
	         bagslot_id <= invslot::GENERAL_END)
		slot_id = invbag::GENERAL_BAGS_BEGIN +
		          (bagslot_id - invslot::GENERAL_BEGIN) * invbag::SLOT_COUNT +
		          bagidx;

	else if (bagslot_id >= invslot::BANK_BEGIN &&
	         bagslot_id <= invslot::BANK_END)
		slot_id = invbag::BANK_BAGS_BEGIN +
		          (bagslot_id - invslot::BANK_BEGIN) * invbag::SLOT_COUNT +
		          bagidx;

	else if (bagslot_id >= invslot::TRADE_BEGIN &&
	         bagslot_id <= invslot::TRADE_END)
		slot_id = invbag::TRADE_BAGS_BEGIN +
		          (bagslot_id - invslot::TRADE_BEGIN) * invbag::SLOT_COUNT +
		          bagidx;
...
uint8 EQ::InventoryProfile::CalcBagIdx(int16 slot_id) {
	...
	if (slot_id >= invbag::GENERAL_BAGS_BEGIN &&
		...
		index = (slot_id - invbag::GENERAL_BAGS_BEGIN) % invbag::SLOT_COUNT;
	         ...
	         slot_id <= invbag::CURSOR_BAG_END)
		index = (slot_id -
		         invbag::CURSOR_BAG_BEGIN);  // % invbag::SLOT_COUNT; - not
		                                     // needed since range is 10 slots
	...
	*/
	...
	// odd..but, ok... (probably a range-slot conversion for ItemInstance*
	// Object::item
	else if (slot_id >= invslot::WORLD_BEGIN && slot_id <= invslot::WORLD_END)
		index =
		    (slot_id - invslot::WORLD_BEGIN);  // % invbag::SLOT_COUNT; - not
		                                       // needed since range is 10 slots
...
uint8 EQ::InventoryProfile::CalcMaterialFromSlot(int16 equipslot) {
	switch (equipslot) {
		...
		case invslot::slotWrist2:
			return textures::armorWrist;  // Both wrists share a single texture
			                              // slot.
...
bool EQ::InventoryProfile::CanItemFitInContainer(const ItemData* ItemToTry,
                                                 const ItemData* Container) {
	if (!ItemToTry || !Container) return false;

	if (ItemToTry->Size > Container->BagSize) return false;

	if ((Container->BagType == item::BagTypeQuiver) &&
...
bool EQ::InventoryProfile::SupportsClickCasting(int16 slot_id) {
	// there are a few non-potion items that identify as ItemTypePotion..so, we
	// still need to ubiquitously include the equipment range
...
uint8 EQ::InventoryProfile::FindBrightestLightType() {
	...
	for (auto iter = m_worn.begin(); iter != m_worn.end(); ++iter) {
		if ((iter->first < invslot::EQUIPMENT_BEGIN ||
		     iter->first > invslot::EQUIPMENT_END)) {
			continue;
		}
	...
	for (auto iter = m_inv.begin(); iter != m_inv.end(); ++iter) {
		if (iter->first < invslot::GENERAL_BEGIN ||
		    iter->first > invslot::GENERAL_END) {
			continue;
		}
...
int EQ::InventoryProfile::GetSlotByItemInstCollection(
	...
	for (auto iter = collection.begin(); iter != collection.end(); ++iter) {
		...
		if (t_inst && !t_inst->IsClassBag()) {
			for (auto b_iter = t_inst->_cbegin(); b_iter != t_inst->_cend();
			     ++b_iter) {
				if (b_iter->second == inst) {
					return InventoryProfile::CalcSlotId(iter->first,
					                                    b_iter->first);
				}
			}
		}
...
void EQ::InventoryProfile::dumpItemCollection(
    const std::map<int16, ItemInstance*>& collection) {
	for (auto it = collection.cbegin(); it != collection.cend(); ++it) {
		auto inst = it->second;
		if (!inst || !inst->GetItem()) continue;

		std::string slot = StringFormat(
		    "Slot %d: %s (%d)", it->first, it->second->GetItem()->Name,
		    (inst->GetCharges() <= 0) ? 1 : inst->GetCharges());
...
void EQ::InventoryProfile::dumpBagContents(
    ItemInstance* inst, std::map<int16, ItemInstance*>::const_iterator* it) {
	...
	for (auto itb = inst->_cbegin(); itb != inst->_cend(); ++itb) {
		...
		std::string subSlot = StringFormat(
		    "	Slot %d: %s (%d)",
		    InventoryProfile::CalcSlotId((*it)->first, itb->first),
		    baginst->GetItem()->Name,
		    (baginst->GetCharges() <= 0) ? 1 : baginst->GetCharges());
		std::cout << subSlot << std::endl;
	}
}

// Internal Method: Retrieves item within an inventory bucket
EQ::ItemInstance* EQ::InventoryProfile::_GetItem(
    const std::map<int16, ItemInstance*>& bucket, int16 slot_id) const {
...
// Internal Method: "put" item into bucket, without regard for what is currently
// in bucket Assumes item has already been allocated
int16 EQ::InventoryProfile::_PutItem(int16 slot_id, ItemInstance* inst) {
	// If putting a nullptr into slot, we need to remove slot without memory
	// delete
	if (inst == nullptr) {
	...
	if (slot_id == invslot::slotCursor) {
		...
		m_cursor.pop();  // no memory delete, clients of this function know what
		                 // they are doing
		...
		ItemInstance* baginst =
		    GetItem(InventoryProfile::CalcSlotId(slot_id));  // Get parent bag
	...
	if (result == INVALID_INDEX) {
		LogError("Inventory::_PutItem: Invalid slot_id specified ({})",
		         slot_id);
...
int16 EQ::InventoryProfile::_HasItem(std::map<int16, ItemInstance*>& bucket,
                                     int16 item_id, uint8 quantity) {
	...
	for (auto iter = bucket.begin(); iter != bucket.end(); ++iter) {
		...
		if (inst->GetID() == item_id) {
			quantity_found +=
			    (inst->GetCharges() <= 0) ? 1 : inst->GetCharges();
			if (quantity_found >= quantity) return iter->first;
		}
		...
		for (auto bag_iter = inst->_cbegin(); bag_iter != inst->_cend();
		     ++bag_iter) {
			...
			if (bag_inst->GetID() == item_id) {
				quantity_found +=
				    (bag_inst->GetCharges() <= 0) ? 1 : bag_inst->GetCharges();
...
int16 EQ::InventoryProfile::_HasItem(ItemInstQueue& iqueue, int16 item_id,
                                     uint8 quantity) {
	// The downfall of this (these) queue procedure is that callers presume that
	// when an item is found, it is presented as being available on the cursor.
	// In cases of a parity check, this is sufficient. However, in cases where
	// referential criteria is considered, this can lead to unintended results.
	// Functionality should be observed when referencing the return value of
	// this query
	...
	for (auto iter = iqueue.cbegin(); iter != iqueue.cend(); ++iter) {
		...
		if (inst->GetID() == item_id) {
			quantity_found +=
			    (inst->GetCharges() <= 0) ? 1 : inst->GetCharges();
			if (quantity_found >= quantity) return invslot::slotCursor;
		}
		...
		for (auto bag_iter = inst->_cbegin(); bag_iter != inst->_cend();
			...
			if (bag_inst->GetID() == item_id) {
				quantity_found +=
				    (bag_inst->GetCharges() <= 0) ? 1 : bag_inst->GetCharges();
				if (quantity_found >= quantity)
					return InventoryProfile::CalcSlotId(invslot::slotCursor,
					                                    bag_iter->first);
			}
		}

		// We only check the visible cursor due to lack of queue processing
		// ability (client allows duplicate in limbo)
		break;
	}
...
// Internal Method: Checks an inventory bucket for a particular item
int16 EQ::InventoryProfile::_HasItemByUse(
    std::map<int16, ItemInstance*>& bucket, uint8 use, uint8 quantity) {
	uint8 quantity_found = 0;
	...
	for (auto iter = bucket.begin(); iter != bucket.end(); ++iter) {
		...
		for (auto bag_iter = inst->_cbegin(); bag_iter != inst->_cend();
			...
			if (bag_inst->IsClassCommon() &&
			    bag_inst->GetItem()->ItemType == use) {
				quantity_found +=
				    (bag_inst->GetCharges() <= 0) ? 1 : bag_inst->GetCharges();
				if (quantity_found >= quantity)
					return InventoryProfile::CalcSlotId(iter->first,
...
// Internal Method: Checks an inventory queue type bucket for a particular item
int16 EQ::InventoryProfile::_HasItemByUse(ItemInstQueue& iqueue, uint8 use,
                                          uint8 quantity) {
	uint8 quantity_found = 0;
