...
class BaseGuildManager {
   public:
	...
	bool GetEntireGuild(uint32 guild_id,
	                    std::vector<CharGuildInfo *>
	                        &members);  // caller is responsible for deleting
	                                    // each pointer in the resulting vector.
	bool GuildExists(uint32 guild_id) const;
	bool GetGuildMOTD(uint32 guild_id, char *motd_buffer,
	...
	//	bool	EditGuild(uint32 guild_id, uint8 ranknum, GuildRankLevel_Struct*
	// grl);

	unsigned char *MakeOldGuildList(uint32 &length)
	    const;  // make a guild list packet, returns ownership of the buffer.
   ...
   protected:
	...
	virtual void SendGuildRefresh(uint32 guild_id, bool name, bool motd,
	                              bool rank, bool relation) = 0;
	virtual void SendCharRefresh(uint32 old_guild_id, uint32 guild_id,
	                             uint32 charid) = 0;
	...
	GuildInfo *_CreateGuild(uint32 guild_id, const char *guild_name,
	                        uint32 account_id, uint8 minstatus,
	                        const char *guild_motd, const char *motd_setter,
