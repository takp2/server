...
int8 EQ::inventory::ConvertEquipmentSlotToTextureIndex(
    const InventorySlot& inventory_slot) {
	if ((!inventory_slot.Typeless() &&
	     !inventory_slot.IsTypeIndex(invtype::typePossessions)) ||
	    !inventory_slot.IsContainerIndex(invbag::SLOT_INVALID))
		return textures::textureInvalid;
...
bool EQ::InventorySlot::IsValidSlot() const {
	if (_typeless) return false;

	int16 slot_count = invtype::GetInvTypeSize(_type_index);
	if (!slot_count || _slot_index < invslot::SLOT_BEGIN ||
	    _slot_index >= slot_count)
		return false;

	if (_container_index < invbag::SLOT_INVALID ||
	    _container_index >= invbag::SLOT_COUNT)
		return false;
...
bool EQ::InventorySlot::IsDeleteSlot() const {
	if (_typeless)
		return (_slot_index == invslot::SLOT_INVALID &&
		        _container_index == invbag::SLOT_INVALID);
	else
		return (_type_index == invtype::TYPE_INVALID &&
		        _slot_index == invslot::SLOT_INVALID &&
		        _container_index == invbag::SLOT_INVALID);
}

bool EQ::InventorySlot::IsEquipmentIndex(int16 slot_index) {
	/*if (slot_index < inventory::EquipmentBegin || slot_index >
	   inventory::EquipmentEnd) return false;*/
	if ((slot_index < invslot::EQUIPMENT_BEGIN ||
	     slot_index > invslot::EQUIPMENT_END) &&
	    slot_index != legacy::SLOT_QUEST)
		return false;
...
bool EQ::InventorySlot::IsGeneralIndex(int16 slot_index) {
	/*if (slot_index < inventory::GeneralBegin || slot_index >
	   inventory::GeneralEnd) return false;*/
	if (slot_index < invslot::GENERAL_BEGIN ||
	    slot_index > invslot::GENERAL_END)
		return false;
...
bool EQ::InventorySlot::IsCursorIndex(int16 slot_index) {
	/*if (slot_index != invslot::slotCursor)
	    return false;*/
	if (slot_index != invslot::slotCursor) return false;

	return true;
}

bool EQ::InventorySlot::IsWeaponIndex(int16 slot_index) {
	/*if ((slot_index != inventory::slotRange) && (slot_index !=
	   inventory::slotPrimary) && (slot_index != inventory::slotSecondary))
	    return false;*/
	if ((slot_index != invslot::slotRange) &&
	    (slot_index != invslot::slotPrimary) &&
	    (slot_index != invslot::slotSecondary))
		return false;
...
bool EQ::InventorySlot::IsEquipmentSlot() const {
	if (!_typeless && (_type_index != invtype::typePossessions)) return false;

	if (_container_index != invbag::SLOT_INVALID) return false;
...
bool EQ::InventorySlot::IsSlot() const {
	if (!_typeless && (_type_index == invtype::TYPE_INVALID)) return false;
	if (_slot_index == invslot::SLOT_INVALID) return false;
	if (_container_index != invbag::SLOT_INVALID) return false;
...
EQ::InventorySlot EQ::InventorySlot::ToOwner() const {
	if (IsSlot() || IsContainer()) return InventorySlot(_type_index);

	return InventorySlot();
}

const std::string EQ::InventorySlot::ToString() const {
	return StringFormat("(%i%s, %i, %i, %i)", _type_index,
	                    (_typeless ? " [typeless]" : ""), _slot_index,
	                    _container_index);
}

const std::string EQ::InventorySlot::ToName() const {
	return StringFormat(
	    "InventorySlot - _type_index: %i%s, _slot_index: %i, _container_index: "
	    "%i, _socket_index: %i",
	    _type_index, (_typeless ? " [typeless]" : ""), _slot_index,
	    _container_index);
}
...
//{
//	if ((slot_index >= inventory::EquipmentBegin) && (slot_index <=
// inventory::EquipmentEnd) && (slot_index != invslot::slotAmmo)) 		return true;
//
//	return false;
// }
...
//{
//	if ((_type_index != invtype::typePossessions) || (_container_index !=
// invbag::SLOT_INVALID) || (_socket_index != inventory::socketInvalid)) 		return
// false;
//
//	return IsBonusIndex(_slot_index);
// }

bool inventory_slot_typeless_lessthan(const EQ::InventorySlot& lhs,
                                      const EQ::InventorySlot& rhs) {
	if (lhs.SlotIndex() < rhs.SlotIndex()) return true;

	if ((lhs.SlotIndex() == rhs.SlotIndex()) &&
	    (lhs.ContainerIndex() < rhs.ContainerIndex()))
		return true;
...
bool EQ::InventorySlot::operator<(const InventorySlot& rhs) const {
	...
	if (TypeIndex() < rhs.TypeIndex()) return true;

	if ((TypeIndex() == rhs.TypeIndex()) && (SlotIndex() < rhs.SlotIndex()))
		return true;

	if ((TypeIndex() == rhs.TypeIndex()) && (SlotIndex() == rhs.SlotIndex()) &&
	    (ContainerIndex() < rhs.ContainerIndex()))
		return true;
