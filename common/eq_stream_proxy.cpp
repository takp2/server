...
EQStreamProxy::EQStreamProxy(EQStream *&stream, const StructStrategy *structs,
                             OpcodeManager **opcodes)
    : m_stream(stream), m_structs(structs), m_opcodes(opcodes) {
...
EQStreamProxy::EQStreamProxy(EQOldStream *&stream,
...
std::string EQStreamProxy::Describe() const { return (m_structs->Describe()); }
...
void EQStreamProxy::Close() { m_stream->Close(); }

uint32 EQStreamProxy::GetRemoteIP() const { return (m_stream->GetRemoteIP()); }
...
void EQStreamProxy::RemoveData() { m_stream->RemoveData(); }

bool EQStreamProxy::IsInUse() { return m_stream->IsInUse(); }

bool EQStreamProxy::CheckState(EQStreamState state) {
	if (m_stream) return (m_stream->CheckState(state));
