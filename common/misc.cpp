...
*/
bool ItemParse(const char *data, int length,
               std::map<int, std::map<int, std::string> > &items, int id_pos,
               int name_pos, int max_field, int level) {
	int i;
	char *end, *ptr;
	...
	if (!buffsize || buffsize < (length + 1)) {
		...
		std::allocator<char> > > > > > > > &,int,int,int,int)'::`2'::buffer',
		which is passed as an argument to 'realloc', will cause the original
		memory block to be leaked. common	misc.cpp	90 similar message for
		91
		*/
	...
	for (i = 0; i < name_pos - 1; i++) {
		...
		if (!end) {
			std::cerr << "ItemParse: Level " << level
			          << ": (1) Expected '|' not found near '" << ptr << "'"
			          << std::endl;
			return false;
		} else {
			*end = 0;
	...
	for (i = 0; i < 10; i++) {
		if (*ptr == '"') {
			...
			if (end) {
				...
				Unprotect(sub, '"');
				if (!ItemParse(sub.c_str(), sub.length(), items, id_pos,
				               name_pos, max_field, level + 1)) {
					return false;
				}
				ptr = end + 1;
			} else {
				std::cerr << "ItemParse: Level " << level
				          << ": (6) Expected closing '\"' not found near '"
				          << (ptr + 1) << "'" << std::endl;
				return false;
			}
		}
		if (*ptr == '|') {
			...
			std::cerr << "ItemParse: Level " << level << ": (7) Expected '|' (#"
			          << i << ") not found near '" << ptr << "'" << std::endl;
			return false;
		}
	}
	return true;
...
int Tokenize(std::string s, std::map<int, std::string> &tokens, char delim) {
	...
	while (s.length()) {
		if (s[0] == delim) {
			...
			while ((end = s.find(delim, end + 1)) != std::string::npos &&
			       s[end - 1] == '\\')
				;
...
void dump_message_column(unsigned char *buffer, unsigned long length,
                         std::string leader, FILE *to) {
	unsigned long i, j;
...
std::string long2ip(unsigned long ip) {
	...
	ipoctet.ip = ip;
	sprintf(temp, "%d.%d.%d.%d", ipoctet.octet.a, ipoctet.octet.b,
	        ipoctet.octet.c, ipoctet.octet.d);
...
int EQsprintf(char *buffer, const char *pattern, const char *arg1,
...
void build_hex_line(const char *buffer, unsigned long length,
                    unsigned long offset, char *out_buffer,
