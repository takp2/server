...
PacketFileWriter::~PacketFileWriter() { CloseFile(); }
...
bool PacketFileWriter::SetPacketStamp(const char *name, uint32 stamp) {
	...
	if (magic == OLD_PACKET_FILE_MAGIC) {
		...
		if (fwrite(&hdr.packet_file_stamp, sizeof(hdr.packet_file_stamp), 1,
		           in) != 1) {
...
void PacketFileWriter::WritePacket(uint16 eq_op, uint32 packlen,
                                   const unsigned char *packet, bool to_server,
	...
	/*
	    Could log only the packets we care about, but this is most of the
	   stream, so just log them all...
...
bool PacketFileWriter::_WriteBlock(uint16 eq_op, const void *d, uint16 len,
                                   bool to_server, const struct timeval &tv) {
...
PacketFileReader::PacketFileReader() { packet_file_stamp = 0; }

OldPacketFileReader::OldPacketFileReader() : PacketFileReader() { in = NULL; }

OldPacketFileReader::~OldPacketFileReader() { CloseFile(); }
...
bool OldPacketFileReader::OpenFile(const char *name) {
	...
	if (head.packet_file_magic != OLD_PACKET_FILE_MAGIC) {
		...
		if (head.packet_file_magic > (OLD_PACKET_FILE_MAGIC)) {
			fprintf(stderr,
			        "Error: this is a build file, not a packet file, its "
			        "allready processed!\n");
		} else {
	...
	if (head.packet_file_stamp > now) {
		fprintf(stderr,
		        "Error: invalid timestamp in file. Your clock or the "
...
bool OldPacketFileReader::ReadPacket(uint16 &eq_op, uint32 &packlen,
                                     unsigned char *packet, bool &to_server,
                                     struct timeval &tv) {
	if (in == NULL) return (false);
	...
	if (fread(packet, 1, s.len, in) != s.len) {
		if (feof(in))
			fprintf(stderr,
			        "Error: EOF encountered when expecting packet data.\n");
		else
...
NewPacketFileReader::NewPacketFileReader() : PacketFileReader() { in = NULL; }

NewPacketFileReader::~NewPacketFileReader() { CloseFile(); }
...
bool NewPacketFileReader::ReadPacket(uint16 &eq_op, uint32 &packlen,
                                     unsigned char *packet, bool &to_server,
                                     struct timeval &tv) {
	if (in == NULL) return (false);
	if (feof(in)) return (false);
