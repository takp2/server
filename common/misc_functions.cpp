...
void CoutTimestamp(bool ms) {
	...
	std::cout << (gmt_t->tm_year + 1900) << "/" << std::setw(2)
	          << std::setfill('0') << (gmt_t->tm_mon + 1) << "/" << std::setw(2)
	          << std::setfill('0') << gmt_t->tm_mday << " " << std::setw(2)
	          << std::setfill('0') << gmt_t->tm_hour << ":" << std::setw(2)
	          << std::setfill('0') << gmt_t->tm_min << ":" << std::setw(2)
	          << std::setfill('0') << gmt_t->tm_sec;
	if (ms)
...
uint32 ResolveIP(const char* hostname, char* errbuf) {
	...
	if (hostname == 0) {
		if (errbuf) snprintf(errbuf, ERRBUF_SIZE, "ResolveIP(): hostname == 0");
		return 0;
	}
	...
	if ((phostent = gethostbyname(hostname)) == nullptr) {
		...
		if (errbuf)
			snprintf(errbuf, ERRBUF_SIZE,
			         "Unable to get the host name. Error: %i",
			         WSAGetLastError());
#else
		if (errbuf)
			snprintf(errbuf, ERRBUF_SIZE,
...
#ifdef _WINDOWS
	memcpy((char FAR*)&(server_sin.sin_addr), phostent->h_addr,
	       phostent->h_length);
#else
...
bool ParseAddress(const char* iAddress, uint32* oIP, uint16* oPort,
	...
	if (sep.argnum == 1 && sep.IsNumber(1)) {
		*oIP = ResolveIP(sep.arg[0], errbuf);
		if (*oIP == 0) return false;
		if (oPort) *oPort = atoi(sep.arg[1]);
		return true;
	}
	return false;
}
...
InitWinsock::~InitWinsock() { WSACleanup(); }
...
float EQ13toFloat(int d) { return (float(d) / float(1 << 2)); }

float NewEQ13toFloat(int d) { return (float(d) / float(1 << 6)); }

float EQ19toFloat(int d) { return (float(d) / float(1 << 3)); }

int FloatToEQ13(float d) { return int(d * float(1 << 2)); }

int NewFloatToEQ13(float d) { return int(d * float(1 << 6)); }

int FloatToEQ19(float d) { return int(d * float(1 << 3)); }

/*
    Heading of 0 points in the pure positive Y direction

*/
int FloatToEQH(float d) { return (int((360.0f - d) * float(1 << 11)) / 360); }

float EQHtoFloat(int d) { return (360.0f - float((d * 360) >> 11)); }

// returns a swapped-bit value for use in client translator and inventory
// functions
uint32 SwapBits21And22(uint32 mask) {
	...
	if ((bool)(mask & BIT21) != (bool)(mask & BIT22)) mask ^= SWAPBITS;
