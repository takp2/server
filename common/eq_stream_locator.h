...
class EQStreamInfo {
   public:
	EQStreamInfo() {}
	EQStreamInfo(uint32 isrc_ip, uint32 idst_ip, uint16 isrc_port,
	             uint16 idst_port) {
		src_ip = isrc_ip;
		dst_ip = idst_ip;
		src_port = isrc_port;
...
inline bool operator<(const EQStreamInfo &l, const EQStreamInfo &r) {
	...
	printf("0x%.8x:%d -> 0x%.8x:%d < \n", l.src_ip, l.src_port, l.dst_ip,
	l.dst_port); printf("0x%.8x:%d -> 0x%.8x:%d ", r.src_ip, r.src_port,
	r.dst_ip, r.dst_port);
	...
	if (l.src_ip != r.src_ip) return (l.src_ip < r.src_ip);
	if (l.dst_ip != r.dst_ip) return (l.dst_ip < r.dst_ip);
...
inline bool operator==(const EQStreamInfo &l, const EQStreamInfo &r) {
	...
	//		//maybe swapped
	//		return(l.src_port == r.dst_port && l.dst_ip == r.src_ip && l.dst_port
	//== r.src_port);
	//	}
	return (l.src_ip == r.src_ip && l.src_port == r.src_port &&
	        l.dst_ip == r.dst_ip && l.dst_port == r.dst_port);
}
...
class EQStreamLocator {
   ...
   public:
	...
	void RemoveStream(const EQStreamInfo &i) {
		iterator res;
		res = streams.find(i);
		if (res != streams.end()) streams.erase(res);
	...
	T *GetStream(const EQStreamInfo &i) {
		...
		if (res == streams.end()) return (nullptr);
		return (res->second);
	}
