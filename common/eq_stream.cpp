// Code from https://github.com/EQMacEmu/Server/blob/main/common/eq_stream.cpp
// Licnese GPL2 with Copyright (C) 2005 Michael S. Finger
...
void EQStream::init(bool resetSession) {
	...
	if (GetExecutablePlatform() == ExePlatformWorld ||
	    GetExecutablePlatform() == ExePlatformZone ||
	    GetExecutablePlatform() == ExePlatformUCS) {
	...
	if (uint16(SequencedBase + SequencedQueue.size()) != NextOutSeq) {
		Log(Logs::Detail, Logs::Netcode,
		    _L "init Invalid Sequenced queue: BS %d + SQ %d != NOS %d" __L,
	...
	if (NextSequencedSend > (SequencedBase + SequencedQueue.size())) {
		Log(Logs::Detail, Logs::Netcode,
		    _L
		    "init Next Send Sequence is beyond the end of the queue NSS %d > "
		    "SQ %d" __L,
		    NextSequencedSend, SequencedQueue.size());
	}
}
...
EQRawApplicationPacket *EQStream::MakeApplicationPacket(EQProtocolPacket *p) {
	...
	Log(Logs::Detail, Logs::Netcode,
	    _L "Creating new application packet, length %d" __L, p->size);
	// _raw(NET__APP_CREATE_HEX, 0xFFFF, p);
	ap = p->MakeAppPacket();
	return ap;
}

EQRawApplicationPacket *EQStream::MakeApplicationPacket(
    const unsigned char *buf, uint32 len) {
	EQRawApplicationPacket *ap = nullptr;
	Log(Logs::Detail, Logs::Netcode,
	    _L "Creating new application packet, length %d" __L, len);
	ap = new EQRawApplicationPacket(buf, len);
	return ap;
}

EQProtocolPacket *EQStream::MakeProtocolPacket(const unsigned char *buf,
                                               uint32 len) {
...
void EQStream::ProcessPacket(EQProtocolPacket *p) {
	...
	if (p->opcode > 0xff) {
		p->opcode =
		    htons(p->opcode);  // byte order is backwards in the protocol packet
		EQRawApplicationPacket *ap = MakeApplicationPacket(p);
		if (ap) InboundQueuePush(ap);
		return;
	}

	if (!Session && p->opcode != OP_SessionRequest &&
	    p->opcode != OP_SessionResponse) {
		Log(Logs::Detail, Logs::Netcode,
		    _L "Session not initialized, packet ignored" __L);
	...
	switch (p->opcode) {
		case OP_Combined: {
			...
			while (processed < p->size) {
				...
				EQProtocolPacket *subp = MakeProtocolPacket(
				    p->pBuffer + processed + 1, subpacket_length);
				Log(Logs::Detail, Logs::Netcode,
				    _L "Extracting combined packet of length %d" __L,
				    subpacket_length);
		...
		case OP_AppCombined: {
			...
			while (processed < p->size) {
				...
				if ((subpacket_length =
					...
					Log(Logs::Detail, Logs::Netcode,
					    _L
					    "Extracting combined app packet of length %d, short "
					    "len" __L,
					    subpacket_length);
					ap = MakeApplicationPacket(p->pBuffer + processed + 1,
					                           subpacket_length);
					processed += subpacket_length + 1;
				} else {
					subpacket_length =
					    ntohs(*(uint16 *)(p->pBuffer + processed + 1));
					Log(Logs::Detail, Logs::Netcode,
					    _L
					    "Extracting combined app packet of length %d, short "
					    "len" __L,
					    subpacket_length);
					ap = MakeApplicationPacket(p->pBuffer + processed + 3,
					                           subpacket_length);
		...
		case OP_Packet: {
			if (!p->pBuffer || (p->Size() < 4)) {
				Log(Logs::Detail, Logs::Netcode,
				    _L "Received OP_Packet that was of malformed size" __L);
				break;
			}
			...
			if (check == SeqFuture) {
				Log(Logs::Detail, Logs::Netcode,
				    _L "Future OP_Packet: Expecting Seq=%d, but got Seq=%d" __L,
				    NextInSeq, seq);
				// _raw(NET__DEBUG, seq, p);

				PacketQueue[seq] = p->Copy();
				Log(Logs::Detail, Logs::Netcode,
				    _L "OP_Packet Queue size=%d" __L, PacketQueue.size());
				...
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Duplicate OP_Packet: Expecting Seq=%d, but got Seq=%d" __L,
				    NextInSeq, seq);
				// _raw(NET__DEBUG, seq, p);
				SendOutOfOrderAck(
				    seq);  // we already got this packet but it was out of order
			} else {
				// In case we did queue one before as well.
				EQProtocolPacket *qp = RemoveQueue(seq);
				if (qp) {
					Log(Logs::General, Logs::Netcode,
					    "[NET_TRACE] OP_Packet: Removing older queued packet "
					    "with sequence %d",
					    seq);
					delete qp;
				}
				...
				if (*(p->pBuffer + 2) == 0x00 && *(p->pBuffer + 3) == 0x19) {
					EQProtocolPacket *subp =
					    MakeProtocolPacket(p->pBuffer + 2, p->size - 2);
					Log(Logs::Detail, Logs::Netcode,
					    _L
					    "seq %d, Extracting combined packet of length %d" __L,
					    seq, subp->size);
					...
					EQRawApplicationPacket *ap =
					    MakeApplicationPacket(p->pBuffer + 2, p->size - 2);
					if (ap) {
						ap->copyInfo(p);
						InboundQueuePush(ap);
					}
		...
		case OP_Fragment: {
			if (!p->pBuffer || (p->Size() < 4)) {
				Log(Logs::Detail, Logs::Netcode,
				    _L "Received OP_Fragment that was of malformed size" __L);
				break;
			}
			uint16 seq = ntohs(*(uint16 *)(p->pBuffer));
			SeqOrder check = CompareSequence(NextInSeq, seq);
			if (check == SeqFuture) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Future OP_Fragment: Expecting Seq=%d, but got Seq=%d" __L,
				    NextInSeq, seq);
				// _raw(NET__DEBUG, seq, p);

				PacketQueue[seq] = p->Copy();
				Log(Logs::Detail, Logs::Netcode,
				    _L "OP_Fragment Queue size=%d" __L, PacketQueue.size());
				...
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Duplicate OP_Fragment: Expecting Seq=%d, but got "
				    "Seq=%d" __L,
				    NextInSeq, seq);
				// _raw(NET__DEBUG, seq, p);
				SendOutOfOrderAck(seq);
				...
				if (qp) {
					Log(Logs::General, Logs::Netcode,
					    "[NET_TRACE] OP_Fragment: Removing older queued packet "
					    "with sequence %d",
					    seq);
					delete qp;
				}
				...
				NextInSeq++;
				if (oversize_buffer) {
					memcpy(oversize_buffer + oversize_offset, p->pBuffer + 2,
					       p->size - 2);
					oversize_offset += p->size - 2;
					Log(Logs::Detail, Logs::Netcode,
					    _L
					    "Fragment of oversized of length %d, seq %d: now at "
					    "%d/%d" __L,
					    p->size - 2, seq, oversize_offset, oversize_length);
					if (oversize_offset == oversize_length) {
						if (*(p->pBuffer + 2) == 0x00 &&
						    *(p->pBuffer + 3) == 0x19) {
							EQProtocolPacket *subp = MakeProtocolPacket(
							    oversize_buffer, oversize_offset);
							Log(Logs::Detail, Logs::Netcode,
							    _L
							    "seq %d, Extracting combined oversize packet "
							    "of length %d" __L,
							    seq, subp->size);
							...
							EQRawApplicationPacket *ap = MakeApplicationPacket(
							    oversize_buffer, oversize_offset);
							Log(Logs::Detail, Logs::Netcode,
							    _L
							    "seq %d, completed combined oversize packet of "
							    "length %d" __L,
							    seq, ap->size);
							if (ap) {
								ap->copyInfo(p);
					...
					Log(Logs::Detail, Logs::Netcode,
					    _L
					    "First fragment of oversized of seq %d: now at "
					    "%d/%d" __L,
					    seq, oversize_offset, oversize_length);
				}
			}
		} break;
		case OP_KeepAlive: {
			NonSequencedPush(
			    new EQProtocolPacket(p->opcode, p->pBuffer, p->size));
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Received and queued reply to keep alive Opcode: %d Size: "
			    "%d" __L,
			    p->opcode, p->size);
		} break;
		case OP_Ack: {
			if (!p->pBuffer || (p->Size() < 4)) {
				Log(Logs::Detail, Logs::Netcode,
				    _L "Received OP_Ack that was of malformed size" __L);
				break;
			}
			uint16 seq = ntohs(*(uint16 *)(p->pBuffer));
			AckPackets(seq);
		...
		case OP_SessionRequest: {
			if (p->Size() < sizeof(SessionRequest)) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Received OP_SessionRequest that was of malformed "
				    "size" __L);
				break;
			}
			if (GetState() == ESTABLISHED) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Received OP_SessionRequest in ESTABLISHED state (%d) "
				    "streamactive (%i) attempt (%i)" __L,
				    GetState(), streamactive, sessionAttempts);

				// client seems to try a max of 30 times (initial+3 retries)
				// then gives up, giving it a few more attempts just in case
				// streamactive means we identified the opcode for the stream,
				// we cannot re-establish this connection
			...
			sessionAttempts++;
			// we set established below, so statistics will not be reset for
			// session attempts/stream active.
			...
			Log(Logs::Detail, Logs::Netcode,
			    _L "Received OP_SessionRequest: session %lu, maxlen %d" __L,
			    (unsigned long)Session, MaxLen);
		...
		case OP_SessionResponse: {
			if (p->Size() < sizeof(SessionResponse)) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Received OP_SessionResponse that was of malformed "
				    "size" __L);
				break;
			}
			...
			NextInSeq = 0;
			SetState(ESTABLISHED);
			if (!Session) Session = ntohl(Response->Session);
			compressed = (Response->Format & FLAG_COMPRESSED);
			encoded = (Response->Format & FLAG_ENCODED);

			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Received OP_SessionResponse: session %lu, maxlen %d, key %lu, "
			    "compressed? %s, encoded? %s" __L,
			    (unsigned long)Session, MaxLen, (unsigned long)Key,
			    compressed ? "yes" : "no", encoded ? "yes" : "no");

			// Kinda kludgy, but trie for now
			if (StreamType == UnknownStream) {
				if (compressed) {
					if (remote_port == 9000 ||
					    (remote_port == 0 && p->src_port == 9000)) {
		...
		case OP_SessionDisconnect: {
			...
			if (state == ESTABLISHED) {
				...
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Received unsolicited OP_SessionDisconnect. Treating like "
				    "a client-initiated disconnect." __L);
				_SendDisconnect();
				SetState(CLOSED);
			} else if (state == CLOSING) {
				// we were waiting for this anyways, ignore pending messages,
				// send the reply and be closed.
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Received OP_SessionDisconnect when we have a pending "
				    "close, they beat us to it. Were happy though." __L);
				_SendDisconnect();
				SetState(CLOSED);
			} else {
				// we are expecting this (or have already gotten it, but dont
				// care either way)
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Received expected OP_SessionDisconnect. Moving to closed "
				    "state." __L);
				SetState(CLOSED);
			}
		} break;
		case OP_OutOfOrderAck: {
			if (!p->pBuffer || (p->Size() < 4)) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Received OP_OutOfOrderAck that was of malformed size" __L);
				break;
			}
			uint16 seq = ntohs(*(uint16 *)(p->pBuffer));
			MOutboundQueue.lock();
			...
			if (uint16(SequencedBase + SequencedQueue.size()) != NextOutSeq) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Pre-OOA Invalid Sequenced queue: BS %d + SQ %d != NOS "
				    "%d" __L,
				    SequencedBase, SequencedQueue.size(), NextOutSeq);
			}
			...
			if (NextSequencedSend > (SequencedBase + SequencedQueue.size())) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Pre-OOA Next Send Sequence is beyond the end of the queue "
				    "NSS %d > SQ %d" __L,
				    NextSequencedSend, SequencedQueue.size());
			}
			// if the packet they got out of order is between our last acked
			// packet and the last sent packet, then its valid.
			if (CompareSequence(SequencedBase, seq) != SeqPast &&
			    CompareSequence(NextOutSeq, seq) == SeqPast) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Received OP_OutOfOrderAck for sequence %d, starting "
				    "retransmit at the start of our unacked buffer (seq %d, "
				    "was %d)." __L,
				...
				if (!retransmit_acked_packets) {
					...
					Log(Logs::Detail, Logs::Netcode,
					    _L
					    "OP_OutOfOrderAck marking packet acked in queue (queue "
					    "index = %d, queue size = %d)." __L,
					    index, sqsize);
					if (index < sqsize) {
				...
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Received OP_OutOfOrderAck for out-of-window %d. Window "
				    "(%d->%d)." __L,
			...
			if (uint16(SequencedBase + SequencedQueue.size()) != NextOutSeq) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Post-OOA Invalid Sequenced queue: BS %d + SQ %d != NOS "
				    "%d" __L,
				    SequencedBase, SequencedQueue.size(), NextOutSeq);
			}
			...
			if (NextSequencedSend > (SequencedBase + SequencedQueue.size())) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Post-OOA Next Send Sequence is beyond the end of the "
				    "queue NSS %d > SQ %d" __L,
				    NextSequencedSend, SequencedQueue.size());
			}
		...
		case OP_SessionStatRequest: {
			if (p->Size() < sizeof(SessionStats)) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Received OP_SessionStatRequest that was of malformed "
				    "size" __L);
				break;
			}
			...
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Received Stats: %lu packets received, %lu packets sent, "
			    "Deltas: local %lu, (%lu <- %lu -> %lu) remote %lu" __L,
			    (unsigned long)ntohl(Stats->packets_received),
			    (unsigned long)ntohl(Stats->packets_sent),
			    (unsigned long)ntohl(Stats->last_local_delta),
			    (unsigned long)ntohl(Stats->low_delta),
			    (unsigned long)ntohl(Stats->average_delta),
			    (unsigned long)ntohl(Stats->high_delta),
			    (unsigned long)ntohl(Stats->last_remote_delta));
			uint64 x = Stats->packets_received;
			Stats->packets_received = Stats->packets_sent;
			Stats->packets_sent = x;
			NonSequencedPush(new EQProtocolPacket(OP_SessionStatResponse,
			                                      p->pBuffer, p->size));
			...
			if (GetExecutablePlatform() == ExePlatformWorld ||
				...
				if (RETRANSMIT_TIMEOUT_MULT && ntohl(Stats->average_delta)) {
					// recalculate retransmittimeout using the larger of the
					// last rtt or average rtt, which is multiplied by the rule
					// value
					if ((ntohl(Stats->last_local_delta) +
					     ntohl(Stats->last_remote_delta)) >
					    (ntohl(Stats->average_delta) * 2)) {
						retransmittimeout = (ntohl(Stats->last_local_delta) +
						                     ntohl(Stats->last_remote_delta)) *
						                    RETRANSMIT_TIMEOUT_MULT;
					} else {
						retransmittimeout = ntohl(Stats->average_delta) * 2 *
						                    RETRANSMIT_TIMEOUT_MULT;
					}
					if (retransmittimeout > RETRANSMIT_TIMEOUT_MAX)
						retransmittimeout = RETRANSMIT_TIMEOUT_MAX;
					Log(Logs::Detail, Logs::Netcode,
					    _L "Retransmit timeout recalculated to %dms" __L,
					    retransmittimeout);
				}
			}
		} break;
		case OP_SessionStatResponse: {
			Log(Logs::Detail, Logs::Netcode,
			    _L "Received OP_SessionStatResponse. Ignoring." __L);
		} break;
		case OP_OutOfSession: {
			Log(Logs::Detail, Logs::Netcode,
			    _L "Received OP_OutOfSession. Ignoring." __L);
		} break;
		default:
			EQRawApplicationPacket *ap = MakeApplicationPacket(p);
			if (ap) InboundQueuePush(ap);
...
void EQStream::QueuePacket(const EQApplicationPacket *p, bool ack_req) {
	...
	if (newp != nullptr) FastQueuePacket(&newp, ack_req);
}
...
void EQStream::FastQueuePacket(EQApplicationPacket **p, bool ack_req) {
	...
	*p = nullptr;  // clear caller's pointer.. effectively takes ownership
	...
	if (OpMgr == nullptr || *OpMgr == nullptr) {
		Log(Logs::Detail, Logs::Netcode,
		    _L
		    "Packet enqueued into a stream with no opcode manager, "
		    "dropping." __L);
		delete pack;
		return;
	}
	...
	if (!ack_req) {
		NonSequencedPush(
		    new EQProtocolPacket(opcode, pack->pBuffer, pack->size));
		delete pack;
	} else {
...
void EQStream::SendPacket(uint16 opcode, EQApplicationPacket *p) {
	...
	if (LogSys.log_settings[Logs::PacketServerClient].is_category_enabled ==
	    1) {
		if (p->GetOpcode() != OP_SpecialMesg) {
			Log(Logs::General, Logs::PacketServerClient,
			    "[%s - 0x%04x] [Size: %u]",
			    OpcodeManager::EmuToName(p->GetOpcode()), p->GetOpcode(),
			    p->Size());
		}
	}

	if (LogSys.log_settings[Logs::PacketServerClientWithDump]
	        .is_category_enabled == 1) {
		if (p->GetOpcode() != OP_SpecialMesg) {
			Log(Logs::General, Logs::PacketServerClientWithDump,
			    "[%s - 0x%04x] [Size: %u] %s",
			    OpcodeManager::EmuToName(p->GetOpcode()), p->GetOpcode(),
			    p->Size(), DumpPacketToString(p).c_str());
		}
	}

	// Convert the EQApplicationPacket to 1 or more EQProtocolPackets
	if (p->size >
	    (MaxLen - 8)) {  // proto-op(2), seq(2), app-op(2) ... data ... crc(2)
		...
		if (length != p->Size())
			LogNetcode(_L "Packet adjustment, len [{0}] to [{1}]" __L,
			           p->Size(), length);
		...
		*(uint32 *)(out->pBuffer + 2) = htonl(length);
		...
		LogNetcode(_L
		           "First fragment: used [{0}]/[{1}]. Payload size [{2}] in "
		           "the packet" __L,
		           used, length, p->size);
		SequencedPush(out);
		...
		while (used < length) {
			...
			used += chunksize;
			LogNetcode(_L
			           "Subsequent fragment: len [{0}], used [{1}]/[{2}]." __L,
			           chunksize, used, length);
		}
		delete p;
		delete[] tmpbuff;
	} else {
...
void EQStream::SequencedPush(EQProtocolPacket *p) {
	...
	if (uint16(SequencedBase + SequencedQueue.size()) != NextOutSeq) {
		LogNetcode(_L
		           "Pre-Push Invalid Sequenced queue: BS [{0}] + SQ [{1}] != "
		           "NOS [{2}]" __L,
		           SequencedBase, SequencedQueue.size(), NextOutSeq);
	}
	if (NextSequencedSend > (SequencedBase + SequencedQueue.size())) {
		LogNetcode(_L
		           "Pre-Push Next Send Sequence is beyond the end of the queue "
		           "NSS [{0}] > SQ [{1}]" __L,
		           NextSequencedSend, SequencedQueue.size());
	}

	LogNetcode(_L
	           "Pushing sequenced packet [{0}] of length [{1}]. Base Seq is "
	           "[{2}]." __L,
	...
	*(uint16 *)(p->pBuffer) = htons(NextOutSeq);
	...
	if (uint16(SequencedBase + SequencedQueue.size()) != NextOutSeq) {
		LogNetcode(_L
		           "Push Invalid Sequenced queue: BS [{0}] + SQ [{1}] != NOS "
		           "[{2}]" __L,
		           SequencedBase, SequencedQueue.size(), NextOutSeq);
	}
	if (NextSequencedSend > (SequencedBase + SequencedQueue.size())) {
		LogNetcode(_L
		           "Push Next Send Sequence is beyond the end of the queue NSS "
		           "[{0}] > SQ [{1}]" __L,
		           NextSequencedSend, SequencedQueue.size());
	}
	MOutboundQueue.unlock();
...
void EQStream::SendAck(uint16 seq) {
	...
	SetLastAckSent(seq);
	NonSequencedPush(
	    new EQProtocolPacket(OP_Ack, (unsigned char *)&Seq, sizeof(uint16)));
}

void EQStream::SendOutOfOrderAck(uint16 seq) {
	LogNetcodeDetail(_L "Sending out of order ack with sequence [{0}]" __L,
	                 seq);
	uint16 Seq = htons(seq);
	NonSequencedPush(new EQProtocolPacket(
	    OP_OutOfOrderAck, (unsigned char *)&Seq, sizeof(uint16)));
}
...
void EQStream::Write(int eq_fd) {
	...
	if (GetExecutablePlatform() == ExePlatformWorld ||
	    GetExecutablePlatform() == ExePlatformZone || stream_startup) {
		// if we have a timeout defined and we have not received an ack recently
		// enough, retransmit from beginning of queue
		if (RETRANSMIT_TIMEOUT_MULT && !SequencedQueue.empty() &&
		    NextSequencedSend && (GetState() == ESTABLISHED) &&
		    ((retransmittimer + retransmittimeout) > Timer::GetCurrentTime())) {
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Timeout since last ack received, starting retransmit at the "
			    "start of our unacked "
			    "buffer (seq %d, was %d)." __L,
			    SequencedBase, SequencedBase + NextSequencedSend);
			NextSequencedSend = 0;
			retransmittimer =
			    Timer::GetCurrentTime();  // don't want to endlessly retransmit
			                              // the first packet
		}
	}
	...
	sitr = SequencedQueue.begin();
	if (sitr != SequencedQueue.end()) sitr += NextSequencedSend;
	...
	while (!SeqEmpty || !NonSeqEmpty) {
		...
		if (!NonSequencedQueue.empty()) {
			if (!p) {
				// If we don't have a packet to try to combine into, use this
				// one as the base And remove it form the queue
				p = NonSequencedQueue.front();
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Starting combined packet with non-seq packet of len "
				    "%d" __L,
				    p->size);
				NonSequencedQueue.pop();
			} else if (!p->combine(NonSequencedQueue.front())) {
				// Tryint to combine this packet with the base didn't work (too
				// big maybe) So just send the base packet (we'll try this
				// packet again later)
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Combined packet full at len %d, next non-seq packet is "
				    "len %d" __L,
				    p->size, (NonSequencedQueue.front())->size);
				ReadyToSend.push(p);
				BytesWritten += p->size;
				p = nullptr;
				...
				if (BytesWritten > threshold) {
					...
					Log(Logs::Detail, Logs::Netcode,
					    _L "Exceeded write threshold in nonseq (%d > %d)" __L,
					    BytesWritten, threshold);
					break;
				}
			} else {
				// Combine worked, so just remove this packet and it's spot in
				// the queue
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Combined non-seq packet of len %d, yeilding %d "
				    "combined." __L,
				    (NonSequencedQueue.front())->size, p->size);
		...
		if (sitr != SequencedQueue.end()) {
			if (uint16(SequencedBase + SequencedQueue.size()) != NextOutSeq) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Pre-Send Seq NSS=%d Invalid Sequenced queue: BS %d + SQ "
				    "%d != NOS %d" __L,
				    NextSequencedSend, SequencedBase, SequencedQueue.size(),
				    NextOutSeq);
			}
			...
			if (NextSequencedSend > (SequencedBase + SequencedQueue.size())) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Pre-Send Next Send Sequence is beyond the end of the "
				    "queue NSS %d > SQ %d" __L,
				    NextSequencedSend, SequencedQueue.size());
			}
			uint16 seq_send =
			    SequencedBase + NextSequencedSend;  // just for logging...

			if (SequencedQueue.empty()) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Tried to write a packet with an empty queue (%d is past "
				    "next out %d)" __L,
				    seq_send, NextOutSeq);
				SeqEmpty = true;
				continue;
			...
			if (GetExecutablePlatform() == ExePlatformWorld ||
				...
				if (!RETRANSMIT_ACKED_PACKETS && (*sitr)->acked) {
					Log(Logs::Detail, Logs::Netcode,
					    _L
					    "Not retransmitting seq packet %d because already "
					    "marked as acked" __L,
					    seq_send);
					sitr++;
					NextSequencedSend++;
				} else if (!p) {
					// If we don't have a packet to try to combine into, use
					// this one as the base Copy it first as it will still live
					// until it is acked
					p = (*sitr)->Copy();
					Log(Logs::Detail, Logs::Netcode,
					    _L
					    "Starting combined packet with seq packet %d of len "
					    "%d" __L,
					    seq_send, p->size);
					++sitr;
					NextSequencedSend++;
				} else if (!p->combine(*sitr)) {
					// Trying to combine this packet with the base didn't work
					// (too big maybe) So just send the base packet (we'll try
					// this packet again later)
					Log(Logs::Detail, Logs::Netcode,
					    _L
					    "Combined packet full at len %d, next seq packet %d is "
					    "len %d" __L,
					    p->size, seq_send, (*sitr)->size);
					ReadyToSend.push(p);
					BytesWritten += p->size;
					p = nullptr;
					...
					if (BytesWritten > threshold) {
						// Sent enough this round, lets stop to be fair
						Log(Logs::Detail, Logs::Netcode,
						    _L "Exceeded write threshold in seq (%d > %d)" __L,
						    BytesWritten, threshold);
						break;
					}
				} else {
					...
					Log(Logs::Detail, Logs::Netcode,
					    _L
					    "Combined seq packet %d of len %d, yielding %d "
					    "combined." __L,
					    seq_send, (*sitr)->size, p->size);
					++sitr;
					NextSequencedSend++;
				}
			} else {
				if (!p) {
					...
					if (p != nullptr) {
						Log(Logs::Detail, Logs::Netcode,
						    _L
						    "Starting combined packet with seq packet %d of "
						    "len %d" __L,
						    seq_send, p->size);
					} else {
						Log(Logs::Detail, Logs::Netcode,
						    _L
						    "Starting combined packet with seq packet %d" __L,
						    seq_send);
					}
					...
					Log(Logs::Detail, Logs::Netcode,
					    _L
					    "Combined seq packet %d of len %d, yeilding %d "
					    "combined." __L,
					    seq_send, (*sitr)->size, p->size);
					++sitr;
					NextSequencedSend++;
			...
			if (uint16(SequencedBase + SequencedQueue.size()) != NextOutSeq) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Post send Invalid Sequenced queue: BS %d + SQ %d != NOS "
				    "%d" __L,
				    SequencedBase, SequencedQueue.size(), NextOutSeq);
			}
			if (NextSequencedSend > (SequencedBase + SequencedQueue.size())) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Post send Next Send Sequence is beyond the end of the "
				    "queue NSS %d > SQ %d" __L,
				    NextSequencedSend, SequencedQueue.size());
			}
		} else {
	...
	// We have a packet still, must have run out of both seq and non-seq, so
	// send it
	if (p) {
		Log(Logs::Detail, Logs::Netcode,
		    _L "Final combined packet not full, len %d" __L, p->size);
		ReadyToSend.push(p);
		BytesWritten += p->size;
	}
	...
	if (SeqEmpty && NonSeqEmpty) {
		...
		if (CheckState(CLOSING)) {
			Log(Logs::Detail, Logs::Netcode,
			    _L "All outgoing data flushed, closing stream." __L);
			// we are waiting for the queues to empty, now we can do our
			// disconnect. this packet will not actually go out until the next
			// call to Write().
			_SendDisconnect();
...
void EQStream::WritePacket(int eq_fd, EQProtocolPacket *p) {
	...
	std::cout << "Sending to: " << (int)*(unsigned char *)&ip << "."
	          << (int)*((unsigned char *)&ip + 1) << "."
	          << (int)*((unsigned char *)&ip + 2) << "."
	          << (int)*((unsigned char *)&ip + 3) << ","
	          << (int)ntohs(address.sin_port) << "(" << p->size << ")"
	          << std::endl;
	...
	if (p->opcode != OP_SessionRequest && p->opcode != OP_SessionResponse) {
		if (compressed) {
			uint32 newlen =
			    EQProtocolPacket::Compress(buffer, length, _tempBuffer, 2048);
	...
	sendto(eq_fd, (char *)buffer, length, 0, (sockaddr *)&address,
	       sizeof(address));
	AddBytesSent(length);
}

void EQStream::SendSessionResponse() {
	auto out = new EQProtocolPacket(OP_SessionResponse, nullptr,
	...
	Response->UnknownA = 2;
	Response->Format = 0;
	if (compressed) Response->Format |= FLAG_COMPRESSED;
	if (encoded) Response->Format |= FLAG_ENCODED;
	...
	Log(Logs::Detail, Logs::Netcode,
	    _L
	    "Sending OP_SessionResponse: session %lu, maxlen=%d, key=0x%x, "
	    "compressed? %s, encoded? %s" __L,
	    (unsigned long)Session, MaxLen, Key, compressed ? "yes" : "no",
	    encoded ? "yes" : "no");
...
void EQStream::SendSessionRequest() {
	auto out = new EQProtocolPacket(OP_SessionRequest, nullptr,
	...
	Log(Logs::Detail, Logs::Netcode,
	    _L "Sending OP_SessionRequest: session %lu, maxlen=%d" __L,
	    (unsigned long)ntohl(Request->Session), ntohl(Request->MaxLength));
...
void EQStream::SendKeepAlive() {
	...
	Log(Logs::Detail, Logs::Netcode, _L "Sending OP_KeepAlive: session %lu" __L,
...
void EQStream::_SendDisconnect() {
	...
	auto out =
	    new EQProtocolPacket(OP_SessionDisconnect, nullptr, sizeof(uint32));
	*(uint32 *)out->pBuffer = htonl(Session);
	NonSequencedPush(out);

	Log(Logs::Detail, Logs::Netcode,
	    _L "Sending OP_SessionDisconnect: session %lu" __L,
	    (unsigned long)Session);
}
	...
	if (p) {
		if (OpMgr != nullptr && *OpMgr != nullptr) {
			...
			if (emu_op == OP_Unknown) {
				// Log(Logs::General, Logs::Client_Server_Packet_Unhandled,
				// "Unknown :: [%s - 0x%04x] [Size: %u] %s",
				// OpcodeManager::EmuToName(p->GetOpcode()), p->opcode,
				// p->Size(), DumpPacketToString(p).c_str());
			}
			p->SetOpcode(emu_op);
		}
...
EQRawApplicationPacket *EQStream::PopRawPacket() {
	...
	if (p) {
		if (OpMgr != nullptr && *OpMgr != nullptr) {
			...
			if (emu_op == OP_Unknown) {
				Log(Logs::General, Logs::Netcode,
				    "Unable to convert EQ opcode 0x%.4x to an Application "
				    "opcode.",
				    p->opcode);
			}
...
bool EQStream::HasOutgoingData() {
	...
	if (!flag) {
		// not only wait until we send it all, but wait until they ack
		// everything.
...
void EQStream::Process(const unsigned char *buffer, const uint32 length) {
	...
	if (EQProtocolPacket::ValidateCRC(buffer, length, Key)) {
		if (compressed) {
			newlength =
			    EQProtocolPacket::Decompress(buffer, length, newbuffer, 2048);
		} else {
		...
		Log(Logs::Detail, Logs::Netcode,
		    _L "Incoming packet failed checksum" __L);
		//_SendDisconnect();
		// SetState(CLOSED);
	}
}
...
void EQStream::AckPackets(uint16 seq) {
	...
	if (uint16(SequencedBase + SequencedQueue.size()) != NextOutSeq) {
		Log(Logs::Detail, Logs::Netcode,
		    _L "Pre-Ack Invalid Sequenced queue: BS %d + SQ %d != NOS %d" __L,
		    SequencedBase, SequencedQueue.size(), NextOutSeq);
	}
	if (NextSequencedSend > (SequencedBase + SequencedQueue.size())) {
		Log(Logs::Detail, Logs::Netcode,
		    _L
		    "Pre-Ack Next Send Sequence is beyond the end of the queue NSS %d "
		    "> SQ %d" __L,
		    NextSequencedSend, SequencedQueue.size());
	}
	...
	if (ord == SeqInOrder) {
		...
		Log(Logs::Detail, Logs::Netcode,
		    _L "Received an ack with no window advancement (seq %d)." __L, seq);
	} else if (ord == SeqPast) {
		// they are nacking blocks going back before our buffer, wtf?
		Log(Logs::Detail, Logs::Netcode,
		    _L
		    "Received an ack with backward window advancement (they gave %d, "
		    "our window starts at %d). This is bad." __L,
		    seq, SequencedBase);
	} else {
		Log(Logs::Detail, Logs::Netcode,
		    _L "Received an ack up through sequence %d. Our base is %d." __L,
		    seq, SequencedBase);

		// this is a good ack, we get to ack some blocks.
		seq++;  // we stop at the block right after their ack, counting on the
		        // wrap of both numbers.
		while (SequencedBase != seq) {
			if (SequencedQueue.empty()) {
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "OUT OF PACKETS acked packet with sequence %lu. Next send "
				    "is %d before this." __L,
				    (unsigned long)SequencedBase, NextSequencedSend);
			...
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Removing acked packet with sequence %lu. Next send is %d "
			    "before this." __L,
			    (unsigned long)SequencedBase, NextSequencedSend);
			...
			if (NextSequencedSend > 0) NextSequencedSend--;
			// advance the base sequence number to the seq of the block after
		...
		if (uint16(SequencedBase + SequencedQueue.size()) != NextOutSeq) {
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Post-Ack on %d Invalid Sequenced queue: BS %d + SQ %d != NOS "
			    "%d" __L,
		...
		if (NextSequencedSend > (SequencedBase + SequencedQueue.size())) {
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Post-Ack Next Send Sequence is beyond the end of the queue "
			    "NSS %d > SQ %d" __L,
			    NextSequencedSend, SequencedQueue.size());
		}
	}
...
void EQStream::SetNextAckToSend(uint32 seq) {
	MAcks.lock();
	Log(Logs::Detail, Logs::Netcode, _L "Set Next Ack To Send to %lu" __L,
	    (unsigned long)seq);
	NextAckToSend = seq;
	MAcks.unlock();
}
...
void EQStream::SetLastAckSent(uint32 seq) {
	MAcks.lock();
	Log(Logs::Detail, Logs::Netcode, _L "Set Last Ack Sent to %lu" __L,
	    (unsigned long)seq);
	LastAckSent = seq;
	MAcks.unlock();
}
...
void EQStream::ProcessQueue() {
	...
	while ((qp = RemoveQueue(NextInSeq)) != nullptr) {
		Log(Logs::Detail, Logs::Netcode,
		    _L "Processing Queued Packet: Seq=%d" __L, NextInSeq);
		ProcessPacket(qp);
		delete qp;
		Log(Logs::Detail, Logs::Netcode, _L "OP_Packet Queue size=%d" __L,
		    PacketQueue.size());
	}
}
...
void EQStream::SetStreamType(EQStreamType type) {
	Log(Logs::Detail, Logs::Netcode,
	    _L "Changing stream type from %s to %s" __L,
	    StreamTypeString(StreamType), StreamTypeString(type));
	StreamType = type;
	switch (StreamType) {
		case LoginStream:
			app_opcode_size = 1;
			...
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Login stream has app opcode size %d, is not compressed or "
			    "encoded." __L,
			    app_opcode_size);
			break;
		...
		case MailStream:
			...
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Chat/Mail stream has app opcode size %d, is not compressed, "
			    "and is encoded." __L,
			    app_opcode_size);
			break;
		case ZoneStream:
		...
		default:
			...
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "World/Zone stream has app opcode size %d, is compressed, and "
			    "is not encoded." __L,
			    app_opcode_size);
			break;
	}
...
EQStream::SeqOrder EQStream::CompareSequence(uint16 expected_seq, uint16 seq) {
	if (expected_seq == seq) {
		...
		return SeqInOrder;
	            ...
	            (uint32)seq <
	                ((uint32)expected_seq + EQStream::MaxWindowSize)) ||
...
void EQStream::SetState(EQStreamState state) {
	MState.lock();
	Log(Logs::Detail, Logs::Netcode, _L "Changing state from %d to %d" __L,
	    State, state);
	State = state;
	stale_count = 0;
	MState.unlock();
...
void EQStream::CheckTimeout(uint32 now, uint32 timeout) {
	...
	bool outgoing_data =
	    HasOutgoingData();  // up here to avoid recursive locking

	if (orig_state == CLOSING && !outgoing_data) {
		Log(Logs::Detail, Logs::Netcode,
		    _L "Out of data in closing state, disconnecting." __L);
		SetState(CLOSED);
	} else if (LastPacket && (now - LastPacket) > timeout) {
		switch (orig_state) {
			case CLOSING:
				// if we time out in the closing state, they are not acking us,
				// just give up
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Timeout expired in closing state. Moving to closed "
				    "state." __L);
				_SendDisconnect();
				SetState(CLOSED);
				break;
			case DISCONNECTING:
				// we timed out waiting for them to send us the disconnect
				// reply, just give up.
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Timeout expired in disconnecting state. Moving to closed "
				    "state." __L);
				SetState(CLOSED);
				break;
			case CLOSED:
				...
				if (stale_count > 1) {
					...
					Log(Logs::Detail, Logs::Netcode,
					    _L "Timeout expired in closed state??" __L);
				}
				break;
			case ESTABLISHED:
				// we timed out during normal operation. Try to be nice about
				// it. we will almost certainly time out again waiting for the
				// disconnect reply, but oh well.
				Log(Logs::Detail, Logs::Netcode,
				    _L
				    "Timeout expired in established state. Closing "
				    "connection." __L);
				_SendDisconnect();
				SetState(DISCONNECTING);
				break;
	...
	} else if (orig_state == ESTABLISHED &&
	           (!LastSent || ((now - LastSent) > 9800))) {
		// send a keepalive
		SendKeepAlive();
	}
}
...
void EQStream::AdjustRates(uint32 average_delta) {
	if (GetExecutablePlatform() == ExePlatformWorld ||
		...
		if (average_delta && (average_delta <= AVERAGE_DELTA_MAX)) {
			...
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Adjusting data rate to thresh %d, decay %d based on avg delta "
			    "%d" __L,
			...
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Not adjusting data rate because avg delta over max (%d > "
			    "%d)" __L,
...
void EQStream::Close() {
	...
	if (HasOutgoingData()) {
		...
		Log(Logs::Detail, Logs::Netcode,
		    _L
		    "Stream requested to Close(), but there is pending data, waiting "
		    "for it." __L);
		SetState(CLOSING);
	} else {
		...
		_SendDisconnect();
		Log(Logs::Detail, Logs::Netcode,
		    _L "Stream closing immediate due to Close()" __L);
		SetState(DISCONNECTING);
	}
}
	...
	if (!InboundQueue.empty()) {
		...
		if (p == nullptr) {
			...
			if (p->size == sig->first_length) {
				Log(Logs::General, Logs::Netcode,
				    "[IDENT_TRACE] %s:%d: First opcode matched 0x%x and length "
				    "matched %d",
				    long2ip(GetRemoteIP()).c_str(), ntohs(GetRemotePort()),
				    sig->first_eq_opcode, p->size);
				res = MatchSuccessful;
				...
				Log(Logs::General, Logs::Netcode,
				    "[IDENT_TRACE] %s:%d: First opcode matched 0x%x, but "
				    "length %d did not match expected %d",
				    long2ip(GetRemoteIP()).c_str(), ntohs(GetRemotePort()),
				    sig->first_eq_opcode, p->size, sig->first_length);
				res = MatchFailed;
			}
		} else {
			// first opcode did not match..
			Log(Logs::General, Logs::Netcode,
			    "[IDENT_TRACE] %s:%d: First opcode 0x%x did not match expected "
			    "0x%x",
			    long2ip(GetRemoteIP()).c_str(), ntohs(GetRemotePort()),
			    p->opcode, sig->first_eq_opcode);
			res = MatchFailed;
		}
	}
	MInboundQueue.unlock();
...
void Fragment::ClearData() { safe_delete_array(data); }
...
void FragmentGroup::Add(uint16 frag_id, uchar *data, uint32 size) {
	...
	// The frag_id is attempting to reference an element outside the bounds of
	// the group array
	else {
		return;
	}
...
FragmentGroup *FragmentGroupList::Get(uint16 find_seq) {
	...
	while (it != fragment_group_list.end()) {
		if ((*it)->GetSeq() == find_seq) return (*it);
		++it;
	}
	return nullptr;
}
...
void FragmentGroupList::RemoveAll() {
	for (std::vector<FragmentGroup *>::iterator it =
	         fragment_group_list.begin();
	     it != fragment_group_list.end(); ++it) {
		safe_delete(*it);
	}
}
...
EQOldStream::~EQOldStream() {
	...
	Log(Logs::Detail, Logs::Netcode,
	    "Killing outbound and inbound packet queue");
	RemoveData();
	SetState(CLOSED);
}
...
void EQOldStream::ResendBefore(uint16 dwARQ) {
	MOutboundQueue.lock();
	// Log(Logs::Detail, Logs::Netcode, _L "Resend Before Called %d B0." __L,
	// dwARQ);
			...
			if (dwARQ > 5000) {
				if ((*it)->dwARQ <= dwARQ) {
					if (dwARQ > 60000) {
						if ((*it)->dwARQ > 5000) {
							(*it)->Resend = true;
							// Log(Logs::Detail, Logs::Netcode, _L "Flagging %d
							// for resend B2." __L, (*it)->dwARQ);
						}
					} else {
						(*it)->Resend = true;
						// Log(Logs::Detail, Logs::Netcode, _L "Flagging %d for
						// resend B3." __L, (*it)->dwARQ);
					}
				}
			} else {
...
void EQOldStream::ResendRequest(uint16 count_size, const uchar *bits,
                                uint16 arsp_start) {
	if (count_size == 0 || bits == nullptr) return;
	MOutboundQueue.lock();
	int j = 0;
	int i = 0;
	std::deque<EQOldPacket *>::iterator it;
	...
	for (it = SendQueue.begin(); it != SendQueue.end() && i < count_size;) {
		if ((*it)->HDR.a1_ARQ && (*it)->dwARQ == match_arsp) {
			if (bit_fields & 0x80) {
				...
				(*it)->Resend = false;
				(*it)->acked = true;
				// Log(Logs::Detail, Logs::Netcode, _L "Acking packet %d." __L,
				// match_arsp);
			} else {
				(*it)->Resend = true;
				// Log(Logs::Detail, Logs::Netcode, _L "Flagging %d for resend."
				// __L, match_arsp);
			}
			j++;
			match_arsp++;
...
void EQOldStream::IncomingARSP(uint16 dwARSP) {
	...
	// Log(Logs::Detail, Logs::Netcode, _L "ARSP Received %d. ARSP0" __L,
	// dwARSP);
	bool acked;
	std::deque<EQOldPacket *>::iterator it;
	for (it = SendQueue.begin(); it != SendQueue.end();) {
...
void EQOldStream::IncomingARQ(uint16 dwARQ) {
	...
	if (!no_ack_sent_timer->Enabled()) {
		no_ack_sent_timer->Start(
		    250);  // Agz: If we dont get any outgoing packet we can put an
		           // ack response in before 250ms has passed we send a pure ack response
		           // if (debug_level >= 2)
		           //            cout << Timer::GetCurrentTime() << "
		           //            no_ack_sent_timer->Start(200)" << endl;
	}
}
...
void EQOldStream::OutgoingARSP(void) {
	no_ack_sent_timer->Disable();  // Agz: We have sent the ack response
	                               //    if (debug_level >= 2)
	                               //        cout << Timer::GetCurrentTime() << " no_ack_sent_timer->Disable()"
	                               //        << endl;
}

/************ PARCE A EQPACKET ************/
void EQOldStream::ParceEQPacket(uint16 dwSize, uchar *pPacket) {
	if (pm_state != EQStreamState::ESTABLISHED) return;
...
/*
    Return true if its safe to delete this packet now, if we buffer it return
   false this way i can skip memcpy the packet when buffering it
*/
bool EQOldStream::ProcessPacket(EQOldPacket *pack, bool from_buffer) {
	...
	//	Log(Logs::Detail, Logs::Netcode, _L "Dropping incoming packet to
	// simulate packet loss arq %i" __L, pack->dwARQ); 	return true;
	//}
	/************ CHECK FOR ACK/SEQ RESET ************/
	if (pack->HDR.a5_SEQStart) {
		if (dwLastCACK == (unsigned int)pack->dwARQ - (unsigned int)1) {
			Log(Logs::Detail, Logs::Netcode,
			    _L "Packet invalid seqstart? %i:%i" __L, pack->dwARQ,
			    pack->dwSEQ);
			return true;
		}
		...
		Log(Logs::Detail, Logs::Netcode, _L "Packet seqstart %i:%i" __L,
		    pack->dwARQ, pack->dwSEQ);
		//      CACK.dwGSQ = 0xFFFF; changed next if to else instead
	}

	CACK.dwGSQ = pack->dwSEQ;  // Get current sequence #.
	// Log(Logs::Detail, Logs::Netcode, _L "Packet incoming arq%i:seq%i:arsp%i"
	// __L, pack->dwARQ, pack->dwSEQ, pack->dwARSP);

	/************ Process ack responds ************/
	// Quagmire: Moved this to above "ack request" checking in case the packet
	// is dropped in there
	if (pack->HDR.b2_ARSP) {
	...
	if (pack->HDR.a1_ARQ) {
		...
		if (pack->dwARQ != dwnextCACK) {
			...
			if ((pack->dwARQ == dwLastCACK) ||  // this was last ack request
			    (dwLastCACK > 5000 && pack->dwARQ < dwLastCACK &&
			     (dwLastCACK < 60000 ||
			      (dwLastCACK > 60000 && pack->dwARQ > 5000))) ||  // in past
			    (dwLastCACK < 5000 && (pack->dwARQ > 60000 ||
			                           pack->dwARQ < dwLastCACK))) {  // in past
				// Log(Logs::Detail, Logs::Netcode, _L "Packet discarded %i:%i"
				// __L, pack->dwARQ, pack->dwSEQ);
				return true;
			}

			// Debug check, if we want to buffer a packet we got from the buffer
			// something is wrong...
			if (from_buffer) {
				return true;
				// cerr << "ERROR: Rebuffering a packet in
				// EQPacketManager::ProcessPacket" << endl;
			}
			...
			while (iterator != buffered_packets.end()) {
				if ((*iterator)->dwARQ == pack->dwARQ) {
					// Log(Logs::Detail, Logs::Netcode, _L "Packet already
					// buffered %i" __L, pack->dwARQ);
					return true;  // This packet was already buffered
				}
				iterator++;
			}
			if (!no_ack_sent_timer->Enabled()) no_ack_sent_timer->Start(250);

			// Log(Logs::Detail, Logs::Netcode, _L "Buffering Packet %i" __L,
			// pack->dwARQ);
	...
	if (pack->b4_size > 0) {
	    ...
	    for (int i = 0; i < pack->b4_size; i++) {
	        for (int k = 0; k < 8; k++) {
	            sprintf(&str[i * 8 + k], "%s", pack->ack_fields[i] & 0x80 ? "1"
	: "0"); pack->ack_fields[i] <<= 1;
	        }
	...
	Log(Logs::Detail, Logs::Netcode, _L "Packet incoming arq%i:seq%i:arsp%i
	a0:%d|a1:%d|a2:%d|a3:%d|a4:%d|a5:%d|a6:%d|a7:%d|b0:%d|b1:%d|b2:%d|b3:%d|b4:%s|b5:%d|b6:%d|b7:%d"
	__L, pack->dwARQ, pack->dwSEQ, pack->dwARSP, pack->HDR.a0_Unknown ? 1 : 0,
	...
	/************ START ACK REQ CHECK ************/
	if (pack->HDR.a1_ARQ) IncomingARQ(pack->dwARQ);
	...
	if (pack->HDR.a2_Closing && pack->HDR.a6_Closing) {
		...
		if (state == ESTABLISHED) {
			...
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Received OP_SessionDisconnect. Treating like a "
			    "client-initiated disconnect." __L);
			_SendDisconnect();
			SetState(CLOSED);
		} else if (state == CLOSING) {
			// we were waiting for this anyways, ignore pending messages, send
			// the reply and be closed.
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Received OP_SessionDisconnect when we have a pending close, "
			    "they beat us to it. Were happy though." __L);
			_SendDisconnect();
			SetState(CLOSED);
		} else {
			// we are expecting this (or have already gotten it, but dont care
			// either way)
			Log(Logs::Detail, Logs::Netcode,
			    _L
			    "Received expected OP_SessionDisconnect. Moving to closed "
	...
	if (pack->HDR.a4_ASQ) {
		...
		// Log(Logs::Detail, Logs::Netcode, _L "Packet Flags a1:%d dbASQ_low: %d
		// a4:%d dbASQ_high: %d" __L, pack->HDR.a1_ARQ, pack->dbASQ_low,
		// pack->HDR.a4_ASQ, pack->dbASQ_high);

		if (pack->HDR.a1_ARQ) CACK.dbASQ_low = pack->dbASQ_low;
	}
	/************ End get ack seq num ************/
	...
	if (pack->HDR.a3_Fragment) {
		...
		// If we dont have a fragment group with the right sequence number,
		// create a new one
		if (fragment_group == 0) {
			fragment_group = new FragmentGroup(
			    pack->fraginfo.dwSeq, pack->dwOpCode, pack->fraginfo.dwTotal);
			fragment_group_list.Add(fragment_group);
		}

		// Add this fragment to the fragment group
		fragment_group->Add(pack->fraginfo.dwCurr, pack->pExtra,
		                    pack->dwExtraSize);
		...
		if (pack->fraginfo.dwCurr == (pack->fraginfo.dwTotal - 1)) {
			...
			// Why does the client sometimes send fragments with 0-size packets,
			// and why do they get completed? This should help prevent a crash.
			if (sizep == 0 || !buf || fragment_group->GetOpcode() == 0) {
				safe_delete_array(buf);
				return true;
			}

			EQRawApplicationPacket *app = new EQRawApplicationPacket(
			    fragment_group->GetOpcode(), buf, sizep);
			safe_delete_array(buf);
	...
	/************ ELSE - NO FRAGMENT ************/
		...
		if (!pack || pack && !pack->pExtra && pack->dwExtraSize > 0 ||
		    pack->dwOpCode == 0) {
			return true;
		}

		EQRawApplicationPacket *app = MakeApplicationPacket(pack);
		// if(app->GetRawOpcode() != 62272 && (app->GetRawOpcode() != 0 ||
		// app->Size() > 2)) //ClientUpdate 	Log(Logs::Detail, Logs::Netcode,
		//"Received old opcode - 0x%x size: %i", app->GetRawOpcode(),
		// app->Size());
		if (app) OutQueue.push_back(app);
		return true;
	}
	/************ END FRAGMENT CHECK ************/
...
void EQOldStream::CheckBufferedPackets() {
	...
	while (iterator != buffered_packets.end()) {
		...
		if ((*iterator)->dwARQ == nextARQ) {
			// Log(Logs::Detail, Logs::Netcode, _L "Processing Packet fromn
			// Buffer Seq # %d" __L, nextARQ);
...
void EQOldStream::MakeEQPacket(EQProtocolPacket *app, bool ack_req) {
	...
	/************ PM STATE = NOT ACTIVE ************/
	if (CheckState(CLOSED) || CheckState(CLOSING) ||
	    CheckState(DISCONNECTING) || !app || app && app->GetRawOpcode() == 0) {
		return;
	}
	...
	bool bFragment = false;  // This is set later on if fragseq should be
	...
	if (app->size == 19) {
		if (app->opcode == 0x9f40) {
			// we have a mob update opcode - see if the back of the sendqueue
			// has one to add this to
			MOutboundQueue.lock();
			if (!SendQueue.empty()) {
				EQOldPacket *oldpack;
				...
				if (oldpack->resend_count == 0 && oldpack->dwOpCode == 0x9f40) {
					...
					if (count < 30) {
						...
						memcpy((void *)newdata, (void *)olddata,
						       oldpack->dwExtraSize);
	...
	/************ PM STATE = ACTIVE ************/
	{
		for (int i = 0; i <= fragsleft; i++) {
			EQOldPacket *pack = new EQOldPacket();
			// IF NON PURE ACK THEN ALWAYS INCLUDE A ACKSEQ              // Agz:
			// Not anymore... Always include ackseq if not a fragmented packet
			if (i == 0 && ack_req)     // If this will be a fragmented packet, only
			                           // include ackseq in first fragment
			...
			/************ Normal ackseq ************/
				...
				if (ack_req) {  // If the caller of this function wants us to
					            // put an ack request on this packet
					pack->HDR.a1_ARQ = 1;
					pack->acked = false;
					pack->dwARQ = SACK.dwARQ++;
					SACK.dwGSQcount = 0;
				...
				if (pack->HDR.a4_ASQ) SACK.dbASQ_low++;
			}
				...
				if (LogSys.log_settings[Logs::PacketServerClientWithDump]
					...
					if (app_opcode != OP_SpecialMesg &&
					    (!RuleB(EventLog, SkipCommonPacketLogging) ||
					     (RuleB(EventLog, SkipCommonPacketLogging) &&
					      app_opcode != OP_MobHealth &&
					      app_opcode != OP_MobUpdate &&
					      app_opcode != OP_ClientUpdate))) {
						Log(Logs::General, Logs::PacketServerClientWithDump,
						    "[%s - 0x%04x] [Size: %u] %s",
						    OpcodeManager::EmuToName(app_opcode), app->opcode,
						    app->size, DumpProtocolPacketToString(app).c_str());
					}
				}
				if (LogSys.log_settings[Logs::PacketServerClient]
				        .is_category_enabled == 1) {
					EmuOpcode app_opcode = (*OpMgr)->EQToEmu(app->opcode);
					if (app_opcode != OP_SpecialMesg &&
					    (!RuleB(EventLog, SkipCommonPacketLogging) ||
			...
			if (app->size && app->pBuffer) {
				if (pack->HDR.a3_Fragment) {
					...
					if (i == fragsleft) {
						...
						pack->dwExtraSize =
						    app->size - 510 - 512 * ((app->size / 512) - 1);
					} else if (i == 0) {
						pack->dwExtraSize =
						    510;  // The first packet in a fragment group has
						          // 510 bytes for data
					} else {
						pack->dwExtraSize =
						    512;  // Other packets in a fragment group has 512
						          // bytes for data
					}
				...
				memcpy((void *)pack->pExtra, (void *)app->pBuffer,
				       pack->dwExtraSize);
...
void EQOldStream::CheckTimers(void) {
	...
	if (datarate_timer->Check(0)) {
		...
		if (dataflow < 0) dataflow = 0;
	}
}

void EQOldStream::QueuePacket(const EQApplicationPacket *p, bool ack_req) {
	//	ack_req = true;	// It's broke right now, dont delete this line till fix
	// it. =P
	...
	if (OpMgr == nullptr || *OpMgr == nullptr) {
		Log(Logs::Detail, Logs::Netcode,
		    _L
		    "Packet enqueued into a stream with no opcode manager, dropping.");
		delete p;
		return;
	}
...
void EQOldStream::FastQueuePacket(EQApplicationPacket **p, bool ack_req) {
	...
	*p = nullptr;  // clear caller's pointer.. effectively takes ownership
	...
	EQProtocolPacket *pack2 =
	    new EQProtocolPacket(opcode, pack->pBuffer, pack->size);

	// if(pack->emu_opcode != OP_MobUpdate && pack->emu_opcode != OP_MobHealth
	// && pack->emu_opcode != OP_HPUpdate) 	Log(Logs::Detail, Logs::Netcode, _L
	//"Sending old opcode 0x%04x" __L, opcode);
	MakeEQPacket(pack2, ack_req);
	delete pack;
	delete pack2;
}
...
EQApplicationPacket *EQOldStream::PopPacket() {
	...
	// resolve the opcode if we can. do not resolve protocol packets in
	// oldstreams
	if (p) {
		if (OpMgr != nullptr && *OpMgr != nullptr && p->GetRawOpcode() != 0 &&
		    p->GetRawOpcode() != 0xFFFF) {
...
void EQOldStream::ReceiveData(uchar *buf, int len) { ParceEQPacket(len, buf); }
...
void EQOldStream::SendPacketQueue(bool Block) {
	...
	while (packit != SendQueue.end() && !DataQueueFull()) {
		...
		if (pack != nullptr &&
		    (pack->resend_count == 0 ||
		     pack->LastSent == 0 ||  // first time sent packets
		     (!pack->acked &&
		          (pack->Resend &&
		           ((time_now - pack->LastSent) >
		            20)) ||  // flagged for resend, but was just barely sent
		      (!pack->Resend && (pack->resend_count) < 10 &&
		       ((time_now - pack->LastSent) > 3000)))))  // packet timeout
		{
			if (pack->HDR.a2_Closing &&
			    pack->HDR.a6_Closing)  // Closing bits. Terminates the
			                           // connection properly.
			{
				size = pack->ReturnPacket(&data, this);
				sendto(listening_socket, (char *)data, size, 0, (sockaddr *)&to,
				       sizeof(to));
				// Log(Logs::Detail, Logs::Netcode, _L "Sending Closing Packet
				// %d." __L, pack->dwARQ);
				dataflow += size;
			...
			{
				if (pack->HDR.a1_ARQ) {
					// if we are too far out on arq's, this will help prevent
					// client from going into desync
					if (pack->dwARQ > dwLastARSP &&
					    (pack->dwARQ - dwLastARSP - 100) >= 0)
						break;
					if (pack->dwARQ < dwLastARSP &&
					    (65535 - dwLastARSP + pack->dwARQ - 100) >= 0)
						break;
					keep_alive_timer->Disable();
				}
				if (!pack->Resend) sentpacket++;
				size = pack->ReturnPacket(&data, this);
				pack->resend_count++;
				pack->Resend = false;
				...
				//	if (pack->HDR.a1_ARQ)
				//		Log(Logs::Detail, Logs::Netcode, _L "Sending Packet %d."
				//__L, pack->dwARQ);
				sendto(listening_socket, (char *)data, size, 0, (sockaddr *)&to,
				       sizeof(to));
				//}
				...
				if (pack->HDR.a1_ARQ && !pack->HDR.a3_Fragment &&
				    (sentpacket > 10) && GetState() == ESTABLISHED) {
					break;
				}
	...
	if (sentpacket > 0) keep_alive_timer->Start(450);
	// ************ Processing finished ************ //
	MOutboundQueue.unlock();
}
...
void EQOldStream::FinalizePacketQueue() {
	...
	if (GetState() != CLOSING) MakeClosePacket();
	for (auto packit = SendQueue.begin(); packit != SendQueue.end();) {
		p = (*packit);
		if (p->HDR.a1_ARQ || p->HDR.a3_Fragment) {
			if (!p->acked) {
...
// this could be expanded to check more than the fitst opcode if
// we needed more complex matching
EQStream::MatchState EQOldStream::CheckSignature(
    const EQStream::Signature *sig) {
	EQRawApplicationPacket *p = nullptr;
	...
	if (!OutQueue.empty()) {
		...
		std::vector<EQRawApplicationPacket *>::iterator itr = OutQueue.begin();
		p = *itr;
		if (sig->ignore_eq_opcode != 0 &&
		    p->GetRawOpcode() == sig->ignore_eq_opcode) {
			if (OutQueue.empty()) {
				p = nullptr;
			}
		}
		if (p == nullptr) {
			...
			if (p->size == sig->first_length) {
				Log(Logs::Detail, Logs::Netcode,
				    "[TRACE] %s:%d: First opcode matched 0x%x and length "
				    "matched %d",
				    long2ip(GetRemoteIP()).c_str(), ntohs(GetRemotePort()),
				    sig->first_eq_opcode, p->size);
				...
				Log(Logs::Detail, Logs::Netcode,
				    "[TRACE] %s:%d: First opcode matched 0x%x and length (%d) "
				    "is ignored",
				    long2ip(GetRemoteIP()).c_str(), ntohs(GetRemotePort()),
				    sig->first_eq_opcode, p->size);
				res = EQStream::MatchState::MatchSuccessful;
				...
				// opcode matched but length did not.
				Log(Logs::Detail, Logs::Netcode,
				    "[TRACE] %s:%d: First opcode matched 0x%x, but length %d "
				    "did not match expected %d",
				    long2ip(GetRemoteIP()).c_str(), ntohs(GetRemotePort()),
				    sig->first_eq_opcode, p->size, sig->first_length);
			...
			// first opcode did not match..
			Log(Logs::Detail, Logs::Netcode,
			    "[TRACE] %s:%d: First opcode 0x%x did not match expected 0x%x",
			    long2ip(GetRemoteIP()).c_str(), ntohs(GetRemotePort()),
			    p->GetRawOpcode(), sig->first_eq_opcode);
			res = EQStream::MatchState::MatchFailed;
		}
	}
	MInboundQueue.unlock();
...
void EQOldStream::_SendDisconnect() { FinalizePacketQueue(); }
void EQOldStream::Close() {
	if (HasOutgoingData()) {
		...
		_SendDisconnect();
		Log(Logs::Detail, Logs::Netcode,
		    _L "EQStream closing immediate due to Close()" __L);
		SetState(DISCONNECTING);
	}
}
...
EQRawApplicationPacket *EQOldStream::MakeApplicationPacket(EQOldPacket *p) {
	EQRawApplicationPacket *ap = nullptr;
	Log(Logs::Detail, Logs::Netcode,
	    _L "Creating old application packet, length %d" __L, p->dwExtraSize);
	ap = p->MakeAppPacket();
	return ap;
}
