...
class FixedMemoryHashSet {
   ...
   public:
	...
	\param element_count Max number of possible unique elements that can be
	inserted. \param max_element_id Number of offsets to store: eg highest "key"
	that will be used.
	*/
	FixedMemoryHashSet(byte* data, size_type size, key_type element_count,
	...
	*/
	...
	size_type size() const { return current_elements_; }

	//! Returns the maximum number of elements one can insert into the set.
	size_type max_size() const { return max_elements_; }
	...
	*/
	...
	//! Calculates how much memory we should allocate based on element size and
	//! count
	static size_type estimated_size(key_type element_count,
	                                key_type max_elements) {
