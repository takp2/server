...
void oldFileDumpPacketHex(const char* filename, const uchar* buf, uint32 size,
                          uint32 cols, uint32 skip) {
	std::ofstream logfile(filename, std::ios::app);
	// Output as HEX
	char output[4];
	for (uint32 i = skip; i < size; i++) {
		...
		logfile << output;
		// logfile << std::setfill(0) << std::setw(2) << std::hex << (int)buf[i]
		// << " "; // unknown intent [CODEBUG]
	}
	logfile << std::endl
	        << std::endl;
}
...
void FileDumpPacketHex(const char* filename, const uchar* buf, uint32 size,
	...
	for (i = skip; i < size; i++) {
		if ((i - skip) % cols == 0) {
			if (i != skip) logfile << " | " << ascii << std::endl;
...
void FileDumpPacket(const char* filename, const EQApplicationPacket* app) {
	FilePrintLine(filename, true, "Size: %5i, OPCode: 0x%04x", app->size,
	              app->GetOpcode());
	FileDumpPacketHex(filename, app->pBuffer, app->size);
	//	FileDumpPacketAscii(filename, app->pBuffer, app->size);
}
...
/*
    ...
    if prefix_timestamp specified, prints the current date/time to the file + ":
   " + text
*/
void FilePrintLine(const char* filename, bool prefix_timestamp,
                   const char* text, ...) {
	std::ofstream logfile(filename, std::ios::app);
	if (prefix_timestamp) {
		time_t rawtime;
		...
		logfile << (gmt_t->tm_year + 1900) << "/" << std::setw(2)
		        << std::setfill('0') << (gmt_t->tm_mon + 1) << "/"
		        << std::setw(2) << std::setfill('0') << gmt_t->tm_mday << " "
		        << std::setw(2) << std::setfill('0') << gmt_t->tm_hour << ":"
		        << std::setw(2) << std::setfill('0') << gmt_t->tm_min << ":"
		        << std::setw(2) << std::setfill('0') << gmt_t->tm_sec << " GMT";
	}
	...
	if (text != 0) {
		...
		if (prefix_timestamp) logfile << ": ";
		logfile << buffer;
	}
	logfile << std::endl;
}

void FilePrint(const char* filename, bool newline, bool prefix_timestamp,
               const char* text, ...) {
	std::ofstream logfile(filename, std::ios::app);
	if (prefix_timestamp) {
		time_t rawtime;
