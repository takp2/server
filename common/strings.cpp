// Code from https://github.com/facebook/folly/tree/main
...
const std::string vStringFormat(const char* format, va_list args) {
	...
	if (characters_used > 0) {
		...
		va_copy(tmpargs, args);
		characters_used =
		    vsnprintf(&output[0], output.capacity(), format, tmpargs);
		va_end(tmpargs);

		output.resize(characters_used);

		// We shouldn't have a format error by this point, but I can't imagine
		// what error we could have by this point. Still, return empty string;
		if (characters_used < 0) output.clear();
	}
	return output;
}
...
const std::string Strings::UcFirst(std::string s) {
	std::string output = s;
	if (!s.empty()) output[0] = static_cast<char>(::toupper(s[0]));
...
std::vector<std::string> Strings::Split(const std::string& str,
	...
	// this will catch the last word since the string is unlikely to end with a
	// delimiter
	if (str.length() > start)
...
std::string::size_type Strings::SearchDelim(const std::string& haystack,
	...
	while (pos != std::string::npos) {
		...
		if ((c == '\0' || c == deliminator) &&
		    (pos == 0 || haystack[pos - 1] == deliminator))
			return pos;
...
std::vector<std::string> Strings::Wrap(std::vector<std::string>& src,
...
std::string Strings::Join(const std::vector<std::string>& ar,
...
void Strings::FindReplace(std::string& string_subject,
	...
	size_t start_pos = 0;
	while ((start_pos = string_subject.find(search_string, start_pos)) !=
	       std::string::npos) {
		string_subject.replace(start_pos, search_string.length(),
		                       replace_string);
		start_pos += replace_string.length();
	}
}

std::string Strings::Replace(std::string subject, const std::string& search,
...
void ParseAccountString(const std::string& s, std::string& account,
...
std::string Strings::Commify(const std::string& number) {
	...
	for (i = string_length - 3; i >= 0; i -= 3) {
		if (i > 0) {
			temp_string = "," +
			              number.substr(static_cast<unsigned long>(i), 3) +
			              temp_string;
		} else {
			temp_string =
			    number.substr(static_cast<unsigned long>(i), 3) + temp_string;
		}
	}

	if (i < 0) {
		temp_string =
		    number.substr(0, static_cast<unsigned long>(3 + i)) + temp_string;
	}

	return temp_string;
}

bool Strings::Contains(std::vector<std::string> container,
                       std::string element) {
...
// ref:
// http://msdn.microsoft.com/library/default.asp?url=/library/en-us/wcecrt/htm/_wcecrt_strncpy_wcsncpy.asp
...
bool atobool(const char* iBool) {
	if (iBool == nullptr) return false;
	if (!strcasecmp(iBool, "true")) return true;
	if (!strcasecmp(iBool, "false")) return false;
	if (!strcasecmp(iBool, "yes")) return true;
	if (!strcasecmp(iBool, "no")) return false;
	if (!strcasecmp(iBool, "on")) return true;
	if (!strcasecmp(iBool, "off")) return false;
	if (!strcasecmp(iBool, "enable")) return true;
	if (!strcasecmp(iBool, "disable")) return false;
	if (!strcasecmp(iBool, "enabled")) return true;
	if (!strcasecmp(iBool, "disabled")) return false;
	if (!strcasecmp(iBool, "y")) return true;
	if (!strcasecmp(iBool, "n")) return false;
	if (atoi(iBool)) return true;
	return false;
}
...
char* CleanMobName(const char* in, char* out) {
	...
	for (i = j = 0; i < strlen(in); i++) {
		...
		if (in[i] == '_') {
			...
			if (isalpha(in[i]) ||
			    (in[i] ==
			     '`'))  // numbers, #, or any other crap just gets skipped
				out[j++] = in[i];
		}
	}
...
char* CleanMobNameWithSpaces(const char* in, char* out) {
	...
	for (i = j = 0; i < strlen(in); i++) {
		if (isalpha(in[i]) || (in[i] == '`') ||
		    (in[i] == '_'))  // numbers, #, or any other crap just gets skipped
			out[j++] = in[i];
	}
	out[j] = 0;  // terimnate the string before returning it
	return out;
...
void replace_all(std::string& in, std::string old, std::string repl) {
	for (std::string::size_type pos = 0;
	     (pos = in.find(old, pos)) != std::string::npos; pos += repl.size()) {
...
std::string Strings::SecondsToTime(int duration, bool is_milliseconds) {
	...
	if (is_milliseconds && duration < 1000) {
		return fmt::format(" {} Millisecond{} ", duration,
		                   duration != 1 ? "s" : "");
	}

	int timer_length =
	    (is_milliseconds ? static_cast<int>(std::ceil(
	...
	if (days && hours && minutes && seconds) {  // DHMS
		time_string = fmt::format(" {} {}, {} {}, {} {}, and {} {} ", days,
		                          day_string, hours, hour_string, minutes,
		                          minute_string, seconds, second_string);
	} else if (days && hours && minutes && !seconds) {  // DHM
		time_string = fmt::format(" {} {}, {} {}, and {} {} ", days, day_string,
		                          hours, hour_string, minutes, minute_string);
	} else if (days && hours && !minutes && seconds) {  // DHS
		time_string = fmt::format(" {} {}, {} {}, and {} {} ", days, day_string,
		                          hours, hour_string, seconds, second_string);
	} else if (days && hours && !minutes && !seconds) {  // DH
		time_string = fmt::format(" {} {} and {} {} ", days, day_string, hours,
		                          hour_string);
	} else if (days && !hours && minutes && seconds) {  // DMS
		time_string =
		    fmt::format(" {} {}, {} {}, and {} {} ", days, day_string, minutes,
		                minute_string, seconds, second_string);
		...
		time_string = fmt::format(" {} {} and {} {} ", minutes, minute_string,
		                          seconds, second_string);
...
std::string Strings::Money(uint32 platinum, uint32 gold, uint32 silver,
                           uint32 copper) {
	std::string money_string = "Unknown";
	if (copper && silver && gold && platinum) {  // CSGP
		money_string =
		    fmt::format(" {} platinum, {} gold, {} silver, and {} copper",
		...
		money_string = fmt::format(" {} gold, {} silver, and {} copper",
		                           Strings::Commify(std::to_string(gold)),
		                           Strings::Commify(std::to_string(silver)),
		                           Strings::Commify(std::to_string(copper)));
	} else if (copper && silver && !gold && !platinum) {  // CS
		money_string = fmt::format(" {} silver and {} copper",
		                           Strings::Commify(std::to_string(silver)),
		                           Strings::Commify(std::to_string(copper)));
	} else if (copper && !silver && !gold && platinum) {  // CP
		money_string = fmt::format(" {} platinum and {} copper",
		                           Strings::Commify(std::to_string(platinum)),
		                           Strings::Commify(std::to_string(copper)));
	} else if (copper && silver && !gold && platinum) {  // CSP
		money_string = fmt::format(" {} platinum, {} silver, and {} copper",
		                           Strings::Commify(std::to_string(platinum)),
		                           Strings::Commify(std::to_string(silver)),
		                           Strings::Commify(std::to_string(copper)));
	} else if (!copper && silver && gold && platinum) {  // SGP
		money_string = fmt::format(" {} platinum, {} gold, and {} silver",
		                           Strings::Commify(std::to_string(platinum)),
		                           Strings::Commify(std::to_string(gold)),
		                           Strings::Commify(std::to_string(silver)));
	} else if (!copper && silver && gold && !platinum) {  // SG
		money_string = fmt::format(" {} gold and {} silver",
		                           Strings::Commify(std::to_string(gold)),
		                           Strings::Commify(std::to_string(silver)));
	} else if (!copper && silver && !gold && platinum) {  // SP
		money_string = fmt::format(" {} platinum and {} silver",
		                           Strings::Commify(std::to_string(platinum)),
		                           Strings::Commify(std::to_string(silver)));
	} else if (copper && !silver && gold && platinum) {  // CGP
		money_string = fmt::format(" {} platinum, {} gold, and {} copper",
		                           Strings::Commify(std::to_string(platinum)),
		                           Strings::Commify(std::to_string(gold)),
		                           Strings::Commify(std::to_string(copper)));
	} else if (copper && !silver && gold && !platinum) {  // CG
		money_string = fmt::format(" {} gold and {} copper",
		                           Strings::Commify(std::to_string(gold)),
		                           Strings::Commify(std::to_string(copper)));
	} else if (!copper && !silver && gold && platinum) {  // GP
		money_string = fmt::format(" {} platinum and {} gold",
		                           Strings::Commify(std::to_string(platinum)),
		                           Strings::Commify(std::to_string(gold)));
	} else if (!copper && !silver && !gold && platinum) {  // P
		money_string = fmt::format(" {} platinum",
		                           Strings::Commify(std::to_string(platinum)));
	} else if (!copper && !silver && gold && !platinum) {  // G
		money_string =
		    fmt::format(" {} gold", Strings::Commify(std::to_string(gold)));
	} else if (!copper && silver && !gold && !platinum) {  // S
		money_string =
		    fmt::format(" {} silver", Strings::Commify(std::to_string(silver)));
	} else if (copper && !silver && !gold && !platinum) {  // C
		money_string =
		    fmt::format(" {} copper", Strings::Commify(std::to_string(copper)));
	}
	return money_string;
}
...
uint32 Strings::TimeToSeconds(std::string time_string) {
	...
	std::transform(time_string.begin(), time_string.end(), time_string.begin(),
	               ::tolower);

	std::string time_unit = time_string;
	time_unit.erase(remove_if(time_unit.begin(), time_unit.end(),
