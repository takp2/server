...
enum {  // reuse times
	...
	HarmTouchReuseTimeNPC =
	    2400,  // NPCs have 40 minute timers according to logs
...
enum {  // timer settings, all in milliseconds
	...
	ZoneTimerResolution =
	    3,  // sleep time between zone main loop runs (milliseconds)
	EnragedTimer = 360000,
...
// chance ratio that a
#define THREATENINGLY_ARRGO_CHANCE \
	32  // 32/128 (25%) chance that a mob will aggro on con Threateningly
...
// the square of the maximum range at which you could possibly use NPC services
// (shop, etc)
...
enum {
	...
	commandMovecharToSpecials =
	    200,                     // ability to send people to cshome/load zones
	...
	commandInterrogateInv =
	    100  // below this == only log on error state and self-only target dump
};
