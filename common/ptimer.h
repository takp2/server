...
enum {  // values for pTimerType
	...
	pTimerSpellStart =
	    5000,  // Do not put any timer IDs above this one unless it is a spell
...
class PersistentTimer {
   public:
	static PersistentTimer *LoadTimer(Database *db, uint32 char_id,
	                                  pTimerType type);

	PersistentTimer(uint32 char_id, pTimerType type, uint32 duration);
	PersistentTimer(uint32 char_id, pTimerType type, uint32 start_time,
	                uint32 duration, bool enable);
	~PersistentTimer() {}
...
class PTimerList {
   public:
	...
	void AddTimer(pTimerType type, uint32 start_time, uint32 duration,
	              bool enable);
...
#define AA_Choose5(val, v1, v2, v3, v4, v5) \
	(val == 1 ? v1 : (val == 2 ? v2 : (val == 3 ? v3 : (val == 4 ? v4 : v5))))
