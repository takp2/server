...
struct LoginInfo_Struct {
	...
	/*488*/ uint8 macversion;  // in mac packet decode, intel and ppc clients
	                           // send different lengths of this so we can
	                           // remember it here and pass it down
	                           /*489*/
};
...
struct CharacterSelect_Struct {
	...
	/*1100*/ EQ::TintProfile
	    cs_colors[10];          // Characters Equipment Colors (RR GG BB 00)
	/*1460*/ uint16 deity[10];  // Characters Deity
	/*1480*/ uint32
	    primary[10];  // Characters primary and secondary IDFile number
	/*1520*/ uint32
	    secondary[10];  // Characters primary and secondary IDFile number
...
struct Spawn_Struct {
	...
	/*0086*/ uint8 NPC;      // NPC type: 0=Player, 1=NPC, 2=Player Corpse,
	                         // 3=Monster Corpse, 4=???, 5=Unknown Spawn,10=Self
	...
	uint8 max_hp;          // (name prolly wrong)takes on the value 100 for players, 100
	                       // or 110 for NPCs and 120 for PC corpses...
...
struct ServerZoneEntry_Struct {
	...
	/*0184*/ uint16
	    equipment[9];                      // Array elements correspond to struct equipment above
	/*0202*/ uint16 unknown;               // Probably part of equipment
	/*0204*/ EQ::TintProfile equipcolors;  // Array elements correspond to
	                                       // struct equipment_colors above
	/*0240*/ uint32
	    bodytexture;  // Texture (0xFF=Player - See list of textures for more)
	/*0244*/ float size;
	/*0248*/ float width;
	/*0252*/ float length;
	/*0256*/ uint32 helm;
	...
	/*0285*/ uint8 title;
	/*0286*/ uint8 extra[18];  // ***Placeholder (At least one flag in here
	                           // disables a zone point or all)
	/*0304*/ char
	    Surname[32];  // Lastname (This has to be wrong.. but 70 is to big =/..)
...
struct NewZone_Struct {
	...
	/*0374*/ uint8 ztype;
	/*0375*/ uint8
	    fog_red[4];  // Red Fog 0-255 repeated over 4 bytes (confirmed)
	/*0379*/ uint8
	    fog_green[4];  // Green Fog 0-255 repeated over 4 bytes (confirmed)
	/*0383*/ uint8
	    fog_blue[4];  // Blue Fog 0-255 repeated over 4 bytes (confirmed)
	/*0387*/ uint8 unknown387;
	/*0388*/ float
	    fog_minclip[4];  // Where the fog begins (lowest clip setting). Repeated
	                     // over 4 floats. (confirmed)
	/*0404*/ float
	    fog_maxclip[4];  // Where the fog ends (highest clip setting). Repeated
	                     // over 4 floats. (confirmed)
	/*0420*/ float gravity;
...
struct MemorizeSpell_Struct {
	...
	/*004*/ uint32
	    spell_id;             // Comment:  Spell id (200 or c8 is minor healing, etc)
	/*008*/ uint32 scribing;  // Comment:  1 if memorizing a spell, set to 0 if
	                          // scribing to book
	                          /*012*/
};
...
struct BeginCast_Struct {
	/*000*/ uint16 caster_id;  // Comment: Unknown -> needs confirming -> ID of
	                           // Spell Caster?
	/*002*/ uint16 spell_id;   // Comment: Unknown -> needs confirming -> ID of
	                           // Spell being Cast?
	/*004*/ uint16
	    cast_time;  // Comment: Unknown -> needs confirming -> in miliseconds?
	/*006*/ uint16 unknown;
	/*008*/
};
...
struct SpellBuff_Struct {
	...
	/*003*/ uint8 activated;  // copied from spell data to buff, only a few
	                          // spells have this as 1
	/*004*/ uint16 spellid;
	/*008*/ uint16 duration;
	/*012*/ uint16 counters;
	/*016*/ uint32
...
struct SpellBuffFade_Struct {
	...
	// next 10 bytes is SpellBuff_Struct - we can set bufffade to 3 to replace
	// buffs by slot_number
	/*002*/ uint8 bufftype;
	/*003*/ uint8 level;
	/*004*/ uint8 bard_modifier;
	/*005*/ uint8 activated;  // Copied from spell data to buff struct.  Only a
...
struct ItemProperties_Struct {
	...
	/*002*/ int8
	    charges;  // Comment: signed int because unlimited charges are -1
...
static const uint32 MAX_PP_MEMSPELL =
    static_cast<uint32>(EQ::spells::CastingSlot::MaxGems);
static const uint32 MAX_PP_REF_SPELLBOOK =
    256;  // Set for Player Profile size retain
static const uint32 MAX_PP_REF_MEMSPELL =
    8;  // Set for Player Profile size retain

static const uint32 MAX_PP_SKILL =
    PACKET_SKILL_ARRAY_SIZE;  // 100 - actual skills buffer size
...
struct PlayerProfile_Struct {
	...
	/*0316*/ struct ItemProperties_Struct
	    invItemProperties[30];  // These correlate with inventory[30]
	/*0616*/ struct SpellBuff_Struct
	    buffs[BUFF_COUNT];  // Player Buffs Currently On
	/*0766*/ int16 containerinv[80];
	/*0926*/ int16 cursorbaginventory[10];  // If a bag is in slot 0, this is
	                                        // where the bag's items are
	/*0946*/ struct ItemProperties_Struct
	    bagItemProperties[80];  // Just like InvItemProperties
	/*1746*/ struct ItemProperties_Struct
	    cursorItemProperties[10];                     // just like invitemprops[]
	/*1846*/ int16 spell_book[MAX_PP_REF_SPELLBOOK];  // Player spells scribed
	                                                  // in their book
	...
	/*3444*/ uint32 zone_id;
	/*3448*/ uint8
	    unknown3448[336];  // On AK, there was a lot of data here in the packet
	                       // the client sent for OP_Save, but none in the
	                       // encoded packet the server sent at zone in.
	/*3784*/ BindStruct
	    binds[5];  // Bind points (primary is first, home city is fifth)
	...
	/*4784*/ int16
	    bank_cont_inv[80];     // Player Bank Inventory Item Numbers (Bags)
	...
	/*4967*/ uint8
	    guildrank;  // Player Guild Rank (0=member, 1=officer, 2=leader)
	...
	/*6724*/ uint8 Unknown6724[1736];  // According to the client, this is all
	                                   // unused/unknown space.
	...
	uint32 famished;
	uint32 boatid;
	uint32 zone_change_count;  // Number of times user has zoned in their career
	                           // (guessing)
	uint32 endurance;
...
struct ChannelMessage_Struct {
	...
	/*134*/ uint16 skill_in_language;  // The players skill in this language?
	                                   // might be wrong
...
/*
** When somebody changes what they're wearing or give a pet a weapon (model
*changes)
*/
struct WearChange_Struct {
	...
	/*003*/ uint8
	    align03;  // struct field alignment only, data here is not meaningful
	/*004*/ uint16 material;
	/*006*/ uint16
	    align06;  // struct field alignment only, data here is not meaningful
...
struct BindWound_Struct {
	...
	/*002*/ uint8 type;  // 0 or 1 complete, 2 Unknown, 3 ACK, 4 Died, 5 Left, 6
	                     // they moved, 7 you moved
	/*003*/ uint8 void_;
};
...
struct ZoneChange_Struct {
	...
	/*072*/ int8
	    success;             // =0 client->server, =1 server->client, -X=specific error
	/*073*/ uint8 error[3];  // =0 ok, =ffffff error
	                         /*076*/
};

// Whatever you send to the client in RequestClientZoneChange_Struct.type, the
// client will send back to the server in ZoneChange_Struct.zone_reason. My
// guess is this is a memo field of sorts.
...
struct Action_Struct {
	...
	/*04*/ uint16 level;            // this only does something for spell ids 1252-1266
	                                // (potions) and only accepts values 1-60, otherwise
	                                // the action uses source entity level
	...
	/*28*/ int16 tap_amount;        // used in client for instant, targettype 13 (tap)
	                                // spells to set the amount that was tapped
...
struct Damage_Struct {
	...
	/*020*/ float pushup_angle;  // associated with force.  Sine of this angle,
	                             // multiplied by force, will be z push.
	                             /*024*/
};
...
struct SpawnPositionUpdate_Struct {
	...
	struct {
		...
		float GetX() {
			...
			int32 vx = static_cast<int32>(vx1 & 0x200 ? vx1 | 0xFFFFFC00
			                                          : vx1);  // extend sign
		...
		float GetY() {
			...
			int32 vy = static_cast<int32>(vy1 & 0x400 ? vy1 | 0xFFFFF800
			                                          : vy1);  // extend sign
		...
		float GetZ() {
			uint32 vz1 =
			    (value >> 11) & 0x7FF;  // 11 bits long, bits 10 through 21
			int32 vz = static_cast<int32>(vz1 & 0x400 ? vz1 | 0xFFFFF800
			                                          : vz1);  // extend sign
		...
		uint32 SetValue(float vx, float vy, float vz) {
			value = ((int)(float)(vx * 16.0f) << 22) |
			        (((int)(float)(vz * 16.0f) & 0x7FF) << 11) |
			        (int)(float)(vy * 16.0f) & 0x7FF;
...
struct Consume_Struct {
	/*000*/ uint32 slot;
	/*004*/ uint32
	    auto_consumed;  // 0xffffffff when auto eating e7030000 when right click
...
struct moneyOnCorpseStruct {
	/*0000*/ uint8
	    response;              // 0 = someone else is, 1 = OK, 2 = not at this time
...
struct GMZoneRequest_Struct {
	...
	/*0084*/ uint32 success;  // 0 if command failed, 1 if succeeded?
	                          /*0088*/
	                          //	/*072*/	int8	success;		// =0 client->server, =1 server->client,
	                          //-X=specific error
...
struct GMSummon_Struct {
	...
	/* 132*/ uint32 zoneID;
	/* 136*/ int y;  // these values are truncated to integer by the client
	                 // before being sent
	/* 140*/ int x;
	/* 144*/ int z;
...
struct GMGoto_Struct {  // x,y is swapped as compared to summon and makes sense
	                    // as own packet
	/*  0*/ char charname[64];
...
struct TimeOfDay_Struct {
	...
	/*004*/ uint16
	    year;  // Comment: Kibanu - Changed from long to uint16 on 7/30/2009
	           /*006*/
};
...
struct ZonePoints {
	/*0000*/ uint32 count;
	/*0004*/ struct ZonePoint_Entry
	    zpe[0];  // Always add one extra to the end after all zonepoints
};
...
struct FaceChange_Struct {
	...
	/*002*/ uint8 eyecolor1;   // Comment: the eyecolors always seem to be the
	                           // same, maybe left and right eye?
...
** Object/Ground Spawn struct
    We rely on the encode/decode to send the proper packet, this is just an
internal data struct like PP.
    */
struct Object_Struct {
	/*00*/ uint32 linked_list_addr[2];  // <Zaphod> They are, get this, prev and
	                                    // next, ala linked list
	/*008*/ uint16 itemid;
	/*010*/ uint16 unknown010;
	/*012*/ int32
	    drop_id;  // this id will be used, if someone clicks on this object
	/*016*/ int16 zone_id;
	...
	/*186*/ int16
	    itemsinbag[10];  // if you drop a bag, thats where the items are
...
//[13:40] <Zaphod> and 0xff seems to be indicative of the tradeskill/openable
// items that end up returning the old style item type in the OP_OpenObject
...
struct ClickObjectAction_Struct {
	/*000*/ uint32
	    player_id;               // Comment: Entity Id of player who clicked object
	/*004*/ uint32 drop_id;      // Comment: Unknown
	/*008*/ uint8 open;          // Comment: 0=close 1=open
	/*009*/ uint8 unknown9;      // Comment: Unknown
	/*010*/ uint8 type;          // Comment: number that determines the type of the item
	                             // (i.e forge)
...
struct ServerSideFilters_Struct {
	uint8 clientattackfilters;  // 0) No, 1) All (players) but self, 2) All
	                            // (players) but group
	uint8 npcattackfilters;     // 0) No, 1) Ignore NPC misses (all), 2) Ignore NPC
	                            // Misses + Attacks (all but self), 3) Ignores NPC
	                            // Misses + Attacks (all but group)
	uint8 clientcastfilters;    // 0) No, 1) Ignore PC Casts (all), 2) Ignore PC
	                            // Casts (not directed towards self)
	uint8 npccastfilters;       // 0) No, 1) Ignore NPC Casts (all), 2) Ignore NPC
	                            // Casts (not directed towards self)
};
...
struct PickPocket_Struct {
	...
	uint16 from;
	uint16 myskill;
	uint16 type;  // -1 you are being picked, 0 failed , 1 = plat, 2 = gold, 3 =
	              // silver, 4 = copper, 5 = item
	uint32 coin;
	uint8 data[6];
};
...
struct LogServer_Struct {
	...
	/*180*/ char localizedEmailAddress[64];  // 'eqdataexceptions@mail.station.sony.com'
	                                         // on live
	                                         /*244*/
};
...
// for use in World ClientList::SendFriendsWho to accomodate the user of
// variable length strings within the struct above.
...
struct Internal_GuildMembers_Struct {  // just for display purposes, this is not
	                                   // actually used in the message encoding.
	char player_name[64];              // variable length.
	uint32 count;                      // network byte order
	uint32 name_length;                // total length of all of the char names, excluding
	                                   // terminators
	uint32 note_length;                // total length of all the public notes, excluding
	                                   // terminators
...
struct Combine_Struct {
	/*000*/ uint8 worldobjecttype;  // Comment: if its a world object like
	                                // forge, id will be here
	/*001*/ uint8 unknown001;       // Comment:
	/*002*/ int16 skill;            // Comment:
	/*004*/ int16 container_slot;   // Comment: the position of the container, or
	                                // 1000 if its a world container
...
struct TempMerchantList {
	...
	uint32 origslot;
	uint32 quantity;  // This is used to determine how many charged items we
	                  // have, since the charges clump together
};
...
struct RaidAddMember_Struct {
	/*000*/ RaidGeneral_Struct
	    raidGen;  // param = (group num-1); 0xFFFFFFFF = no group
	/*136*/ uint8 _class;
	/*137*/ uint8 level;
...
struct Arrow_Struct {
	...
	/*078*/ uint8 behavior;
	/*079*/ uint8 effect_type;  // 9 for spell, uses itemtype for items. 28 is
	                            // also valid, possibly underwater attack?
	/*080*/ uint8 skill;
...
struct ClearObject_Struct {
	/*000*/ uint8 Clear;  // If this is not set to non-zero there is a random
	                      // chance of a client crash.
...
struct BeggingResponse_Struct {
	...
	/*04*/ uint8 skill;
	/*05*/ uint8 unknown1;
	/*06*/ int8 Result;  // -1 = Request, 0 = Fail, 1 = Plat, 2 = Gold, 3 =
	                     // Silver, 4 = Copper
	/*07*/ uint8 unknown2;
	/*08*/ uint32 Amount;
	/*12*/ uint32 unknown3;
...
// S->C and C->S ENTRY for informing us about players' soulmarks. Used in struct
// below this one.
...
struct SoulMarkList_Struct {
	...
	SoulMarkEntry_Struct
	    entries[12];  // I have no idea why SOE hardcodes the amount of
	                  // soulmarks, but it shall never exceed 12. Delete
	                  // soulmarks I guess in the future /shrug
};
...
struct GroupExpSplit_Struct {
	// The membercount variables are all used for the group bonus. The split
	// uses weighted_levels.
	int8 membercount;        // Total members in the group who are in the zone
	int8 close_membercount;  // Total members in the group who are in zone and
	                         // are in physical range of the corpse.
	uint8 max_green_level;   // Highest player level in the group who the NPC is
	                         // not green to.
	uint16 weighted_levels;  // The sum of all player levels in the group in
	                         // range of corpse.  Used to determine group split.
};
