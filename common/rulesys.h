...
class RuleManager {
   public:
	...
	typedef enum {
#define RULE_INT(cat, rule, default_value) Int__##rule,
#include "ruletypes.h"
		_IntRuleCount
	} IntType;

	typedef enum {
#define RULE_REAL(cat, rule, default_value) Real__##rule,
#include "ruletypes.h"
		_RealRuleCount
	} RealType;

	typedef enum {
#define RULE_BOOL(cat, rule, default_value) Bool__##rule,
#include "ruletypes.h"
		_BoolRuleCount
	} BoolType;

	typedef enum {
#define RULE_CATEGORY(catname) Category__##catname,
#include "ruletypes.h"
		_CatCount
	} CategoryType;
	...
	static const uint32 _RulesCount =
	    _IntRuleCount + _RealRuleCount + _BoolRuleCount;
	...
	static const char *GetRuleName(RealType t) {
		return (s_RuleInfo[t + _IntRuleCount].name);
	}
	static const char *GetRuleName(BoolType t) {
		return (s_RuleInfo[t + _IntRuleCount + _RealRuleCount].name);
	}
	...
	bool SetRule(const char *rule_name, const char *rule_value,
	             Database *db = nullptr, bool db_save = false);
   ...
   private:
	...
	typedef enum { IntRule, RealRule, BoolRule } RuleType;

	static bool _FindRule(const char *rule_name, RuleType &type_into,
	                      uint16 &index_into);
