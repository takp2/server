...
class ListElement {
   ...
   public:
	...
	void SetData(const TYPE& d) {
		data = d;
	}  // Quagmire - this may look like a mem leak, but dont change it, this
	   // behavior is expected where it's called
	void SetLastNext(ListElement<TYPE>* p) { GetLast()->SetNext(p); }
...
class LinkedListIterator {
   ...
   public:
	LinkedListIterator(LinkedList<TYPE>& l, direction d = FORWARD)
	    : list(l), dir(d){};
...
void LinkedListIterator<TYPE>::RemoveCurrent(bool DeleteData) {
	...
	current_element->SetNext(0);
	current_element->SetPrev(0);
	if (!DeleteData) current_element->SetData(0);
