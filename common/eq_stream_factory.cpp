...
#endif
// moved these because on windows the output was delayed and causing the console
// window to look bad
#ifdef _WINDOWS
...
void EQStreamFactory::ReaderLoop() {
	...
	while (sock != -1) {
		...
		if (sock == -1) break;  // somebody closed us while we were sleeping.

		if (FD_ISSET(sock, &readset)) {
#ifdef _WINDOWS
			if ((length = recvfrom(sock, (char *)buffer, sizeof(buffer), 0,
			                       (struct sockaddr *)&from, (int *)&socklen)) <
			    2)
#else
			if ((length =
			         recvfrom(sock, buffer, 2048, 0, (struct sockaddr *)&from,
			                  (socklen_t *)&socklen)) < 2)
#endif
			{
				// What do we wanna do?
				...
				MStreams.lock();
				stream_itr = Streams.find(
				    std::make_pair(from.sin_addr.s_addr, from.sin_port));
				oldstream_itr = OldStreams.find(
				    std::make_pair(from.sin_addr.s_addr, from.sin_port));
				if (stream_itr == Streams.end() &&
				    oldstream_itr == OldStreams.end()) {
					if (buffer[1] == OP_SessionRequest) {
						EQStream *s = new EQStream(from);
						s->SetStreamType(StreamType);
						Streams[std::make_pair(from.sin_addr.s_addr,
						                       from.sin_port)] = s;
						WriterWork.Signal();
						Push(s);
						...
						OldStreams[std::make_pair(from.sin_addr.s_addr,
						                          from.sin_port)] = s;
						WriterWork.Signal();
						PushOld(s);
						// s->AddBytesRecv(length);
					...
					if (curstream != nullptr) {
						// dont bother processing incoming packets for closed
						// connections
						if (curstream->CheckClosed())
							curstream = nullptr;
						else
							curstream->PutInUse();
						MStreams.unlock();  // the in use flag prevents the
						                    // stream from being deleted while
						                    // we are using it.

						if (curstream) {
							curstream->AddBytesRecv(length);
							curstream->Process(buffer, length);
							curstream->SetLastPacketTime(
						...
						if (oldcurstream) {
							...
							oldcurstream->SetLastPacketTime(
							    Timer::GetCurrentTime());
...
void EQStreamFactory::CheckTimeout() {
	...
	std::map<std::pair<uint32, uint16>, EQOldStream *>::iterator oldstream_itr;
	for (oldstream_itr = OldStreams.begin();
	     oldstream_itr != OldStreams.end();) {
...
void EQStreamFactory::WriterLoop() {
	...
	while (sock != -1) {
		...
		MStreams.lock();
		for (auto stream_itr = Streams.begin(); stream_itr != Streams.end();
		     stream_itr++) {
			// If it's time to decay the bytes sent, then let's do it before we
			// try to write
			if (decay) stream_itr->second->Decay();

			// bullshit checking, to see if this is really happening, GDB seems
			// to think so...
			if (stream_itr->second == nullptr) {
				fprintf(stderr,
				        "ERROR: nullptr Stream encountered in "
				        "EQStreamFactory::WriterLoop for: %i:%i",
		...
		for (auto oldstream_itr = OldStreams.begin();
		     oldstream_itr != OldStreams.end(); oldstream_itr++) {
			// bullshit checking, to see if this is really happening, GDB seems
			// to think so...
			if (oldstream_itr->second == nullptr) {
				fprintf(stderr,
				        "ERROR: nullptr Stream encountered in "
				        "EQStreamFactory::WriterLoop for: %i:%i",
				        oldstream_itr->first.first,
				        oldstream_itr->first.second);
				continue;
			}

			oldstream_itr->second->CheckTimers();

			// Commented this so all streams, regardless of them having data,
			// send data out. This is so keepalive packets don't screw up the
			// data rate calculations. Slightly more CPU used. if
			// (oldstream_itr->second->HasOutgoingData()) {
