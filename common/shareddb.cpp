...
SharedDatabase::SharedDatabase() : Database() {}

SharedDatabase::SharedDatabase(const char* host, const char* user,
...
bool SharedDatabase::SetHideMe(uint32 account_id, uint8 hideme) {
	std::string query = StringFormat(
	    "UPDATE account SET hideme = %i WHERE id = %i", hideme, account_id);
...
uint8 SharedDatabase::GetGMSpeed(uint32 account_id) {
	std::string query =
	    StringFormat("SELECT gmspeed FROM account WHERE id = '%i'", account_id);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return 0;
	}
...
bool SharedDatabase::SetGMSpeed(uint32 account_id, uint8 gmspeed) {
	std::string query = StringFormat(
	    "UPDATE account SET gmspeed = %i WHERE id = %i", gmspeed, account_id);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return false;
	}
...
bool SharedDatabase::SetGMInvul(uint32 account_id, bool gminvul) {
	std::string query = StringFormat(
	    "UPDATE account SET gminvul = %i WHERE id = %i", gminvul, account_id);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return false;
	}
...
bool SharedDatabase::SetGMFlymode(uint32 account_id, uint8 flymode) {
	std::string query = StringFormat(
	    "UPDATE account SET flymode = %i WHERE id = %i", flymode, account_id);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return false;
	}
...
bool SharedDatabase::SetGMIgnoreTells(uint32 account_id, uint8 ignoretells) {
	std::string query =
	    StringFormat("UPDATE account SET ignore_tells = %i WHERE id = %i",
	                 ignoretells, account_id);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return false;
	}
...
uint32 SharedDatabase::GetTotalTimeEntitledOnAccount(uint32 AccountID) {
	...
	std::string query = StringFormat(
	    "SELECT `time_played` FROM `character_data` WHERE `account_id` = %u",
	    AccountID);
	auto results = QueryDatabase(query);
...
bool SharedDatabase::SaveInventory(uint32 char_id, const EQ::ItemInstance* inst,
                                   int16 slot_id) {
...
bool SharedDatabase::UpdateInventorySlot(uint32 char_id,
	...
	if (inst && inst->IsClassBag() &&
		...
		for (uint8 idx = EQ::invbag::SLOT_BEGIN; idx <= EQ::invbag::SLOT_END;
		     idx++) {
			const EQ::ItemInstance* baginst = inst->GetItem(idx);
			SaveInventory(char_id, baginst,
			              EQ::InventoryProfile::CalcSlotId(slot_id, idx));
		}
...
bool SharedDatabase::DeleteInventorySlot(uint32 char_id, int16 slot_id) {
	// Delete item
	std::string query = StringFormat(
	    "DELETE FROM character_inventory WHERE id = %i AND slotid = %i",
	    char_id, slot_id);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return false;
	...
	if (!EQ::InventoryProfile::SupportsContainers(slot_id)) return true;

	int16 base_slot_id =
	    EQ::InventoryProfile::CalcSlotId(slot_id, EQ::invbag::SLOT_BEGIN);
	query = StringFormat(
	    "DELETE FROM character_inventory WHERE id = %i AND slotid >= %i AND "
	    "slotid < %i",
...
bool SharedDatabase::SetStartingItems(PlayerProfile_Struct* pp,
                                      EQ::InventoryProfile* inv, uint32 si_race,
                                      uint32 si_class, uint32 si_deity,
                                      uint32 si_current_zone, char* si_name,
                                      int admin_level) {
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (slot < 0) slot = inv->FindFreeSlot(0, 0);
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (!item) {
			Log(Logs::General, Logs::Error,
			    "Warning: charid %i has an invalid item_id %i in inventory "
			    "slot %i",
		...
		if (slot_id >= EQ::invslot::CURSOR_QUEUE_BEGIN &&
		    slot_id <= EQ::invslot::CURSOR_QUEUE_END)
			put_slot_id = inv->PushCursor(*inst);
		else if (slot_id >= 3110 && slot_id <= 3179) {
			// Admins: please report any occurrences of this error
			LogError(
			    "Warning: Defunct location for item in inventory: charid={}, "
			    "item_id={}, slot_id={} .. pushing to cursor...",
			    char_id, item_id, slot_id);
			put_slot_id = inv->PushCursor(*inst);
		} else
		...
		if (put_slot_id == INVALID_INDEX) {
			LogError(
			    "Warning: Invalid slot_id for item in inventory: charid={}, "
			    "item_id={}, slot_id={}",
			    char_id, item_id, slot_id);
		}
	}
...
// Overloaded: Retrieve character inventory based on account_id and character
// name
bool SharedDatabase::GetInventory(uint32 account_id, char* name,
                                  EQ::InventoryProfile* inv) {
	// Retrieve character inventory
	std::string query = StringFormat(
	    "SELECT ci.slotid, ci.itemid, ci.charges, ci.custom_data "
	    "FROM character_inventory ci INNER JOIN character_data ch "
	    "ON ch.id = ci.id WHERE ch.name = '%s' AND ch.account_id = %i ORDER BY "
	    "ci.slotid",
	    name, account_id);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (put_slot_id == INVALID_INDEX)
			Log(Logs::General, Logs::Error,
			    "Warning: Invalid slot_id for item in inventory: name=%s, "
...
void SharedDatabase::GetItemsCount(int32& item_count, uint32& max_id) {
	...
	if (row[0]) max_id = atoi(row[0]);

	if (row[1]) item_count = atoi(row[1]);
}
...
bool SharedDatabase::LoadItems(const std::string& prefix) {
	...
	try {
		...
		items_hash = std::unique_ptr<EQ::FixedMemoryHashSet<EQ::ItemData>>(
		    new EQ::FixedMemoryHashSet<EQ::ItemData>(
		        reinterpret_cast<uint8*>(items_mmf->Get()), items_mmf->Size()));
		mutex.Unlock();
...
void SharedDatabase::LoadItems(void* data, uint32 size, int32 items,
                               uint32 max_item_id) {
	EQ::FixedMemoryHashSet<EQ::ItemData> hash(reinterpret_cast<uint8*>(data),
	                                          size, items, max_item_id);
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		item.NoRent =
		    disable_no_rent
		        ...
		        : static_cast<uint8>(std::stoul(row[ItemField::norent]));
		item.NoDrop =
		    disable_no_drop
		        ? static_cast<uint8>(255)
		        : static_cast<uint8>(std::stoul(row[ItemField::nodrop]));
		item.QuestItemFlag =
		    std::stoi(row[ItemField::questitemflag]) ? true : false;
		item.Tradeskills =
		    std::stoi(row[ItemField::tradeskills]) ? true : false;
		item.Stackable_ =
		    static_cast<int8>(std::stoi(row[ItemField::stackable]));
		item.Stackable =
		    (std::stoi(row[ItemField::stackable]) == 1) ? true : false;

		// Type
		item.ItemType =
		    static_cast<uint8>(std::stoul(row[ItemField::itemtype]));

		// Miscellaneous
		item.Light = static_cast<int8>(std::stoi(row[ItemField::light]));
		item.MaxCharges =
		    static_cast<int8>(std::stoi(row[ItemField::maxcharges]));
		item.Size = static_cast<uint8>(std::stoul(row[ItemField::size]));
		item.Soulbound =
		    static_cast<int8>(std::stoi(row[ItemField::soulbound]));
		item.StackSize =
		    static_cast<int16>(std::stoul(row[ItemField::stacksize]));
		...
		item.Material =
		    static_cast<uint8>(std::stoul(row[ItemField::material]));

		// Resists
		item.CR = static_cast<int8>(
		    EQ::Clamp(std::stoi(row[ItemField::cr]), -128, 127));
		item.DR = static_cast<int8>(
		    EQ::Clamp(std::stoi(row[ItemField::dr]), -128, 127));
		item.FR = static_cast<int8>(
		    EQ::Clamp(std::stoi(row[ItemField::fr]), -128, 127));
		item.MR = static_cast<int8>(
		    EQ::Clamp(std::stoi(row[ItemField::mr]), -128, 127));
		item.PR = static_cast<int8>(
		    EQ::Clamp(std::stoi(row[ItemField::pr]), -128, 127));

		// Stats
		item.AAgi = static_cast<int8>(
		    EQ::Clamp(std::stoi(row[ItemField::aagi]), -128, 127));
		item.ACha = static_cast<int8>(
		    EQ::Clamp(std::stoi(row[ItemField::acha]), -128, 127));
		item.ADex = static_cast<int8>(
		    EQ::Clamp(std::stoi(row[ItemField::adex]), -128, 127));
		item.AInt = static_cast<int8>(
		    EQ::Clamp(std::stoi(row[ItemField::aint]), -128, 127));
		item.ASta = static_cast<int8>(
		    EQ::Clamp(std::stoi(row[ItemField::asta]), -128, 127));
		item.AStr = static_cast<int8>(
		    EQ::Clamp(std::stoi(row[ItemField::astr]), -128, 127));
		item.AWis = static_cast<int8>(
		    EQ::Clamp(std::stoi(row[ItemField::awis]), -128, 127));
		...
		// Bane Damage
		item.BaneDmgAmt =
		    static_cast<uint8>(std::stoul(row[ItemField::banedmgamt]));
		item.BaneDmgBody = std::stoi(row[ItemField::banedmgbody]);
		item.BaneDmgRace = std::stoi(row[ItemField::banedmgrace]);

		// Elemental Damage
		item.ElemDmgType =
		    static_cast<uint8>(std::stoul(row[ItemField::elemdmgtype]));
		item.ElemDmgAmt =
		    static_cast<uint8>(std::stoul(row[ItemField::elemdmgamt]));
		...
		item.RecLevel =
		    static_cast<uint8>(std::stoul(row[ItemField::reclevel]));
		item.RecSkill =
		    static_cast<uint8>(std::stoul(row[ItemField::recskill]));
		item.ReqLevel =
		    static_cast<uint8>(std::stoul(row[ItemField::reqlevel]));
		...
		item.BagSlots = static_cast<uint8>(
		    EQ::Clamp(std::stoi(row[ItemField::bagslots]), 0, 10));
		item.BagType = static_cast<uint8>(std::stoul(row[ItemField::bagtype]));
		item.BagWR = static_cast<uint8>(
		    EQ::Clamp(std::stoi(row[ItemField::bagwr]), 0, 100));

		// Bard Effect
		item.Bard.Effect = std::stoi(row[ItemField::bardeffect]);
		item.Bard.Type = std::stoi(row[ItemField::bardtype]);
		item.Bard.Level =
		    static_cast<uint8>(std::stoul(row[ItemField::bardlevel]));
		item.Bard.Level2 =
		    static_cast<uint8>(std::stoul(row[ItemField::bardlevel2]));

		// Book
		item.Book = static_cast<uint8>(std::stoul(row[ItemField::book]));
		item.BookType = std::stoul(row[ItemField::booktype]);
		strn0cpy(item.Filename, row[ItemField::filename],
		...
		item.Click.Level =
		    static_cast<uint8>(std::stoul(row[ItemField::clicklevel]));
		item.Click.Level2 =
		    static_cast<uint8>(std::stoul(row[ItemField::clicklevel2]));
		...
		item.Focus.Level =
		    static_cast<uint8>(std::stoul(row[ItemField::focuslevel]));
		item.Focus.Level2 =
		    static_cast<uint8>(std::stoul(row[ItemField::focuslevel2]));

		// Proc Effect
		item.Proc.Effect = std::stoi(row[ItemField::proceffect]);
		item.Proc.Type = std::stoi(row[ItemField::proctype]);
		item.Proc.Level =
		    static_cast<uint8>(std::stoul(row[ItemField::proclevel]));
		item.Proc.Level2 =
		    static_cast<uint8>(std::stoul(row[ItemField::proclevel2]));
		...
		item.Scroll.Level =
		    static_cast<uint8>(std::stoul(row[ItemField::scrolllevel]));
		item.Scroll.Level2 =
		    static_cast<uint8>(std::stoul(row[ItemField::scrolllevel2]));

		// Worn Effect
		item.Worn.Effect = std::stoi(row[ItemField::worneffect]);
		item.Worn.Type = std::stoi(row[ItemField::worntype]);
		item.Worn.Level =
		    static_cast<uint8>(std::stoul(row[ItemField::wornlevel]));
		item.Worn.Level2 =
		    static_cast<uint8>(std::stoul(row[ItemField::wornlevel2]));
...
std::string SharedDatabase::GetBook(const char* txtfile) {
	...
	std::string query =
	    StringFormat("SELECT txtfile FROM books WHERE name = '%s'", txtfile2);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		txtout.assign(" ", 1);
		return txtout;
...
void SharedDatabase::LoadNPCFactionLists(void* data, uint32 size,
                                         uint32 list_count, uint32 max_lists) {
	EQ::FixedMemoryHashSet<NPCFactionList> hash(reinterpret_cast<uint8*>(data),
	                                            size, list_count, max_lists);
	NPCFactionList faction;

	const std::string query =
	    "SELECT npc_faction.id, npc_faction.primaryfaction, "
	    "npc_faction.ignore_primary_assist, "
	    "npc_faction_entries.faction_id, npc_faction_entries.value, "
	    "npc_faction_entries.npc_value, "
	    "npc_faction_entries.temp FROM npc_faction LEFT JOIN "
	    "npc_faction_entries "
	    "ON npc_faction.id = npc_faction_entries.npc_faction_id ORDER BY "
	    "npc_faction_entries.npc_faction_id, npc_faction_entries.sort_order;";
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return;
	}
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (current_entry >= MAX_NPC_FACTIONS) continue;
		...
		faction.factionnpcvalue[current_entry] =
		    static_cast<int8>(atoi(row[5]));
		faction.factiontemp[current_entry] = static_cast<uint8>(atoi(row[6]));
		++current_entry;
	}

	if (current_id != 0) hash.insert(current_id, faction);
}
...
bool SharedDatabase::LoadNPCFactionLists(const std::string& prefix) {
	...
	try {
		...
		faction_hash = std::unique_ptr<EQ::FixedMemoryHashSet<NPCFactionList>>(
		    new EQ::FixedMemoryHashSet<NPCFactionList>(
		        reinterpret_cast<uint8*>(faction_mmf->Get()),
		        faction_mmf->Size()));
		mutex.Unlock();
	} catch (std::exception& ex) {
...
EQ::ItemInstance* SharedDatabase::CreateItem(uint32 item_id, int8 charges) {
	...
	if (item) {
		...
		if (inst == nullptr) {
			LogError(
			    "Error: valid item data returned a null reference for "
			    "EQ::ItemInstance creation in SharedDatabase::CreateItem()");
			LogError("Item Data = ID: {0}, Name: {1}, Charges: {2}", item->ID,
			         item->Name, charges);
			return nullptr;
		}
	}
...
// Create appropriate ItemInst class
EQ::ItemInstance* SharedDatabase::CreateItem(const EQ::ItemData* item,
                                             int8 charges) {
	EQ::ItemInstance* inst = nullptr;
	if (item) {
		inst = CreateBaseItem(item, charges);
...
EQ::ItemInstance* SharedDatabase::CreateBaseItem(const EQ::ItemData* item,
                                                 int8 charges) {
	EQ::ItemInstance* inst = nullptr;
	if (item) {
		// if maxcharges is -1 that means it is an unlimited use item.
		// set it to 1 charge so that it is usable on creation
		if (charges == 0 && item->MaxCharges == -1) charges = 1;
		...
		if (inst == nullptr) {
			LogError(
			    "Error: valid item data returned a null reference for "
			    "EQ::ItemInstance creation in "
			    "SharedDatabase::CreateBaseItem()");
			LogError("Item Data = ID: {0}, Name: {1}, Charges: {2}", item->ID,
			         item->Name, charges);
			return nullptr;
		}
...
int32 SharedDatabase::DeleteStalePlayerCorpses() {
	...
	if (RuleB(Zone, EnableShadowrest)) {
		std::string query = StringFormat(
		    "UPDATE `character_corpses` SET `is_buried` = 1 WHERE `is_buried` "
		    "= 0 AND "
		    "(UNIX_TIMESTAMP() - UNIX_TIMESTAMP(time_of_death)) > %d AND NOT "
		    "time_of_death = 0",
		...
		std::string sr_query = StringFormat(
		    "DELETE FROM `character_corpses` WHERE `is_buried` = 1 AND "
		    "(UNIX_TIMESTAMP() - UNIX_TIMESTAMP(time_of_death)) > %d "
		    "AND NOT time_of_death = 0",
		    (RuleI(Character, CorpseDecayTimeMS) / 1000) * 2);
		auto sr_results = QueryDatabase(sr_query);
		if (!sr_results.Success()) return -1;

		rows_affected += sr_results.RowsAffected();

	} else {
		std::string query = StringFormat(
		    "DELETE FROM `character_corpses` WHERE (UNIX_TIMESTAMP() - "
		    "UNIX_TIMESTAMP(time_of_death)) > %d "
		    "AND NOT time_of_death = 0",
		    (RuleI(Character, CorpseDecayTimeMS) / 1000));
		auto results = QueryDatabase(query);
		if (!results.Success()) return -1;
	...
	if (RuleB(Character, UsePlayerCorpseBackups)) {
		std::string cb_query =
		    StringFormat("SELECT id FROM `character_corpses_backup`");
		...
		for (auto row = cb_results.begin(); row != cb_results.end(); ++row) {
			...
			std::string cbd_query = StringFormat(
			    ...
			    "SELECT COUNT(*) from character_corpse_items_backup where "
			    "corpse_id = %d) "
			    " = 0",
			    corpse_id, corpse_id);
			auto cbd_results = QueryDatabase(cbd_query);
			if (!cbd_results.Success()) return -1;
...
bool SharedDatabase::UpdateInjectedCommandSettings(
	...
	if (injected.size()) {
		...
		LogInfo("[{0}] New Command(s) Added", injected.size());
	}

	return true;
}

bool SharedDatabase::UpdateOrphanedCommandSettings(
    const std::vector<std::string>& orphaned) {
	if (orphaned.size()) {
		std::string query = fmt::format(
		    "DELETE FROM `command_settings` WHERE `command` IN ({})",
		    Strings::ImplodePair(",", std::pair<char, char>('\'', '\''),
		                         orphaned));
		...
		LogInfo("{} Orphaned Command{} Deleted", orphaned.size(),
		        (orphaned.size() == 1 ? "" : "s"));
	}
...
void SharedDatabase::LoadSkillCaps(void* data) {
	...
	const std::string query =
	    "SELECT skillID, class, level, cap FROM skill_caps ORDER BY skillID, "
	    "class, level";
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		LogError("Error loading skill caps from database: {0}",
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (skillID >= skill_count || class_ >= class_count ||
		    level >= level_count)
			continue;

		uint32 index =
		    (((class_ * skill_count) + skillID) * level_count) + level;
		skill_caps_table[index] = cap;
	}
}

uint16 SharedDatabase::GetSkillCap(uint8 Class_, EQ::skills::SkillType Skill,
                                   uint8 Level) {
	if (!skill_caps_mmf) {
		return 0;
	}
	...
	uint32 skill_count = EQ::skills::HIGHEST_SKILL + 1;
	uint32 level_count = HARD_LEVEL_CAP + 1;
	if (Class_ > class_count || static_cast<uint32>(Skill) > skill_count ||
	    Level > level_count) {
		return 0;
	}
	...
	uint32 index =
	    ((((Class_ - 1) * skill_count) + Skill) * level_count) + Level;
	uint16* skill_caps_table = reinterpret_cast<uint16*>(skill_caps_mmf->Get());
	return skill_caps_table[index];
}

uint8 SharedDatabase::GetTrainLevel(uint8 Class_, EQ::skills::SkillType Skill,
                                    uint8 Level) {
	if (!skill_caps_mmf) {
		return 0;
	}
	...
	if (Level > static_cast<uint8>(SkillMaxLevel)) {
		...
		uint16* skill_caps_table =
		    reinterpret_cast<uint16*>(skill_caps_mmf->Get());
...
void SharedDatabase::LoadDamageShieldTypes(SPDat_Spell_Struct* sp,
                                           int32 iMaxSpellID) {
	std::string query = StringFormat(
...
bool SharedDatabase::LoadSpells(const std::string& prefix, int32* records,
...
void SharedDatabase::LoadSpells(void* data, int max_spells) {
	*(uint32*)data = max_spells;
	SPDat_Spell_Struct* sp =
	    reinterpret_cast<SPDat_Spell_Struct*>((char*)data + sizeof(uint32));
	...
	if (results.ColumnCount() < SPELL_LOAD_FIELD_COUNT) {
		LogSpells(
		    "Fatal error loading spells: Spell field count < "
		    "SPELL_LOAD_FIELD_COUNT([{0}])",
		    SPELL_LOAD_FIELD_COUNT);
		return;
	}
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		strn0cpy(sp[tempid].teleport_zone, row[3],
		         sizeof(sp[tempid].teleport_zone));
		strn0cpy(sp[tempid].you_cast, row[4], sizeof(sp[tempid].you_cast));
		strn0cpy(sp[tempid].other_casts, row[5],
		         sizeof(sp[tempid].other_casts));
		strn0cpy(sp[tempid].cast_on_you, row[6],
		         sizeof(sp[tempid].cast_on_you));
		strn0cpy(sp[tempid].cast_on_other, row[7],
		         sizeof(sp[tempid].cast_on_other));
		strn0cpy(sp[tempid].spell_fades, row[8],
		         sizeof(sp[tempid].spell_fades));
		...
		for (y = 0; y < 4; y++) sp[tempid].components[y] = atoi(row[58 + y]);
		...
		if (tmp_skill < 0 || tmp_skill > EQ::skills::HIGHEST_SKILL)
			sp[tempid].skill = EQ::skills::SkillBegging;
		/* not much better we can do. */  // can probably be changed to
		                                  // client-based 'SkillNone'
		                                  // once activated
		else
		...
		for (y = 0; y < 16; y++) sp[tempid].deities[y] = atoi(row[125 + y]);
		...
		for (r = 0; r < PLAYER_CLASS_COUNT; r++)
			if (sp[tempid].classes[r] < min) min = sp[tempid].classes[r];

		// if we can't cast the spell return 0
		// just so it wont screw up calculations used in other areas of the code
		// seen 127, 254, 255
		if (min < 127) sp[tempid].min_castinglevel = min;

		if (sp[tempid].classes[BARD - 1] < 255) sp[tempid].bardsong = true;

		if (sp[tempid].RecourseLink != 0 && sp[tempid].RecourseLink != 1 &&
		    sp[tempid].RecourseLink != 0xFFFFFFFF)
...
int SharedDatabase::GetMaxBaseDataLevel() {
	...
	if (results.RowCount() == 0) return -1;
...
void SharedDatabase::LoadBaseData(void* data, int max_level) {
	...
	const std::string query =
	    "SELECT * FROM base_data ORDER BY level, class ASC";
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return;
	}
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (lvl >= max_level) {
			LogError(
			    "Non fatal error: base_data.level >= max_level, ignoring.");
			continue;
		}
		...
		BaseDataStruct* bd = reinterpret_cast<BaseDataStruct*>(
		    base_ptr +
		    (((16 * (lvl - 1)) + (cl - 1)) * sizeof(BaseDataStruct)));
...
void SharedDatabase::GetLootTableInfo(uint32& loot_table_count,
                                      uint32& max_loot_table,
                                      uint32& loot_table_entries) {
	loot_table_count = 0;
	max_loot_table = 0;
	loot_table_entries = 0;
	const std::string query =
	    "SELECT COUNT(*), MAX(id), (SELECT COUNT(*) FROM loottable_entries) "
	    "FROM loottable";
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return;
...
void SharedDatabase::GetLootDropInfo(uint32& loot_drop_count,
	...
	const std::string query =
	    "SELECT COUNT(*), MAX(id), (SELECT COUNT(*) FROM lootdrop_entries) "
	    "FROM lootdrop";
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return;
...
void SharedDatabase::LoadLootTables(void* data, uint32 size) {
	EQ::FixedMemoryVariableHashSet<LootTable_Struct> hash(
	    reinterpret_cast<uint8*>(data), size);

	uint8 loot_table[sizeof(LootTable_Struct) +
	                 (sizeof(LootTableEntries_Struct) * 128)];
	LootTable_Struct* lt = reinterpret_cast<LootTable_Struct*>(loot_table);

	const std::string query =
	    "SELECT loottable.id, loottable.mincash, loottable.maxcash, "
	    "loottable.avgcoin, "
	    "loottable_entries.lootdrop_id, loottable_entries.multiplier, "
	    "loottable_entries.droplimit, "
	    "loottable_entries.mindrop, loottable_entries.probability, "
	    "loottable_entries.multiplier_min FROM "
	    "loottable LEFT JOIN loottable_entries ON loottable.id = "
	    "loottable_entries.loottable_id ORDER BY id";
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return;
	}
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (id != current_id) {
			if (current_id != 0)
				hash.insert(
				    current_id, loot_table,
				    (sizeof(LootTable_Struct) +
		...
		if (current_entry > 128) continue;

		if (!row[4]) continue;

		lt->Entries[current_entry].lootdrop_id =
		    static_cast<uint32>(atoul(row[4]));
		lt->Entries[current_entry].multiplier =
		    static_cast<uint8>(atoi(row[5]));
		...
		lt->Entries[current_entry].probability =
		    static_cast<float>(atof(row[8]));
		lt->Entries[current_entry].multiplier_min =
		    static_cast<uint8>(atoi(row[9]));
	...
	if (current_id != 0)
		hash.insert(current_id, loot_table,
		            (sizeof(LootTable_Struct) +
		             (sizeof(LootTableEntries_Struct) * lt->NumEntries)));
}
...
void SharedDatabase::LoadLootDrops(void* data, uint32 size) {
	EQ::FixedMemoryVariableHashSet<LootDrop_Struct> hash(
	    reinterpret_cast<uint8*>(data), size);
	uint8 loot_drop[sizeof(LootDrop_Struct) +
	                (sizeof(LootDropEntries_Struct) * 1260)];
	LootDrop_Struct* ld = reinterpret_cast<LootDrop_Struct*>(loot_drop);

	const std::string query =
	    "SELECT lootdrop.id, lootdrop_entries.item_id, "
	    "lootdrop_entries.item_charges, "
	    "lootdrop_entries.equip_item, lootdrop_entries.chance, "
	    "lootdrop_entries.minlevel, "
	    "lootdrop_entries.maxlevel, lootdrop_entries.multiplier FROM lootdrop "
	    "JOIN lootdrop_entries "
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (id != current_id) {
			if (current_id != 0)
				hash.insert(
				    current_id, loot_drop,
				    (sizeof(LootDrop_Struct) +
		...
		if (current_entry >= 1260) continue;

		ld->Entries[current_entry].item_id = static_cast<uint32>(atoul(row[1]));
		ld->Entries[current_entry].item_charges =
		    static_cast<int8>(atoi(row[2]));
		ld->Entries[current_entry].equip_item =
		    static_cast<uint8>(atoi(row[3]));
		...
		ld->Entries[current_entry].multiplier =
		    static_cast<uint8>(atoi(row[7]));
	...
	if (current_id != 0)
		hash.insert(current_id, loot_drop,
		            (sizeof(LootDrop_Struct) +
		             (sizeof(LootDropEntries_Struct) * ld->NumEntries)));
}
...
bool SharedDatabase::LoadLoot(const std::string& prefix) {
	...
	try {
		...
		loot_table_mmf = std::unique_ptr<EQ::MemoryMappedFile>(
		    new EQ::MemoryMappedFile(file_name_lt));
		loot_table_hash =
		    std::unique_ptr<EQ::FixedMemoryVariableHashSet<LootTable_Struct>>(
		        new EQ::FixedMemoryVariableHashSet<LootTable_Struct>(
		            reinterpret_cast<uint8*>(loot_table_mmf->Get()),
		            loot_table_mmf->Size()));
		std::string file_name_ld =
		    Config->SharedMemDir + prefix + std::string("loot_drop");
		loot_drop_mmf = std::unique_ptr<EQ::MemoryMappedFile>(
		    new EQ::MemoryMappedFile(file_name_ld));
		loot_drop_hash =
		    std::unique_ptr<EQ::FixedMemoryVariableHashSet<LootDrop_Struct>>(
		        new EQ::FixedMemoryVariableHashSet<LootDrop_Struct>(
...
const LootTable_Struct* SharedDatabase::GetLootTable(uint32 loottable_id) {
	if (!loot_table_hash) return nullptr;
...
const LootDrop_Struct* SharedDatabase::GetLootDrop(uint32 lootdrop_id) {
	if (!loot_drop_hash) return nullptr;
...
bool SharedDatabase::VerifyToken(std::string token, int& status) {
	...
	std::string query = StringFormat(
	    "SELECT status FROM tokens WHERE token='%s'", token.c_str());
	auto results = QueryDatabase(query);
