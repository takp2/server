...
class Strings {
   public:
	...
	static std::string Join(const std::vector<std::string>& ar,
	                        const std::string& delim);
	static std::string MillisecondsToTime(int duration);
	static std::string Money(uint32 platinum, uint32 gold = 0,
	                         uint32 silver = 0, uint32 copper = 0);
	...
	static std::vector<std::string> Wrap(std::vector<std::string>& src,
	                                     std::string character);
	static void FindReplace(std::string& string_subject,
	...
	template <typename T>
	static std::string ImplodePair(const std::string& glue,
		...
		for (const T& src_iter : src) {
			oss << encapsulation.first << src_iter << encapsulation.second
			    << glue;
		}
...
// _WIN32 builds require that #include<fmt/format.h> be included in whatever
// code file the invocation is made from (no header files)
template <typename T1, typename T2>
std::vector<std::string> join_pair(const std::string& glue,
                                   const std::pair<char, char>& encapsulation,
	...
	for (const std::pair<T1, T2>& src_iter : src) {
		output.push_back(
		    ...
		    fmt::format("{}{}{}{}{}{}{}", encapsulation.first, src_iter.first,
		                encapsulation.second, glue, encapsulation.first,
		                src_iter.second, encapsulation.second));
	}
...
std::vector<std::string> join_tuple(
    const std::string& glue, const std::pair<char, char>& encapsulation,
	...
	for (const std::tuple<T1, T2, T3, T4>& src_iter : src) {
		output.push_back(
		    ...
		    fmt::format("{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}", encapsulation.first,
		                std::get<0>(src_iter), encapsulation.second, glue,
		                encapsulation.first, std::get<1>(src_iter),
		                encapsulation.second, glue, encapsulation.first,
		                std::get<2>(src_iter), encapsulation.second, glue,
		                encapsulation.first, std::get<3>(src_iter),
