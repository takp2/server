...
bool EQ::skills::IsBasicTradeskill(SkillType skill) {
	// These tradeskill all cap at 250 so it's useful to use if mod is presented
	// for these. Mod is capped at 15 after 250 at least for makepoison
	switch (skill) {
		case SkillMakePoison:
		case SkillTinkering:
		case SkillBaking:
...
bool EQ::skills::IsSpecializedSkill(SkillType skill) {
	// this could be a simple if, but if this is more portable if any IDs change
	// (probably won't) or any other specialized are added (also unlikely)
	switch (skill) {
...
bool EQ::skills::IsMeleeWeaponSkill(SkillType skill) {
	switch (skill) {
		...
		case SkillRoundKick:
			// case SkillTigerClaw:		// note: tiger claw is commented out
			// because many spells are flagged as tiger claw (for whatever
			// reason)
			return true;
		default:
			return false;
...
// returns the base damage value for special skills, except backstab.  taken
// from decompiles.  accurate to PoP era.
...
const std::map<EQ::skills::SkillType, std::string>&
EQ::skills::GetSkillTypeMap() {
...
uint16 EQ::SkillProfile::GetSkill(int skill_id) {
	if (skill_id < 0 || skill_id >= PACKET_SKILL_ARRAY_SIZE) return 0;
