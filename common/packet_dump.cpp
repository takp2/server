...
void DumpPacketHex(const uchar* buf, uint32 size, uint32 cols, uint32 skip) {
	...
	for (i = skip; i < size; i++) {
		if ((i - skip) % cols == 0) {
			if (i != skip) std::cout << " | " << ascii << std::endl;
		...
		//(int)buf[i] << " "; // unknown intent [CODEBUG]
	}
...
std::string DumpPacketHexToString(const uchar* buf, uint32 size, uint32 cols,
                                  uint32 skip) {
...
void DumpPacket(const ServerPacket* pack, bool iShowInfo) {
	if (iShowInfo) {
		std::cout << "Dumping ServerPacket: 0x" << std::hex << std::setfill('0')
		          << std::setw(4) << pack->opcode << std::dec;
...
void DumpPacketBin(uint8 data) { DumpPacketBin((uchar*)&data, sizeof(uint8)); }
...
void DumpPacketBin(const void* iData, uint32 len) {
		...
		if (k % 4 == 0) {
			if (k != 0) {
				std::cout << " | " << std::hex << std::setw(2)
				          << std::setfill('0') << (int)data[k - 4] << std::dec;
				std::cout << " " << std::hex << std::setw(2)
				          << std::setfill('0') << (int)data[k - 3] << std::dec;
				std::cout << " " << std::hex << std::setw(2)
				          << std::setfill('0') << (int)data[k - 2] << std::dec;
				std::cout << " " << std::hex << std::setw(2)
				          << std::setfill('0') << (int)data[k - 1] << std::dec;
				std::cout << std::endl;
			}
	...
	std::cout << " | " << std::hex << std::setw(2) << std::setfill('0')
	          << (int)data[k - 4] << std::dec;
	if (tmp > 1)
		std::cout << " " << std::hex << std::setw(2) << std::setfill('0')
		          << (int)data[k - 3] << std::dec;
	if (tmp > 2)
		std::cout << " " << std::hex << std::setw(2) << std::setfill('0')
		          << (int)data[k - 2] << std::dec;
	if (tmp > 3)
		std::cout << " " << std::hex << std::setw(2) << std::setfill('0')
		          << (int)data[k - 1] << std::dec;
	std::cout << std::endl;
}
