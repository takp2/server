...
bool Client::Process() {
	...
	while (app) {
		...
		if (server.options.IsDumpInPacketsOn()) {
			LogDebug("[Size: {0}] [{1}]", app->size,
			         DumpPacketToString(app).c_str());
		}
		...
		switch (app->GetOpcode()) {
			...
			case OP_LoginOSX: {
				std::string client;
				std::string check =
				    DumpPacketToRawString(app->pBuffer, app->Size());

				if (check.find("eqworld-52.989studios.com") !=
				    std::string::npos) {
			...
			case OP_LoginUnknown1:  // Seems to be related to world status in
			                        // older clients; we use our own logic for
			                        // that though.
			{
			...
			default: {
				if (LogSys.log_settings[Logs::PacketClientServerUnhandled]
				        .is_category_enabled == 1) {
					char dump[64];
					app->build_header_dump(dump);
					LogError(
					    "Recieved unhandled application packet from the "
					    "client: [{0}]",
					    dump);
				}
			}
...
void Client::Handle_SessionReady(const char* data, unsigned int size) {
	...
	if (version != cv_old) {
		...
		if (mode == (unsigned int)lm_from_world) {
			LogError(
			    "Session ready indicated logged in from world(unsupported "
...
void Client::Handle_Login(const char* data, unsigned int size,
	...
	if (version != cv_old) {
		...
		string error = "Unauthorized client from " + string(inet_ntoa(in)) +
		               " , exiting them.";
	...
	std::transform(allowOSX.begin(), allowOSX.end(), allowOSX.begin(),
	               ::toupper);
	std::transform(allowPC.begin(), allowPC.end(), allowPC.begin(), ::toupper);
	std::transform(allowPCT.begin(), allowPCT.end(), allowPCT.begin(),
	               ::toupper);
	...
	if (!allowedClient) {
		string error = "Unauthorized client from " + string(inet_ntoa(in)) +
		               " using client < " + client + " > , exiting them.";
		LogError(error.c_str());
		return;
	}
	...
	if (client == "OSX") {
		...
		if (size < strlen("eqworld-52.989studios.com") + 1) return;

		// Get rid of that 989 studios part of the string, plus remove null term
		// zero.
		string userpass =
		    ourdata.substr(0, ourdata.find("eqworld-52.989studios.com") - 1);
	...
	if (!server.db->GetLoginDataFromAccountName(username, d_pass_hash,
	                                            d_account_id)) {
		LogError("Error logging in, user {0} does not exist in the database.",
		         username.c_str());
		LogError("platform : {} , username : {} does not exist", platform,
		         username);
		if (server.options.CanAutoCreateAccounts()) {
			LogInfo("platform : {} , username : {} is created", platform,
			        username);
			server.db->CreateLoginData(username.c_str(), userandpass,
			                           d_account_id);

		} else {
			FatalError(
			    "Account does not exist and auto creation is not enabled.");
			return;
		}
		result = false;
	} else {
	...
	if (result) {
		if (!sentsessioninfo) {
			LogInfo("username : {} logging on platform : {} is a success",
			        username, platform);
...
void Client::Handle_Play(const char* data) {
	if (status != cs_logged_in) {
		LogError(
		    "Client sent a play request when they either were not logged in, "
		    "discarding.");
		return;
	}

	if (data) {
		server.server_manager->SendOldUserToWorldRequest(
		    data, account_id, connection->GetRemoteIP());
	}
}

void Client::SendServerListPacket() {
	EQApplicationPacket* outapp =
	    server.server_manager->CreateOldServerListPacket(this);

	if (server.options.IsDumpOutPacketsOn()) {
		LogInfo("[Size: {0}] [{1}]", outapp->size,
