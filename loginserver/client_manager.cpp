...
void ClientManager::Process() {
	...
	if (old_stream) {
		while (oldcur) {
			...
			LogInfo("New client connection from {0}:{1}", inet_ntoa(in),
			        ntohs(oldcur->GetRemotePort()));
	...
	while (iter != clients.end()) {
		if ((*iter)->Process() == false) {
			Log(Logs::General, Logs::LoginServer,
			    "Client had a fatal error and had to be removed from the "
			    "login.");
			delete (*iter);
...
void ClientManager::RemoveExistingClient(unsigned int account_id) {
	...
	while (iter != clients.end()) {
		if ((*iter)->GetAccountID() == account_id) {
			Log(Logs::General, Logs::LoginServer,
			    "Client attempting to log in and existing client already "
			    "logged in, removing existing client.");
			delete (*iter);
			iter = clients.erase(iter);
		} else {
			++iter;
...
Client *ClientManager::GetClient(unsigned int account_id) {
	...
	if (count > 1) {
		Log(Logs::General, Logs::Error,
		    "More than one client with a given account_id existed in the "
		    "client list.");
	}
	return cur;
}
