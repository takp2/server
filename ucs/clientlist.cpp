...
void Client::SendUptime() {
	...
	auto message = fmt::format(
	    "UCS has been up for {:02}d {:02}h {:02}m {:02}s", d, h, m, s);
...
static void ProcessSetMessageStatus(std::string SetMessageCommand) {
	...
	std::string::size_type NumStart =
	    SetMessageCommand.find_first_of("123456789");

	while (NumStart != std::string::npos) {
		std::string::size_type NumEnd =
		    SetMessageCommand.find_first_of(" ", NumStart);
		...
		MessageNumber =
		    atoi(SetMessageCommand.substr(NumStart, NumEnd - NumStart).c_str());
...
static void ProcessCommandBuddy(Client *c, std::string Buddy) {
	...
	if (Buddy.substr(0, 1) == "-") SubAction = 0;
...
Client::Client(EQStream *eqs) {
	...
	for (auto &elem : JoinedChannels) elem = nullptr;
...
void Clientlist::CLClearStaleConnections() {
	...
	while (Iterator != ClientChatConnections.end()) {
		if ((*Iterator)->GetStale()) {
			LogInfo("Removing stale connection for [{0}]",
...
void Clientlist::CheckForStaleConnections(Client *c) {
	...
	for (Iterator = ClientChatConnections.begin();
	     Iterator != ClientChatConnections.end(); ++Iterator) {
		if (((*Iterator) != c) && !(*Iterator)->GetStale() &&
		    ((c->GetName() == (*Iterator)->GetName()) &&
		     (c->GetConnectionType() == (*Iterator)->GetConnectionType()))) {
			LogInfo("Flagging old connection as stale for [{0}]",
			        c->GetName().c_str());
			(*Iterator)->SetStale();
		}
	}
...
void Clientlist::Process() {
	...
	while ((eqs = chatsf->Pop())) {
		...
		LogInfo("New Client UDP connection from [{0}]:[{1}]",
		        long2ip(eqs->GetRemoteIP()).c_str(),
	...
	while (it != ClientChatConnections.end()) {
		...
		while (KeyValid && !(*it)->GetForceDisconnect() &&
		       (app = (*it)->ClientStream->PopPacket())) {
			...
			switch (opcode) {
				case OP_ChatLogin: {
					...
					ConnectionTypeIndicator =
					    VARSTRUCT_DECODE_TYPE(char, PacketBuffer);
					...
					// Strip off the SOE.EQ.<shortname>.
					//
					std::string::size_type LastPeriod =
					    ChatlistString.find_last_of(".");
					...
					/*if (!database.VerifyMailKey(CharacterName,
					(*it)->ClientStream->GetRemoteIP(), Key)) {
					    Log(Logs::Detail, Logs::UCSServer,
					        "Chat Key for %s does not match, closing
					connection.", Chatlist); KeyValid = false; break;
					}*/

					(*it)->SetAccountID(
					    database.FindAccount(CharacterName.c_str(), (*it)));
		...
		if (!KeyValid || (*it)->GetForceDisconnect()) {
			LogInfo(
			    "Force disconnecting client: [{0}]:[{1}], KeyValid=[{2}], "
			    "GetForceDisconnect()=[{3}]",
...
void Clientlist::ProcessOPChatCommand(Client *c, std::string CommandString) {
	...
	if (Space != std::string::npos) {
		...
		std::string::size_type ParametersStart =
		    CommandString.find_first_not_of(" ", Space);
...
void Client::AddCharacter(int CharID, const char *CharacterName, int Level,
	...
	LogDebug("Adding character [{0}] with ID [{1}] for [{2}]", CharacterName,
	         CharID, GetName().c_str());
...
void Client::AddToChannelList(ChatChannel *JoinedChannel) {
	...
	for (int i = 0; i < MAX_JOINED_CHANNELS; i++)
		if (JoinedChannels[i] == nullptr) {
			...
			LogDebug("Added Channel [{0}] to slot [{1}] for [{2}]",
			         JoinedChannel->GetName().c_str(), i + 1,
			         GetName().c_str());
			return;
		}
}
...
void Client::RemoveFromChannelList(ChatChannel *JoinedChannel) {
	for (int i = 0; i < MAX_JOINED_CHANNELS; i++)
		if (JoinedChannels[i] == JoinedChannel) {
			// Shuffle all the channels down. Client likes them all nice and
			// consecutive.
			//
...
void Client::JoinChannels(std::string ChannelNameList) {
	...
	LogInfo("Client: [{0}] joining channels [{1}]", GetName().c_str(),
	        ChannelNameList.c_str());
	...
	while (CurrentPos != std::string::npos) {
		...
		std::string::size_type Comma =
		    ChannelNameList.find_first_of(", ", CurrentPos);

		if (Comma == std::string::npos) {
			ChatChannel *JoinedChannel = ChannelList->AddClientToChannel(
			    ChannelNameList.substr(CurrentPos), this);

			if (JoinedChannel) AddToChannelList(JoinedChannel);

			break;
		}

		ChatChannel *JoinedChannel = ChannelList->AddClientToChannel(
		    ChannelNameList.substr(CurrentPos, Comma - CurrentPos), this);
	...
	for (int i = 0; i < MAX_JOINED_CHANNELS; i++) {
		if (JoinedChannels[i] != nullptr) {
			...
			JoinedChannelsList =
			    JoinedChannelsList + JoinedChannels[i]->GetName();

			sprintf(tmp, "%i=%s(%i)", i + 1,
			        JoinedChannels[i]->GetName().c_str(),
			        JoinedChannels[i]->MemberCount(Status));
	...
	auto outapp =
	    new EQApplicationPacket(OP_Chat, JoinedChannelsList.length() + 1);
	...
	if (ChannelCount == 0) ChannelMessage = "You are not on any channels.";

	outapp =
	    new EQApplicationPacket(OP_ChannelMessage, ChannelMessage.length() + 3);
...
void Client::LeaveChannels(std::string ChannelNameList) {
	LogInfo("Client: [{0}] leaving channels [{1}]", GetName().c_str(),
	        ChannelNameList.c_str());
	...
	while (CurrentPos != std::string::npos) {
		...
		if (Comma == std::string::npos) {
			ChatChannel *JoinedChannel = ChannelList->RemoveClientFromChannel(
			    ChannelNameList.substr(CurrentPos), this);

			if (JoinedChannel) RemoveFromChannelList(JoinedChannel);
...
void Client::LeaveAllChannels(bool SendUpdatedChannelList) {
	...
	if (SendUpdatedChannelList) SendChannelList();
}
...
void Client::SendChannelList() {
	...
	for (int i = 0; i < MAX_JOINED_CHANNELS; i++) {
		if (JoinedChannels[i] != nullptr) {
			if (ChannelCount) ChannelMessage = ChannelMessage + ",";
...
void Client::SendChannelMessage(std::string Message) {
	...
	LogInfo("[{0}] tells [{1}], [[{2}]]", GetName().c_str(),
	        ChannelName.c_str(), Message.substr(MessageStart + 1).c_str());
	...
	if (RequiredChannel) {
		if (RuleB(Chat, EnableAntiSpam)) {
			if (!RequiredChannel->IsModerated() ||
				...
				int AllowedMessages =
				    RuleI(Chat, MinimumMessagesPerInterval) + GetKarma();
				AllowedMessages =
				    AllowedMessages > RuleI(Chat, MaximumMessagesPerInterval)
				        ? RuleI(Chat, MaximumMessagesPerInterval)
				        : AllowedMessages;
				...
				if (AttemptedMessages > AllowedMessages) {
					if (AttemptedMessages >
					    RuleI(Chat, MaxMessagesBeforeKick)) {
						ForceDisconnect = true;
					}
					if (GlobalChatLimiterTimer) {
						char TimeLeft[256];
						sprintf(TimeLeft,
						        "You are currently rate limited, you cannot "
						        "send more messages for %i seconds.",
						        (GlobalChatLimiterTimer->GetRemainingTime() /
						         1000));
					...
					RequiredChannel->SendMessageToChannel(
					    Message.substr(MessageStart + 1), this);
				}
			} else
				GeneralChannelMessage(
				    "Channel " + ChannelName +
...
void Client::SendChannelMessageByNumber(std::string Message) {
	...
	Log(Logs::Detail, Logs::UCSServer, "%s tells %s, [%s]", GetName().c_str(),
	    RequiredChannel->GetName().c_str(),
...
void Client::SendChannelMessage(std::string ChannelName, std::string Message,
                                Client *Sender) {
	if (!Sender) return;
	...
	int PacketLength =
	    ChannelName.length() + Message.length() + FQSenderName.length() + 3;
...
void Client::AnnounceLeave(ChatChannel *Channel, Client *c) {
	...
	auto outapp =
	    new EQApplicationPacket(OP_ChannelAnnounceLeave, PacketLength);
...
void Client::GeneralChannelMessage(std::string Message) {
	auto outapp =
	    new EQApplicationPacket(OP_ChannelMessage, Message.length() + 3);
	char *PacketBuffer = (char *)outapp->pBuffer;
	VARSTRUCT_ENCODE_TYPE(uint8, PacketBuffer, 0x00);
	VARSTRUCT_ENCODE_TYPE(uint8, PacketBuffer, 0x00);
	VARSTRUCT_ENCODE_STRING(PacketBuffer, Message.c_str());
...
void Client::SetChannelPassword(std::string ChannelPassword) {
	std::string::size_type PasswordStart =
	    ChannelPassword.find_first_not_of(" ");

	if (PasswordStart == std::string::npos) {
		std::string Message =
		    "Incorrect syntax: /chat password <new password> <channel name>";
		GeneralChannelMessage(Message);
		return;
	}

	std::string::size_type Space =
	    ChannelPassword.find_first_of(" ", PasswordStart);
	...
	std::string Password =
	    ChannelPassword.substr(PasswordStart, Space - PasswordStart);

	std::string::size_type ChannelStart =
	    ChannelPassword.find_first_not_of(" ", Space);
	...
	LogInfo("Set password of channel [[{0}]] to [[{1}]] by [{2}]",
	        ChannelName.c_str(), Password.c_str(), GetName().c_str());
	...
	if (!RequiredChannel->IsOwner(GetName()) &&
	    !RequiredChannel->IsModerator(GetName()) && !IsChannelAdmin()) {
		std::string Message =
		    "You do not own or have moderator rights on channel " + ChannelName;
		GeneralChannelMessage(Message);
		return;
	}
...
void Client::SetChannelOwner(std::string CommandString) {
	...
	if (PlayerStart == std::string::npos) {
		std::string Message =
		    "Incorrect syntax: /chat setowner <player> <channel>";
		GeneralChannelMessage(Message);
		return;
	}

	std::string::size_type Space =
	    CommandString.find_first_of(" ", PlayerStart);
	...
	std::string NewOwner =
	    CapitaliseName(CommandString.substr(PlayerStart, Space - PlayerStart));
	...
	std::string ChannelName =
	    CapitaliseName(CommandString.substr(ChannelStart));

	if ((ChannelName.length() > 0) && isdigit(ChannelName[0]))
		ChannelName = ChannelSlotName(atoi(ChannelName.c_str()));

	LogInfo("Set owner of channel [[{0}]] to [[{1}]]", ChannelName.c_str(),
	        NewOwner.c_str());
...
void Client::ChannelInvite(std::string CommandString) {
	...
	if (PlayerStart == std::string::npos) {
		std::string Message =
		    "Incorrect syntax: /chat invite <player> <channel>";
		GeneralChannelMessage(Message);
		return;
	}
	...
	LogInfo("[[{0}]] invites [[{1}]] to channel [[{2}]]", GetName().c_str(),
	        Invitee.c_str(), ChannelName.c_str());
	...
	if (!RequiredChannel->IsOwner(GetName()) &&
	    !RequiredChannel->IsModerator(GetName())) {
		std::string Message =
		    "You do not own or have moderator rights to channel " + ChannelName;
	...
	RequiredClient->GeneralChannelMessage(
	    GetName() + " has invited you to join channel " + ChannelName);

	GeneralChannelMessage("Invitation sent to " + Invitee +
	                      " to join channel " + ChannelName);
}
...
void Client::ChannelGrantModerator(std::string CommandString) {
	...
	if (PlayerStart == std::string::npos) {
		GeneralChannelMessage(
		    "Incorrect syntax: /chat grant <player> <channel>");
		return;
	}
	...
	LogInfo("[[{0}]] gives [[{1}]] moderator rights to channel [[{2}]]",
	        GetName().c_str(), Moderator.c_str(), ChannelName.c_str());
	...
	if (RequiredChannel->IsModerator(Moderator)) {
		...
		if (RequiredClient)
			RequiredClient->GeneralChannelMessage(
			    GetName() + " has removed your moderator rights to channel " +
			    ChannelName);

		GeneralChannelMessage("Removing moderator rights from " + Moderator +
		...
		if (RequiredClient)
			RequiredClient->GeneralChannelMessage(
			    GetName() + " has made you a moderator of channel " +
			    ChannelName);

		GeneralChannelMessage(Moderator + " is now a moderator on channel " +
		                      ChannelName);
	}
}
...
void Client::ChannelGrantVoice(std::string CommandString) {
	...
	if (PlayerStart == std::string::npos) {
		GeneralChannelMessage(
		    "Incorrect syntax: /chat voice <player> <channel>");
		return;
	}
	...
	LogInfo("[[{0}]] gives [[{1}]] voice to channel [[{2}]]", GetName().c_str(),
	        Voicee.c_str(), ChannelName.c_str());
	...
	if (RequiredChannel->HasVoice(Voicee)) {
		...
		if (RequiredClient)
			RequiredClient->GeneralChannelMessage(
			    GetName() + " has removed your voice rights to channel " +
			    ChannelName);

		GeneralChannelMessage("Removing voice from " + Voicee + " in channel " +
		                      ChannelName);
	} else {
		...
		if (RequiredClient)
			RequiredClient->GeneralChannelMessage(
			    GetName() + " has given you voice in channel " + ChannelName);

		GeneralChannelMessage(Voicee + " now has voice in channel " +
		                      ChannelName);
	}
}
...
void Client::ChannelKick(std::string CommandString) {
	...
	if (PlayerStart == std::string::npos) {
		GeneralChannelMessage(
		    "Incorrect syntax: /chat kick <player> <channel>");
		return;
	}
	...
	LogInfo("[[{0}]] kicks [[{1}]] from channel [[{2}]]", GetName().c_str(),
	        Kickee.c_str(), ChannelName.c_str());
	...
	if (RequiredChannel->IsModerator(Kickee)) {
		...
		GeneralChannelMessage("Removing moderator rights from " + Kickee +
		                      " to channel " + ChannelName);
	}

	RequiredClient->GeneralChannelMessage(
	    GetName() + " has kicked you from channel " + ChannelName);
...
std::string Client::ChannelSlotName(int SlotNumber) {
	if ((SlotNumber < 1) || (SlotNumber > MAX_JOINED_CHANNELS)) return "";

	if (JoinedChannels[SlotNumber - 1] == nullptr) return "";
...
void Client::SendHelp() {
	...
	GeneralChannelMessage(
	    "/join, /leave, /leaveall, /list, /announce, /autojoin, ;set");
	GeneralChannelMessage(
	    ";oplist, ;grant, ;invite, ;kick, ;moderate, ;password, ;voice");
...
Client *Clientlist::IsCharacterOnline(std::string CharacterName) {
	// This method is used to determine if the character we are a sending an
	// email to is connected to the mailserver, so we can send them a new email
	// notification.
	//
	// The way live works is that it sends a notification if a player receives
	// an email for their 'primary' mailbox, i.e. for the character they are
	// logged in as, or for the character whose mailbox they have selected in
	// the mail window.
	//
