...
ChatChannel::ChatChannel(std::string inName, std::string inOwner,
                         std::string inPassword, bool inPermanent,
                         int inMinimumStatus)
    : DeleteTimer(0) {
	Name = inName;
	...
	LogDebug(
	    "New ChatChannel created: Name: [[{0}]], Owner: [[{1}]], Password: "
	    "[[{2}]], MinStatus: [{3}]",
...
ChatChannel::~ChatChannel() {
	...
	while (iterator.MoreElements()) iterator.RemoveCurrent(false);
}

ChatChannel *ChatChannelList::CreateChannel(std::string Name, std::string Owner,
                                            std::string Password,
                                            bool Permanent, int MinimumStatus) {
	ChatChannel *NewChannel = new ChatChannel(
	    CapitaliseName(Name), Owner, Password, Permanent, MinimumStatus);
...
void ChatChannelList::SendAllChannels(Client *c) {
	...
	if (!c->CanListAllChannels()) {
		c->GeneralChannelMessage(
		    "You do not have permission to list all the channels.");
		return;
	}
	...
	while (iterator.MoreElements()) {
		...
		if (!CurrentChannel ||
		    (CurrentChannel->GetMinStatus() > c->GetAccountStatus())) {
			iterator.Advance();

			continue;
		}

		if (ChannelsInLine > 0) Message += ", ";

		sprintf(CountString, "(%i)",
		        CurrentChannel->MemberCount(c->GetAccountStatus()));
	...
	if (ChannelsInLine > 0) c->GeneralChannelMessage(Message);
}
...
void ChatChannelList::RemoveAllChannels() {
	...
	while (iterator.MoreElements()) iterator.RemoveCurrent();
}
...
int ChatChannel::MemberCount(int Status) {
	...
	while (iterator.MoreElements()) {
		...
		if (ChannelClient && (!ChannelClient->GetHideMe() ||
		                      (ChannelClient->GetAccountStatus() < Status)))
			Count++;
...
void ChatChannel::SetOwner(std::string inOwner) {
	...
	if (Permanent) database.SetChannelOwner(Name, Owner);
}
...
void ChatChannel::AddClient(Client *c) {
	...
	if (IsClientInChannel(c)) {
		LogInfo("Client [{0}] already in channel [{1}]", c->GetName().c_str(),
		        GetName().c_str());
	...
	LogDebug("Adding [{0}] to channel [{1}]", c->GetName().c_str(),
	         Name.c_str());
...
bool ChatChannel::RemoveClient(Client *c) {
	...
	LogDebug("RemoveClient [{0}] from channel [{1}]", c->GetName().c_str(),
	         GetName().c_str());
	...
	while (iterator.MoreElements()) {
		...
		if (CurrentClient == c) {
			...
			if (CurrentClient->IsAnnounceOn())
				if (!HideMe ||
				    (CurrentClient->GetAccountStatus() > AccountStatus))
	...
	if ((PlayersInChannel == 0) && !Permanent) {
		...
		LogDebug(
		    "Starting delete timer for empty password protected channel [{0}]",
		    Name.c_str());
...
void ChatChannel::SendOPList(Client *c) {
	...
	for (auto &&m : Moderators) c->GeneralChannelMessage(m);
}
...
void ChatChannel::SendChannelMembers(Client *c) {
	...
	while (iterator.MoreElements()) {
		...
		// Don't list hidden characters with status higher or equal than the
		// character requesting the list.
		//
		if (!ChannelClient ||
		    (ChannelClient->GetHideMe() &&
		     (ChannelClient->GetAccountStatus() >= AccountStatus))) {
			iterator.Advance();
			continue;
		}

		if (MembersInLine > 0) Message += ", ";
	...
	if (MembersInLine > 0) c->GeneralChannelMessage(Message);
}
...
void ChatChannel::SetModerated(bool inModerated) {
	...
	while (iterator.MoreElements()) {
		...
		if (ChannelClient && !ChannelClient->GetStale()) {
			if (Moderated)
				ChannelClient->GeneralChannelMessage("Channel " + Name +
				                                     " is now moderated.");
			else
				ChannelClient->GeneralChannelMessage(
				    "Channel " + Name + " is no longer moderated.");
		}
...
bool ChatChannel::IsClientInChannel(Client *c) {
	...
	while (iterator.MoreElements()) {
		if (iterator.GetData() == c) return true;
...
ChatChannel *ChatChannelList::AddClientToChannel(std::string ChannelName,
                                                 Client *c) {
	if (!c) return nullptr;

	if ((ChannelName.length() > 0) && (isdigit(ChannelName[0]))) {
		c->GeneralChannelMessage(
		    "The channel name can not begin with a number.");
	...
	if ((NormalisedName.length() > 64) || (Password.length() > 64)) {
		c->GeneralChannelMessage(
		    "The channel name or password cannot exceed 64 characters.");

		return nullptr;
	}

	LogDebug("AddClient to channel [[{0}]] with password [[{1}]]",
	         NormalisedName.c_str(), Password.c_str());

	ChatChannel *RequiredChannel = FindChannel(NormalisedName);

	if (!RequiredChannel)
		RequiredChannel =
		    CreateChannel(NormalisedName, c->GetName(), Password, false, 0);

	if (RequiredChannel->GetMinStatus() > c->GetAccountStatus()) {
		std::string Message =
		    "You do not have the required account status to join channel " +
		    NormalisedName;
	...
	if (RequiredChannel->IsClientInChannel(c)) return nullptr;
	...
	if (RequiredChannel->CheckPassword(Password) ||
	    ...
	    RequiredChannel->IsModerator(c->GetName()) || c->IsChannelAdmin()) {
		RequiredChannel->AddClient(c);

		return RequiredChannel;
	}

	c->GeneralChannelMessage("Incorrect password for channel " +
	                         (NormalisedName));

	return nullptr;
}

ChatChannel *ChatChannelList::RemoveClientFromChannel(std::string inChannelName,
                                                      Client *c) {
	if (!c) return nullptr;
	...
	if (!RequiredChannel) return nullptr;

	// RemoveClient will return false if there is no-one left in the channel,
	// and the channel is not permanent and has no password.
	//
	if (!RequiredChannel->RemoveClient(c)) RemoveChannel(RequiredChannel);
...
void ChatChannelList::Process() {
	...
	while (iterator.MoreElements()) {
		...
		if (CurrentChannel && CurrentChannel->ReadyToDelete()) {
			LogDebug(
			    "Empty temporary password protected channel [{0}] being "
			    "destroyed",
...
void ChatChannel::AddInvitee(const std::string &Invitee) {
	if (!IsInvitee(Invitee)) {
		...
		LogDebug("Added [{0}] as invitee to channel [{1}]", Invitee.c_str(),
		         Name.c_str());
	}
}
...
void ChatChannel::RemoveInvitee(std::string Invitee) {
	...
	if (it != std::end(Invitees)) {
		Invitees.erase(it);
		LogDebug("Removed [{0}] as invitee to channel [{1}]", Invitee.c_str(),
		         Name.c_str());
	}
}

bool ChatChannel::IsInvitee(std::string Invitee) {
	return std::find(std::begin(Invitees), std::end(Invitees), Invitee) !=
	       std::end(Invitees);
}
...
void ChatChannel::AddModerator(const std::string &Moderator) {
	if (!IsModerator(Moderator)) {
		...
		LogInfo("Added [{0}] as moderator to channel [{1}]", Moderator.c_str(),
		        Name.c_str());
	}
}
...
void ChatChannel::RemoveModerator(const std::string &Moderator) {
	auto it =
	    std::find(std::begin(Moderators), std::end(Moderators), Moderator);

	if (it != std::end(Moderators)) {
		Moderators.erase(it);
		LogInfo("Removed [{0}] as moderator to channel [{1}]",
		        Moderator.c_str(), Name.c_str());
	}
}

bool ChatChannel::IsModerator(std::string Moderator) {
	return std::find(std::begin(Moderators), std::end(Moderators), Moderator) !=
	       std::end(Moderators);
}
...
void ChatChannel::AddVoice(const std::string &inVoiced) {
	if (!HasVoice(inVoiced)) {
		...
		LogInfo("Added [{0}] as voiced to channel [{1}]", inVoiced.c_str(),
		        Name.c_str());
	}
}
...
void ChatChannel::RemoveVoice(const std::string &inVoiced) {
	...
	if (it != std::end(Voiced)) {
		...
		LogInfo("Removed [{0}] as voiced to channel [{1}]", inVoiced.c_str(),
		        Name.c_str());
	}
}

bool ChatChannel::HasVoice(std::string inVoiced) {
	return std::find(std::begin(Voiced), std::end(Voiced), inVoiced) !=
	       std::end(Voiced);
}
